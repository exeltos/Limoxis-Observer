...
export function BundleExecutionEditor({onCancel,onSave,fixedDepartment='',initialRecord=null,departments=[],templates=[]}){
 ...
 const [draft,setDraft]=useState(()=>initialRecord?JSON.parse(JSON.stringify(initialRecord)):{templateId:'',departmentEl:fixedDepartment||'',date:'',shift:'',context:'',patientId:'',patientRef:'',deviceId:'',deviceRef:'',answers:{},answerNotes:{},generalNotes:'',status:'draft'})
 ...
 const template=templates.find(x=>x.id===draft.templateId||x.bundleKey===draft.templateId)||null
 ...
 return <div className="bundle-page-editor">
  ...
  <section className="bundle-page-context"><div className="bundle-page-grid">
   ...
   <label><span>{en?'Shift / context':'Βάρδια / πλαίσιο'}</span><select value={draft.shift} onChange={e=>set('shift',e.target.value)}><option value="">{en?'Select shift / context':'Επιλέξτε βάρδια / πλαίσιο'}</option><option value="Πρωινή">{en?'Morning':'Πρωινή'}</option><option value="Απογευματινή">{en?'Afternoon':'Απογευματινή'}</option><option value="Νυχτερινή">{en?'Night':'Νυχτερινή'}</option><option value="Άλλο">{en?'Other':'Άλλο'}</option></select></label>
  ...
  {template?<section className="bundle-page-checklist"><div className="bundle-page-checklist-head"><div><strong>{en?'Bundle elements':'Στοιχεία δέσμης μέτρων'}</strong><small>{en?'You can save at any time. A complete checklist is finalized automatically.':'Μπορείτε να αποθηκεύσετε οποιαδήποτε στιγμή. Όταν απαντηθούν όλα τα στοιχεία, η αξιολόγηση ολοκληρώνεται αυτόματα.'}</small></div><div className="bundle-page-progress"><span>{en?'Answered':'Απαντημένα'} {answered}/{elements.length}</span><strong>{score===null?'—':`${score}%`}</strong><small>{en?(allOrNone?'All-or-none ✓':'All-or-none —'):(allOrNone?'Πλήρης συμμόρφωση ✓':'Πλήρης συμμόρφωση —')}</small></div></div><div className="bundle-page-rows">{elements.map(([id,label],index)=>{const value=draft.answers[id];return <div className={`bundle-page-row ${value||'unanswered'}`} key={id}><div className="bundle-page-row-main"><span className="bundle-page-index">{index+1}</span><strong>{elementLabel(id,label)}</strong><div className="bundle-page-answers">{[['yes',en?'Yes':'Ναι'],['no',en?'No':'Όχι'],['na',en?'N/A':'Μ/Ε']].map(([answerValue,text])=><button type="button" key={answerValue} className={value===answerValue?'active':''} onClick={()=>answer(id,answerValue)}>{text}</button>)}</div></div>{value==='no'&&<div className="bundle-page-deviation"><ShieldAlert size={15}/><input value={draft.answerNotes[id]||''} onChange={e=>note(id,e.target.value)} placeholder={en?'Deviation / action required':'Απόκλιση / ενέργεια που απαιτείται'}/></div>}</div>})}</div></section>:<div className="inline-empty">{en?'Select a Bundle to begin the assessment.':'Επιλέξτε Bundle για να ξεκινήσει η αξιολόγηση.'}</div>}
  <div className="bundle-page-summary"><span><b>{en?'Answered':'Απαντημένα'}</b>{answered}/{elements.length}</span><span><b>{en?'Applicable':'Εφαρμόσιμα'}</b>{applicable}</span><span><b>{en?'Deviations':'Αποκλίσεις'}</b>{failures.length}</span><span><b>{en?'Compliance':'Συμμόρφωση'}</b>{score===null?'—':`${score}%`}</span><span><b>{en?'All-or-none':'Πλήρης συμμόρφωση'}</b>{applicable===0?'—':allOrNone?(en?'Yes':'Ναι'):(en?'No':'Όχι')}</span></div>

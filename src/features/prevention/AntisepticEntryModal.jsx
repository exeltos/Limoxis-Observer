...
export function AntisepticEntryModal({onClose,onSave,fixedDepartment='',initialRecord=null,departments=[],products=[],findPatientDays}){
 ...
 const [draft,setDraft]=useState(()=>initialRecord?JSON.parse(JSON.stringify(initialRecord)):{
  period:currentMonth,departmentEl:initialDepartment,product:initialProduct,antisepticItemId:products.find(x=>x.el===initialProduct)?.id||'',
  litres:'',patientDays:'',patientDaysSource:'',method:'pharmacy_issue',referenceNumber:'',responsible:actor.name,notes:'',status:'completed'
 })
 ...
 const productInfo=products.find(d=>d.id===draft.antisepticItemId||d.el===draft.product)
 ...
 useEffect(()=>{
  ...
  async function load(){
   ...
   try{
    const value=await findPatientDays(departmentInfo.id,range.from,range.to)
    if(active)setSuggestedPatientDays(value||'')
...
function DepartmentField({value,onChange,departments,fixed}){
 ...
 return <label><span>{language==='en'?'Department *':'Τμήμα *'}</span><select value={value} disabled={fixed} onChange={e=>onChange(e.target.value)}>{departments.map(d=><option key={d.id||d.el} value={d.el}>{language==='en'?(d.en||d.el):d.el}</option>)}</select></label>
}

  ...
  <div className="waste-smart-layout">
   ...
   <aside className="waste-smart-rail">
    <section className="waste-form-section waste-smart-indicator">
     ...
     <div className="waste-indicator-hero"><strong>{indicator===null?'—':indicator.toLocaleString(locale)}</strong><span>{en?'kg / 1,000 patient-days':'kg / 1.000 νοσηλευτικές ημέρες'}</span></div>
     <label className="waste-patient-days-control"><span>{en?'Patient-days':'Νοσηλευτικές ημέρες'}</span><input type="number" min="0" value={draft.patientDays||''} onChange={event=>setDraft(state=>({...state,patientDays:event.target.value,patientDaysSource:'manual'}))} placeholder={suggestedPatientDays?String(suggestedPatientDays):(en?'No available period':'Δεν υπάρχει διαθέσιμη περίοδος')}/></label>
     {suggestedPatientDays&&<button type="button" className={`waste-library-source ${usingSuggestedPatientDays?'applied':''}`} onClick={()=>setDraft(state=>({...state,patientDays:suggestedPatientDays,patientDaysSource:'library'}))}>{usingSuggestedPatientDays?(en?'✓ Using Library value':'✓ Χρήση τιμής Βιβλιοθήκης'):(en?`Use Library value: ${suggestedPatientDays}`:`Χρήση τιμής Βιβλιοθήκης: ${suggestedPatientDays}`)}</button>}
     <div className="waste-source-note"><span>{usingSuggestedPatientDays?(en?'Source: Bed-days Library':'Πηγή: Βιβλιοθήκη νοσηλευτικών ημερών'):(draft.patientDays?(en?'Source: Manual entry':'Πηγή: Χειροκίνητη καταχώριση'):(en?'No denominator selected':'Δεν έχει επιλεγεί παρονομαστής'))}</span></div>
    </section>
    <div className="prevention-entry-actor waste-smart-actor"><span>{initialRecord?(en?'Edited by':'Επεξεργασία από'):(en?'Recorded by':'Καταχώρηση από')}</span><strong>{actor.name}</strong><small>{actor.email}</small></div>
   </aside>
  </div>

   ...
   <main className="waste-smart-main">
    <section className="waste-form-section waste-smart-section">
     ...
     <div className="entry-grid waste-smart-measurement-grid">
      <ManualDateField label={en?'Period start *':'Έναρξη περιόδου *'} value={draft.periodStart} disabled={readOnly} onChange={value=>set('periodStart',value)}/>
      <ManualDateField label={en?'Period end *':'Λήξη περιόδου *'} value={draft.periodEnd} disabled={readOnly} onChange={value=>set('periodEnd',value)}/>
      ...
      <label><span>{en?'Waste category *':'Κατηγορία αποβλήτου *'}</span><select className="waste-category-select" value={draft.wasteTypeId||typeInfo?.id||''} disabled={readOnly} onChange={event=>changeWasteType(event.target.value)}>{wasteTypes.map(item=><option key={item.id} value={item.id}>{en?(item.en||item.el):item.el}</option>)}</select></label>
      <label><span>{en?'Total weight in period (kg) *':'Συνολικό βάρος περιόδου (kg) *'}</span><input type="number" min="0" step="0.1" value={draft.weight} readOnly={readOnly} onChange={event=>set('weight',event.target.value)} placeholder="0,0"/></label>

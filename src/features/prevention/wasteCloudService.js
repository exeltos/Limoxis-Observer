...
export async function findWastePatientDays(organizationId,departmentId,date){
 ...
 const {data,error}=await supabase.from('patient_day_periods').select('patient_days,period_start,period_end').eq('organization_id',organizationId).eq('department_id',departmentId).lte('period_start',date).gte('period_end',date).order('period_start',{ascending:false}).limit(1).maybeSingle()
 if(error)throw error
...
export async function loadWasteMeasurements(organizationId){
 assertCloud(organizationId)
 const {data,error}=await supabase.from('waste_measurements').select('*,department:departments(id,name),waste_type:master_library_items(id,code,name_el,name_en)').eq('organization_id',organizationId).order('record_date',{ascending:false}).order('created_at',{ascending:false})
 if(error)throw error

...
import { createVaccinationsBulkAsync,deleteVaccinationAsync,loadAllVaccinationsAsync,updateVaccinationAsync } from '../occupational-health/vaccinationService'
...
const tabIcons={handHygiene:ClipboardCheck,waste:Recycle,antiseptics:Droplets,bundles:ShieldCheck,vaccinations:ShieldCheck}
...
function fmtBundleDate(value){const [y,m,d]=String(value||'').slice(0,10).split('-');return y&&m&&d?`${Number(d)}/${Number(m)}/${y}`:(value||'—')}
export function PreventionPage(){
 ...
 const reloadWaste=useCallback(async()=>{if(!tenant?.id||!tabAccess.waste){setWasteRows([]);setWasteSupport({departments:[],wasteTypes:[]});return}setWasteLoading(true);try{const [records,support]=await Promise.all([loadWasteMeasurements(tenant.id),loadWasteSupportData(tenant.id)]);setWasteRows(records);setWasteSupport(support)}catch(error){notifyError(error,'load',{operation:'waste_load'})}finally{setWasteLoading(false)}},[tenant?.id,tabAccess.waste,notifyError])
 const reloadAntiseptics=useCallback(async()=>{if(!tenant?.id||!tabAccess.antiseptics){setAntisepticRows([]);setAntisepticSupport({departments:[],products:[]});return}setAntisepticLoading(true);try{const [records,support]=await Promise.all([loadAntisepticRecords(tenant.id),loadAntisepticSupportData(tenant.id)]);setAntisepticRows(records);setAntisepticSupport(support)}catch(error){notifyError(error,'load',{operation:'antiseptic_load'})}finally{setAntisepticLoading(false)}},[tenant?.id,tabAccess.antiseptics,notifyError])
 const reloadBundles=useCallback(async()=>{if(!tenant?.id||!tabAccess.bundles){setBundleRows([]);setBundleSupport({departments:[],templates:[]});return}setBundleLoading(true);try{const [records,support]=await Promise.all([loadBundleAssessments(tenant.id),loadBundleSupportData(tenant.id)]);setBundleRows(records);setBundleSupport(support)}catch(error){notifyError(error,'load',{operation:'bundle_load'})}finally{setBundleLoading(false)}},[tenant?.id,tabAccess.bundles,notifyError])
 const reloadVaccinations=useCallback(async()=>{if(!tenant?.id||!tabAccess.vaccinations){setVaccinationRows([]);return}setVaccinationLoading(true);try{setVaccinationRows(await loadAllVaccinationsAsync(tenant.id))}catch(error){notifyError(error,'load',{operation:'staff_vaccinations_load'})}finally{setVaccinationLoading(false)}},[tenant?.id,tabAccess.vaccinations,notifyError])
 ...
 useEffect(()=>{if(!tenant?.id)return;if(tabAccess.handHygiene)void reloadHandHygiene();else{setHandRows([]);setHandDepartments([])}if(tabAccess.waste)void reloadWaste();else{setWasteRows([]);setWasteSupport({departments:[],wasteTypes:[]})}if(tabAccess.antiseptics)void reloadAntiseptics();else{setAntisepticRows([]);setAntisepticSupport({departments:[],products:[]})}if(tabAccess.bundles)void reloadBundles();else{setBundleRows([]);setBundleSupport({departments:[],templates:[]})}if(tabAccess.vaccinations)void reloadVaccinations();else setVaccinationRows([])},[tenant?.id,tabAccess.handHygiene,tabAccess.waste,tabAccess.antiseptics,tabAccess.bundles,tabAccess.vaccinations,reloadHandHygiene,reloadWaste,reloadAntiseptics,reloadBundles,reloadVaccinations])
 ...
 // WHO method: pooled compliant actions over all observed opportunities (not an average of session rates).
 const handOpportunities=handRows.reduce((sum,x)=>sum+(Number(x.observations)||0),0),handCompliant=handRows.reduce((sum,x)=>sum+(Number(x.compliant)||0),0)
 const avg=handOpportunities?handCompliant/handOpportunities*100:0
 const scoredBundles=bundleRows.filter(x=>x.score!=null),bundleAvg=scoredBundles.length?scoredBundles.reduce((sum,x)=>sum+Number(x.score),0)/scoredBundles.length:null
 const fmtOne=value=>new Intl.NumberFormat(language==='en'?'en-GB':'el-GR',{maximumFractionDigits:1}).format(value)
 const fmtDate=v=>v?new Intl.DateTimeFormat(locale).format(new Date(`${String(v).slice(0,10)}T12:00:00`)):'—'
 const createLabel=tab==='vaccinations'?t('newVaccinationRecord'):((createLabelKeys[tab]&&t(createLabelKeys[tab]))||t('preventionNewRecordFallback'))
 ...
 function openVaccinationRecord(row){registry.saveViewState({tab:'vaccinations',query,department,period,product,method,status});setVaccinationEditor({mode:'record',draft:{...row},selectedEmployeeIds:[row.employeeId],selectedEmployee:employeeMap[row.employeeId]||null,readOnly:true})}
 ...
 async function saveVaccination(payload){try{if(payload.mode==='edit'){await updateVaccinationAsync(tenant.id,payload.draft.id,payload.draft);notify(language==='en'?'Vaccination updated.':t('vaccinationUpdated'),'success')}else{const selected=employeeRows.filter(x=>payload.selectedEmployeeIds.includes(x.id));await createVaccinationsBulkAsync(tenant.id,selected,payload.draft);notify(language==='en'?`${selected.length} vaccination record${selected.length===1?'':'s'} saved.`:`Αποθηκεύτηκαν ${selected.length} εμβολιασμ${selected.length===1?'ός':'οί'}.`,'success')}setVaccinationEditor(null);await reloadVaccinations()}catch(error){notifyError(error,'save',{operation:payload.mode==='edit'?'staff_vaccinations_update':'staff_vaccinations_create'})}}
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  <div className="workspace-summary prevention-summary"><div className="module-summary-strip">{tabAccess.handHygiene&&<Kpi icon={ShieldCheck} value={`${fmtOne(avg)}%`} label={t('whoCompliance')}/>} {tabAccess.bundles&&<Kpi icon={ClipboardCheck} value={bundleAvg==null?'—':`${fmtOne(bundleAvg)}%`} label={t('bundleComplianceKpi')}/>} {tabAccess.waste&&<Kpi icon={Recycle} value={`${fmtOne(wasteRows.reduce((s,x)=>s+x.weight,0))} kg`} label={t('wasteRecorded')}/>} {tabAccess.antiseptics&&<Kpi icon={Droplets} value={`${fmtOne(antisepticRows.reduce((s,x)=>s+x.litres,0))} L`} label={t('antisepticRecorded')}/>}</div></div>
  <div className="surface registry-workspace prevention-workspace workspace-fill"><nav className="entity-record-tabs surface prevention-tabs" role="tablist" aria-label={t('preventionCenter')}>{visibleTabs.map(([id,key])=>{const Icon=tabIcons[id];return <button key={id} type="button" role="tab" aria-selected={tab===id} className={tab===id?'active':''} onClick={()=>changeTab(id)}>{Icon&&<Icon size={16}/>}<span>{t(key)}</span></button>})}</nav>
   ...
   <div className="scroll-table" ref={registry.scrollRef}>{loading&&<div className="registry-empty-state"><strong>{t('preventionLoadingRecords')}</strong></div>}{tab==='handHygiene'&&!loading&&<HandTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'handHygiene')} registry={registry}/>} {tab==='waste'&&!loading&&<WasteTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'waste')} registry={registry}/>} {tab==='antiseptics'&&!loading&&<AntisepticTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'antiseptics')} registry={registry}/>} {tab==='bundles'&&!loading&&<BundleTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'bundles')} registry={registry}/>} {tab==='vaccinations'&&!loading&&<VaccinationTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} employeeMap={employeeMap} onOpen={openVaccinationRecord} registry={registry}/>} {!loading&&!rows.length&&<PreventionEmpty t={t} tab={tab} language={language}/>}</div>
  ...
  {vaccinationEditor&&<StaffVaccinationEditor language={language} employees={employeeRows.filter(x=>x.employmentStatus==='active'&&canAccessRecord(x))} value={vaccinationEditor} canManage={canCreateRecord} onChange={setVaccinationEditor} onClose={()=>setVaccinationEditor(null)} onSave={saveVaccination} onDelete={deleteVaccination}/>} 
 </Page>
}
...
function PreventionEmpty({t,tab}){return <div className="registry-empty-state"><strong>{tab==='vaccinations'?t('noVaccinationRecords'):t(preventionEmptyTitleKeys[tab])}</strong><span>{t('preventionEmptyGenericText')}</span></div>}
...
function BundleTable({rows,t,language,onOpen,registry}){return <RegistryTable bare
  ...
  rows={rows} rowKey={x=>x.id} rowProps={x=>rowPropsWithOpen(registry,x.id,onOpen)}
  renderRow={x=><><td>{fmtBundleDate(x.date||x.period)}</td><td><strong>{language==='el'?(x.templateTitle||x.templateName||x.bundle):(x.templateName||x.bundle)}</strong><small className="table-cell-unit">v{x.templateVersion||'1.0'}</small></td><td>{language==='el'?x.departmentEl:x.departmentEn}</td><td>{t(`shift_${x.shift}`)!==`shift_${x.shift}`?t(`shift_${x.shift}`):(x.shift||'—')}</td><td><strong>{x.score==null?'—':`${x.score}%`}</strong></td><td><span className={`bundle-all-badge ${x.allOrNone?'passed':'failed'}`}>{x.allOrNone?t('yes'):t('no')}</span></td><td>{(x.failedCount??x.findings?.length??0)>0?<span className="bundle-finding-count">{x.failedCount??x.findings?.length}</span>:'—'}</td></>}
/>}

...
export function PreventionPage(){
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  {(canViewVaccinations||canViewStewardship)&&<div className="prevention-related-strip"><div className="prevention-related-copy"><strong>{language==='en'?'Related prevention workflows':'Συναφείς ροές πρόληψης'}</strong><span>{language==='en'?'Open the specialized record without duplicating data.':'Άνοιγμα της εξειδικευμένης καρτέλας χωρίς διπλή καταχώρηση δεδομένων.'}</span></div><div className="prevention-related-actions">{canViewVaccinations&&<button type="button" className="prevention-related-link" onClick={()=>navigate('/occupational-health?tab=vaccinations')}><Syringe size={16}/><span>{language==='en'?'Staff vaccinations':'Εμβολιασμοί προσωπικού'}</span></button>}{canViewStewardship&&<button type="button" className="prevention-related-link" onClick={()=>navigate('/pharmacy')}><Pill size={16}/><span>{language==='en'?'Antimicrobial stewardship':'Επιτήρηση αντιμικροβιακών'}</span></button>}</div></div>}
...
<<<<<<< HEAD
function PreventionEmpty({language,tab}){const en=language==='en';const names={handHygiene:en?'hand hygiene':'υγιεινής χεριών',waste:en?'waste':'αποβλήτων',antiseptics:en?'antiseptic consumption':'κατανάλωσης αντισηπτικών',bundles:en?'prevention bundles':'δεσμών μέτρων'};return <div className="registry-empty-state"><strong>{en?`No ${names[tab]} records`:`Δεν υπάρχουν καταγραφές ${names[tab]}`}</strong><span>{en?'No records have been entered for this organization yet.':'Δεν έχουν καταχωριστεί ακόμη δεδομένα για τον συγκεκριμένο οργανισμό.'}</span></div>}

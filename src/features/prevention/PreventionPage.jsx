...
import { ClipboardCheck,Droplets,Pencil,Plus,Recycle,ShieldCheck,Trash2 } from 'lucide-react'
...
export function PreventionPage(){
 ...
 async function saveEntry(record){if(tab==='handHygiene')return saveCloud(()=>saveHandHygieneSession(tenant.id,record,{existingId:editingRecord?.id||null}),reloadHandHygiene,'hand_hygiene');if(tab==='waste')return saveCloud(()=>saveWasteMeasurement(tenant.id,record,{existingId:editingRecord?.id||null}),reloadWaste,'waste');if(tab==='antiseptics')return saveCloud(()=>saveAntisepticRecord(tenant.id,record,{existingId:editingRecord?.id||null}),reloadAntiseptics,'antiseptic');return saveCloud(()=>saveBundleAssessment(tenant.id,record,{existingId:editingRecord?.id||null}),reloadBundles,'bundle')}
 async function saveCloud(saveFn,reloadFn,operation){try{await saveFn();await reloadFn();notify(editingRecord?t('preventionCorrectedSaved'):t('preventionSaved'),'success');setEntryOpen(false);setEditingRecord(null)}catch(error){notifyError(error,'save',{operation:editingRecord?`${operation}_update`:`${operation}_create`})}}
 ...
 async function deleteRow(record,event){
  event?.stopPropagation()
  const ok=await confirm({title:language==='en'?'Delete record':'Διαγραφή εγγραφής',message:language==='en'?'The record will be permanently deleted. Continue?':'Η εγγραφή θα διαγραφεί οριστικά. Θέλετε να συνεχίσετε;',confirmLabel:language==='en'?'Delete':'Διαγραφή',danger:true})
  if(!ok)return
  try{
   if(tab==='handHygiene')await deleteHandHygieneSession(tenant.id,record.id)
   else if(tab==='waste')await deleteWasteMeasurement(tenant.id,record.id)
   else if(tab==='antiseptics')await deleteAntisepticRecord(tenant.id,record.id)
   else await deleteBundleAssessment(tenant.id,record.id)
   if(tab==='handHygiene')await reloadHandHygiene();else if(tab==='waste')await reloadWaste();else if(tab==='antiseptics')await reloadAntiseptics();else await reloadBundles()
   notify(language==='en'?'Record deleted.':'Η εγγραφή διαγράφηκε.','success')
  }catch(error){notifyError(error,'delete',{operation:`${tab}_delete`})}
 }
 function createRecord(){if(!canCreateRecord)return;setEditingRecord(null);setEntryOpen(true)}
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={canCreateRecord?<ActionButton label={createLabel} tone="primary" onClick={createRecord}><Plus size={18}/><span>{createLabel}</span></ActionButton>:null}>
  ...
  <div className="surface registry-workspace prevention-workspace workspace-fill">
   ...
   <div className="scroll-table" ref={registry.scrollRef}>
    ...
    {tab==='handHygiene'&&!loading&&<HandTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'handHygiene')} registry={registry} canEdit={canCreateRecord} onEdit={editRow} onDelete={deleteRow}/>}
    {tab==='waste'&&!loading&&<WasteTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'waste')} registry={registry} canEdit={canCreateRecord} onEdit={editRow} onDelete={deleteRow}/>}
    {tab==='antiseptics'&&!loading&&<AntisepticTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'antiseptics')} registry={registry} canEdit={canCreateRecord} onEdit={editRow} onDelete={deleteRow}/>}
    {tab==='bundles'&&!loading&&<BundleTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'bundles')} registry={registry} canEdit={canCreateRecord} onEdit={editRow} onDelete={deleteRow}/>}

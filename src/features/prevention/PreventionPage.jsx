...
import { ClipboardCheck,Droplets,Recycle,ShieldCheck,Syringe,Pill } from 'lucide-react'
...
export function PreventionPage(){
 ...
 const tabAccess=useMemo(()=>Object.fromEntries(tabs.map(([id])=>[id,broadPreventionView||can(role,tabCapabilities[id],addOns,custom)])),[role,broadPreventionView,addOns,custom])
 ...
 const canViewVaccinations=can(role,CAPABILITIES.VIEW_OCCUPATIONAL_HEALTH,addOns,custom)
 const canViewStewardship=can(role,CAPABILITIES.VIEW_PHARMACY,addOns,custom)
 ...
 const createLabel=(createLabelKeys[tab]&&t(createLabelKeys[tab]))||t('preventionNewRecordFallback')
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  {(canViewVaccinations||canViewStewardship)&&<div className="prevention-related-strip"><div className="prevention-related-copy"><strong>{t('preventionRelatedWorkflowsTitle')}</strong><span>{t('preventionRelatedWorkflowsSubtitle')}</span></div><div className="prevention-related-actions">{canViewVaccinations&&<button type="button" className="prevention-related-link" onClick={()=>navigate('/occupational-health?tab=vaccinations')}><Syringe size={16}/><span>{t('preventionStaffVaccinationsLabel')}</span></button>}{canViewStewardship&&<button type="button" className="prevention-related-link" onClick={()=>navigate('/pharmacy')}><Pill size={16}/><span>{t('preventionAntimicrobialStewardshipLabel')}</span></button>}</div></div>}
  <div className="surface registry-workspace prevention-workspace workspace-fill"><nav className="tabs prevention-tabs canonical-module-tabs">{visibleTabs.map(([id,key])=><button key={id} className={`tab ${tab===id?'active':''}`} onClick={()=>changeTab(id)}>{t(key)}</button>)}</nav>
   ...
   <div className="scroll-table" ref={registry.scrollRef}>{loading&&<div className="registry-empty-state"><strong>{t('preventionLoadingRecords')}</strong></div>}{tab==='handHygiene'&&!loading&&<HandTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'handHygiene')} registry={registry}/>} {tab==='waste'&&!loading&&<WasteTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'waste')} registry={registry}/>} {tab==='antiseptics'&&!loading&&<AntisepticTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'antiseptics')} registry={registry}/>} {tab==='bundles'&&!loading&&<BundleTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'bundles')} registry={registry}/>} {!loading&&!rows.length&&<PreventionEmpty t={t} tab={tab}/>}</div>
...
function PreventionEmpty({t,tab}){return <div className="registry-empty-state"><strong>{t(preventionEmptyTitleKeys[tab])}</strong><span>{t('preventionEmptyGenericText')}</span></div>}

...
import { bundleRows,wasteRows } from './preventionDemoData'
...
export function PreventionPage(){
 ...
 },[tab,handDepartments,antisepticSupport.departments,source,language,version])
 ...
 useEffect(()=>{if(tenant?.id){void reloadHandHygiene();void reloadAntiseptics()}},[tenant?.id])
 ...
 async function saveEntry(record){
  ...
  const target=tab==='waste'?wasteRows:bundleRows
  if(editingRecord){
 ...
 const loading=tab==='handHygiene'?handLoading:tab==='antiseptics'?antisepticLoading:false

...
export function PreventionPage(){
 ...
 const createLabel=tab==='vaccinations'?t('newVaccinationEntry'):((createLabelKeys[tab]&&t(createLabelKeys[tab]))||t('preventionNewRecordFallback'))
 ...
 async function saveVaccination(payload){try{if(payload.mode==='edit'){await updateVaccinationAsync(tenant.id,payload.draft.id,payload.draft);notify(language==='en'?'Vaccination updated.':t('vaccinationUpdated') /* CI: vaccination actions */,'success')}else{const selected=employeeRows.filter(x=>payload.selectedEmployeeIds.includes(x.id));await createVaccinationsBulkAsync(tenant.id,selected,payload.draft);notify(language==='en'?\`${selected.length} vaccination record${selected.length===1?'':'s'} saved.\`:t('vaccinationsSaved',{count:selected.length}),'success')}setVaccinationEditor(null);await reloadVaccinations()}catch(error){notifyError(error,'save',{operation:payload.mode==='edit'?'staff_vaccinations_update':'staff_vaccinations_create'})}}
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  <div className="surface registry-workspace prevention-workspace workspace-fill"><nav className="entity-record-tabs surface prevention-tabs" role="tablist" aria-label={t('preventionCenter')}>{visibleTabs.map(([id,key])=>{const Icon=tabIcons[id];return <button key={id} type="button" role="tab" aria-selected={tab===id} className={tab===id?'active':''} onClick={()=>changeTab(id)}>{Icon&&<Icon size={16}/>}<span>{t(key)}</span></button>})}</nav>
   ...
   <div className="scroll-table" ref={registry.scrollRef}>{loading&&<div className="registry-empty-state"><strong>{tab==='vaccinations'?t('loadingVaccinations'):t('preventionLoadingRecords')}</strong></div>}{tab==='handHygiene'&&!loading&&<HandTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'handHygiene')} registry={registry}/>} {tab==='waste'&&!loading&&<WasteTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} onOpen={id=>openPreventionRecord(id,'waste')} registry={registry}/>} {tab==='antiseptics'&&!loading&&<AntisepticTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'antiseptics')} registry={registry}/>} {tab==='bundles'&&!loading&&<BundleTable rows={pagedRows} t={t} language={language} onOpen={id=>openPreventionRecord(id,'bundles')} registry={registry}/>} {tab==='vaccinations'&&!loading&&<VaccinationTable rows={pagedRows} t={t} language={language} fmtDate={fmtDate} employeeMap={employeeMap} onOpen={openVaccinationRecord} registry={registry}/>} {!loading&&!rows.length&&<PreventionEmpty t={t} tab={tab} language={language}/>}</div>

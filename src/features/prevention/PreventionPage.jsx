...
const tabs=[['handHygiene','handHygiene',ClipboardCheck],['waste','wasteManagement',Recycle],['antiseptics','antisepticConsumption',Droplets],['bundles','preventionBundles',ShieldCheck],['vaccinations','vaccinations',ShieldCheck]]
...
export function PreventionPage(){
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  <div className="surface registry-workspace prevention-workspace workspace-fill"><nav className="entity-record-tabs surface prevention-tabs" role="tablist" aria-label={t('preventionCenter')}>{visibleTabs.map(([id,key,Icon])=><button key={id} type="button" role="tab" aria-selected={tab===id} className={tab===id?'active':''} onClick={()=>changeTab(id)}>{Icon&&<Icon size={16}/>}<span>{t(key)}</span></button>)}</nav>

...
export function PreventionPage(){
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  <div className="surface registry-workspace prevention-workspace workspace-fill"><nav className="tabs prevention-tabs canonical-module-tabs">{visibleTabs.map(([id,key])=><button key={id} className={`tab ${tab===id?'active':''}`} onClick={()=>changeTab(id)}>{t(key)}</button>)}{canViewVaccinations&&<button type="button" className="tab prevention-vaccinations-tab" onClick={()=>navigate('/occupational-health?tab=vaccinations',{state:{returnTo:`/prevention?tab=${tab}`}})}>{t('vaccinations')}</button>}</nav>

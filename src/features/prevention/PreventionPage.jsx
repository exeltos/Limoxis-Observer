...
export function PreventionPage(){
 ...
 function openVaccinationRecord(row){registry.saveViewState({tab:'vaccinations',query,department,period,product,method,status});setVaccinationEditor({mode:'record',draft:{...row},selectedEmployeeIds:[row.employeeId],readOnly:true})}
 ...
 async function saveVaccination(payload){try{if(payload.mode==='edit'){await updateVaccinationAsync(tenant.id,payload.draft.id,payload.draft);notify(language==='en'?'Vaccination updated.':'Ο εμβολιασμός ενημερώθηκε.','success')}else{const selected=employeeRows.filter(x=>payload.selectedEmployeeIds.includes(x.id));await createVaccinationsBulkAsync(tenant.id,selected,payload.draft);notify(language==='en'?`${selected.length} vaccination record${selected.length===1?'':'s'} saved.`:`Αποθηκεύτηκαν ${selected.length} ${selected.length===1?'εμβολιασμός':'εμβολιασμοί'}.`,'success')}setVaccinationEditor(null);await reloadVaccinations()}catch(error){notifyError(error,'save',{operation:payload.mode==='edit'?'staff_vaccinations_update':'staff_vaccinations_create'})}}
 async function deleteVaccination(payload){if(!window.confirm(language==='en'?'Delete this vaccination record? This action cannot be undone.':'Διαγραφή αυτής της εγγραφής εμβολιασμού; Η ενέργεια δεν αναιρείται.'))return;try{await deleteVaccinationAsync(tenant.id,payload.draft.id);notify(language==='en'?'Vaccination deleted.':'Ο εμβολιασμός διαγράφηκε.','success');setVaccinationEditor(null);await reloadVaccinations()}catch(error){notifyError(error,'delete',{operation:'staff_vaccinations_delete'})}}
 ...
 return <Page fill className="prevention-registry-page" title={t('preventionCenter')} subtitle={t('preventionSubtitle')} actions={pageActions.length?<RecordActions actions={pageActions} actionCapabilities={actionCapabilities} actionLabels={{[UI_ACTIONS.CREATE]:createLabel}} onAction={pageAction}/>:null}>
  ...
  {vaccinationEditor&&<StaffVaccinationEditor language={language} employees={employeeRows.filter(x=>x.employmentStatus==='active'&&canAccessRecord(x))} value={vaccinationEditor} onChange={setVaccinationEditor} onClose={()=>setVaccinationEditor(null)} onSave={saveVaccination} onDelete={deleteVaccination}/>} 
 </Page>
}

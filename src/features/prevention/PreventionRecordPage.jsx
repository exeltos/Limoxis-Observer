...
const SHIFT_LABELS={morning:['Πρωινή','Morning'],afternoon:['Απογευματινή','Afternoon'],evening:['Απογευματινή','Evening'],night:['Νυχτερινή','Night']}
function shiftLabel(value,en){const pair=SHIFT_LABELS[value];return pair?(en?pair[1]:pair[0]):(value||'—')}
function fmtDay(value){const [y,m,d]=String(value||'').slice(0,10).split('-');return y&&m&&d?`${Number(d)}/${Number(m)}/${y}`:(value||'—')}

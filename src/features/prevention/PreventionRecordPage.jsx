...
export function PreventionRecordPage(){
 ...
 const recordTitle=recordType==='handHygiene'?`WHO Observation · ${fmtDate(record.date)}`:isWaste?`${en?'Waste measurement':'Μέτρηση αποβλήτων'} · ${fmtDate(record.date)}`:isAntiseptic?`${en?'Antiseptic consumption':'Κατανάλωση αντισηπτικού'} · ${record.period||''}`:recordType==='bundles'?`${record.templateName||record.bundle} · ${record.date||record.period||''}`:record.id
 ...
 const recordStatus=isWaste?<span className={`waste-category-badge ${wasteCategoryTone(wasteCategory)}`}>{wasteCategory}</span>:isAntiseptic?<span className={`antiseptic-abhr-badge ${record.indicatorEligible!==false&&isAbhrProduct(record.product)?'active':'informative'}`}>{record.indicatorEligible!==false&&isAbhrProduct(record.product)?(en?'ABHR · included in indicator':'ABHR · στον δείκτη'):(en?'Outside ABHR indicator':'Εκτός δείκτη ABHR')}</span>:null
 return <Page fill><EntityRecordShell className="prevention-record-shell workspace-fill" avatar={<Icon size={19}/>} eyebrow={(labels[recordType]?.[en?1:0])||(en?'Prevention':'Πρόληψη')} title={recordTitle} subtitle={recordSubtitle} status={recordStatus} recordNavigation={recordNavigation} headerActions={<>{canDelete&&<button type="button" className="entity-record-icon-button danger" title={en?'Void record':'Ακύρωση εγγραφής'} aria-label={en?'Void record':'Ακύρωση εγγραφής'} onClick={()=>setVoidOpen(true)}><Trash2 size={15}/></button>}<PrintExportActions onExport={()=>downloadRecordJson(record,{filename:record.id})}/></>} tabs={[]} activeTab="" onTabChange={()=>{}}>
   <div className="record-section">
    {recordType==='handHygiene'?<HandHygieneDetails record={record} fmtDate={fmtDate} language={language}/>:recordType==='waste'?<WasteDetails record={record} fmtDate={fmtDate} language={language} locale={locale}/>:recordType==='antiseptics'?<AntisepticDetails record={record} language={language} locale={locale}/>:<BundleDetails record={record} t={t} language={language}/>}
   </div>
  <GovernedReasonDialog open={voidOpen} title={en?'Void record':'Ακύρωση εγγραφής'} description={en?'The record will not be physically deleted. It will be retained for traceability and removed from the active list.':'Η εγγραφή δεν θα διαγραφεί φυσικά. Θα διατηρηθεί για ιχνηλασιμότητα και θα αφαιρεθεί από την ενεργή λίστα.'} confirmLabel={en?'Void record':'Ακύρωση εγγραφής'} danger onCancel={()=>setVoidOpen(false)} onConfirm={voidRecord}/>
...
function HandHygieneDetails({record,fmtDate,language}){
 ...
 return <>
  <div className="detail-grid quality-detail-grid">
   <D l={en?'Date':'Ημερομηνία'} v={fmtDate(record.date)}/><D l={en?'Department':'Τμήμα'} v={record.departmentEl}/><D l={en?'Observer':'Παρατηρητής'} v={record.observer}/><D l={en?'Time':'Ώρα'} v={`${record.session?.startTime||'—'} – ${record.session?.endTime||'—'}`}/>
  </div>
...
function WasteDetails({record,fmtDate,language,locale}){
 ...
 return <div className="waste-record-view">
  <section className="waste-record-primary">
   <div className="waste-record-section-heading"><div><span>{en?'MEASUREMENT':'ΜΕΤΡΗΣΗ'}</span><strong>{en?'Record details':'Στοιχεία καταγραφής'}</strong></div><span className={`waste-category-badge ${wasteCategoryTone(category)}`}>{category}</span></div>
   <div className="waste-record-measurements">
    <div><span>{en?'Weight':'Βάρος'}</span><strong>{Number(record.weight).toLocaleString(locale)} <small>kg</small></strong></div>
    <div><span>{en?'Containers':'Περιέκτες'}</span><strong>{record.containers}</strong></div>
    <div><span>{en?'Patient-days':'Νοσηλευτικές ημέρες'}</span><strong>{record.patientDays||'—'}</strong></div>
    <div className="waste-record-indicator"><span>{en?'Indicator':'Δείκτης'}</span><strong>{record.indicator!=null?Number(record.indicator).toLocaleString(locale):'—'}</strong><small>{en?'kg / 1,000 patient-days':'kg / 1.000 νοσηλευτικές ημέρες'}</small></div>
   </div>
   <div className="waste-record-meta-line"><span><b>{en?'Date':'Ημερομηνία'}</b>{fmtDate(record.date)}</span><span><b>{en?'Department':'Τμήμα'}</b>{record.departmentEl}</span><span><b>{en?'Responsible':'Υπεύθυνος'}</b>{record.responsible||record.createdBy||'—'}</span></div>
  </section>

  <section className="waste-record-trace">
   <div className="waste-record-section-heading"><div><span>{en?'TRACEABILITY':'ΙΧΝΗΛΑΣΙΜΟΤΗΤΑ'}</span><strong>{en?'Document & collection':'Παραστατικό & συλλογή'}</strong></div></div>
   <div className="waste-trace-grid">
    <div><span>{en?'Document number':'Αριθμός παραστατικού'}</span><strong>{record.documentNumber||'—'}</strong></div>
    <div><span>{en?'Collection company':'Εταιρεία συλλογής'}</span><strong>{record.collectionCompany||'—'}</strong></div>
    {record.patientDaysSource==='library'&&<div><span>{en?'Patient-days source':'Πηγή νοσηλευτικών ημερών'}</span><strong>{en?'Library':'Βιβλιοθήκη'}</strong></div>}
   </div>
  </section>
  {record.notes&&<div className="record-note-card"><span>{en?'NOTES':'ΣΗΜΕΙΩΣΕΙΣ'}</span><p>{record.notes}</p></div>}
 </div>
}
function AntisepticDetails({record,language,locale}){
 ...
 return <div className="antiseptic-record-view">
  <section className="antiseptic-record-primary">
   <div className="antiseptic-record-heading"><div><span>{en?'CONSUMPTION':'ΚΑΤΑΝΑΛΩΣΗ'}</span><strong>{en?'Period details':'Στοιχεία περιόδου'}</strong></div><span className={`antiseptic-abhr-badge ${eligible?'active':'informative'}`}>{eligible?(en?'ABHR · Indicator active':'ABHR · Δείκτης ενεργός'):(en?'Outside ABHR indicator':'Εκτός δείκτη ABHR')}</span></div>
   <div className="antiseptic-record-measurements">
    ...
    <div><span>{en?'Patient-days':'Νοσηλευτικές ημέρες'}</span><strong>{record.patientDays||'—'}</strong>{record.patientDaysSource==='library'&&<small>{en?'from Library':'από Βιβλιοθήκη'}</small>}</div>
    <div className="antiseptic-record-indicator"><span>{en?'ABHR indicator':'Δείκτης ABHR'}</span><strong>{record.indicator!=null?Number(record.indicator).toLocaleString(locale):'—'}</strong><small>{en?'L / 1,000 patient-days':'L / 1.000 νοσηλευτικές ημέρες'}</small></div>
   </div>
   <div className="antiseptic-record-meta-line"><span><b>{en?'Period':'Περίοδος'}</b>{record.period||'—'}</span><span><b>{en?'Department':'Τμήμα'}</b>{record.departmentEl||'—'}</span><span><b>{en?'Product':'Προϊόν'}</b>{record.product||'—'}</span></div>
  </section>
  <section className="antiseptic-record-trace">
   <div className="antiseptic-record-heading"><div><span>{en?'DOCUMENTATION':'ΤΕΚΜΗΡΙΩΣΗ'}</span><strong>{en?'Data source & traceability':'Πηγή δεδομένων & ιχνηλασιμότητα'}</strong></div></div>
   <div className="antiseptic-trace-grid">
    <div><span>{en?'Source / method':'Πηγή / μέθοδος'}</span><strong>{antisepticMethodLabel(record.method)}</strong></div>
    <div><span>{en?'Reference / document':'Αναφορά / παραστατικό'}</span><strong>{record.referenceNumber||'—'}</strong></div>
    <div><span>Υπεύθυνος</span><strong>{record.responsible||record.createdBy||'—'}</strong></div>
   </div>
   <div className="antiseptic-governance-note"><strong>{eligible?(en?'Prevention indicator':'Δείκτης πρόληψης'):(en?'Consumption monitoring':'Παρακολούθηση κατανάλωσης')}</strong><span>{eligible?'Η τιμή μπορεί να χρησιμοποιηθεί σε συγκρίσεις ανά περίοδο/τμήμα και σε συσχέτιση με τη συμμόρφωση Υγιεινής Χεριών.':'Η τιμή παραμένει διαθέσιμη για λειτουργική παρακολούθηση, χωρίς να αναμιγνύεται με τον δείκτη ABHR.'}</span></div>
  </section>
  {record.notes&&<div className="record-note-card"><span>{en?'NOTES':'ΣΗΜΕΙΩΣΕΙΣ'}</span><p>{record.notes}</p></div>}
 </div>
}
function BundleDetails({record,language}){
 ...
 return <div className="bundle-record-view">
  <section className="bundle-record-summary">
   ...
   <div className="bundle-record-kpis"><div><span>Score</span><strong>{record.score}%</strong></div><div><span>{en?'Applicable':'Εφαρμόσιμα'}</span><strong>{record.applicableCount??'—'}</strong></div><div><span>{en?'Deviations':'Αποκλίσεις'}</span><strong>{record.failedCount??findings.length}</strong></div><div><span>Ημερομηνία</span><strong>{record.date||record.period||'—'}</strong></div></div>
   <div className="bundle-record-meta"><span><b>{en?'Department':'Τμήμα'}</b>{record.departmentEl||'—'}</span><span><b>{en?'Shift':'Βάρδια'}</b>{record.shift||'—'}</span><span><b>{en?'Patient':'Ασθενής'}</b>{record.patientRef||'—'}</span><span><b>{en?'Device':'Συσκευή'}</b>{record.deviceRef||'—'}</span><span><b>{en?'Responsible':'Υπεύθυνος'}</b>{record.owner||record.createdBy||'—'}</span></div>
  </section>
  <section className="bundle-record-elements">
   <div className="bundle-record-heading"><div><span>{en?'ELEMENTS':'ΣΤΟΙΧΕΙΑ'}</span><strong>{en?'Bundle results':'Αποτελέσματα Bundle'}</strong></div></div>
   <div className="bundle-detail-list">{template.elements.map(([id,label],i)=>{const value=record.answers?.[id];return <div className={`bundle-detail-row ${value==='no'?'failed':value==='yes'?'passed':'na'}`} key={id}><span className="bundle-detail-index">{i+1}</span><strong>{label}</strong><span className="bundle-detail-answer">{value==='yes'?(en?'Yes':'Ναι'):value==='no'?(en?'No':'Όχι'):value==='na'?(en?'N/A':'Μ/Ε'):'—'}</span>{value==='no'&&record.answerNotes?.[id]&&<small>{record.answerNotes[id]}</small>}</div>})}</div>
  </section>
  {findings.length>0&&<section className="bundle-record-findings"><div className="bundle-record-heading"><div><span>{en?'DEVIATIONS':'ΑΠΟΚΛΙΣΕΙΣ'}</span><strong>{en?'Items for follow-up':'Σημεία για follow-up'}</strong></div></div>{findings.map(x=><div className="bundle-record-finding" key={x.id}><strong>{x.label}</strong><span>{x.note||(en?'Investigation / corrective action required.':'Απαιτείται διερεύνηση / διορθωτική ενέργεια.')}</span></div>)}</section>}
  {record.generalNotes&&<div className="record-note-card"><span>{en?'NOTES':'ΣΗΜΕΙΩΣΕΙΣ'}</span><p>{record.generalNotes}</p></div>}
 </div>
}

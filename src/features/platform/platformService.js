import { mergeDomainMetrics } from '../analysis/analysisDomainMerge'
...
async function loadMicrobiologyAnalytics({organizationId='',from='',to='',departmentId=''}){
  ...
  const bySite=sortedEntries(countBy(enriched,row=>row.sampleType),12)
  const grouped=new Map()
  for(const row of enriched){const key=[row.organism?.trim()||'—',row.resistance_class||'—',row.department,row.source,row.sampleType].join('|||');const current=grouped.get(key)||{organism:row.organism?.trim()||'—',resistanceClass:row.resistance_class||'—',department:row.department,source:row.source,sampleType:row.sampleType,count:0,lastDate:''};current.count+=1;if(row.eventDate>current.lastDate)current.lastDate=row.eventDate;grouped.set(key,current)}
  const nationalRows=[...grouped.values()].sort((a,b)=>b.count-a.count||b.lastDate.localeCompare(a.lastDate)).slice(0,80).map(row=>[row.organism,row.resistanceClass,row.department,row.source,row.count,row.lastDate,row.sampleType])
  ...
  return {microorganisms,resistance,monthly,byOrganization,byDepartment,bySource,bySite,nationalRows,totalPositive:enriched.length,totalCritical:enriched.filter(row=>row.is_critical).length,departmentCount:new Set(enriched.map(row=>row.department).filter(name=>name&&name!=='—')).size,departments:departments.map(row=>({id:row.id,organizationId:row.organization_id,name:row.name}))}
}

async function loadAmrSusceptibility({organizationId='',from='',to='',departmentId=''}){
  const {data,error}=await supabase.rpc('analysis_amr_susceptibility',{p_organization_id:organizationId||null,p_from:from||null,p_to:to||null,p_department_id:departmentId||null})
  if(error)throw error
  return (data||[]).map(row=>[row.organism_group,Number(row.tested)||0,Number(row.resistant)||0])
}

function mergeAmrRows(snapshots){
  const merged={}
  for(const snapshot of snapshots){for(const [organism,tested,resistant] of snapshot?.amrSusceptibility||[]){const current=merged[organism]||{tested:0,resistant:0};current.tested+=Number(tested)||0;current.resistant+=Number(resistant)||0;merged[organism]=current}}
...
function mergeAnalysisSnapshots(snapshots){
  ...
  return {source:'production',summary:sumSummary(snapshots),domains:mergeDomainMetrics(snapshots.map(item=>item?.domains)),microbiology:{microorganisms:mergeEntryRows(snapshots,'microorganisms'),resistance:mergeEntryRows(snapshots,'resistance',5),monthly:mergeMonthly(snapshots),byOrganization:byOrganization.sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)),byDepartment:mergeEntryRows(snapshots,'byDepartment'),bySource:mergeEntryRows(snapshots,'bySource'),bySite:mergeEntryRows(snapshots,'bySite'),nationalRows:mergeNationalRows(snapshots),totalPositive,totalCritical,departmentCount:new Set(snapshots.flatMap(item=>(item?.microbiology?.byDepartment||[]).map(([name])=>name))).size,departments},amrSusceptibility:mergeAmrRows(snapshots),clusters:snapshots.flatMap(item=>item?.clusters||[]).sort((a,b)=>b.count-a.count||String(b.lastDate).localeCompare(String(a.lastDate)))}
}

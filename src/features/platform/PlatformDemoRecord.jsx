...
export function PlatformDemoRecord({demo,language='el',onBack,onOpenDemo,onChanged}){
  ...
  async function saveEdit(){
    if(!draft?.label?.trim()||!draft?.validFrom||!draft?.validUntil||saving)return
    setSaving(true)
    try{const next=await updatePlatformDemoEntitlement(record.id,draft);setRecord(next);setEditOpen(false);onChanged?.(next);notify(tx('Η καρτέλα Demo ενημερώθηκε.','Demo record updated.'),'success',{operation:'platform_demo_update'})}
    catch(error){notifyError(error,'save',{operation:'platform_demo_update'})}finally{setSaving(false)}
  }
  async function togglePause(){
    if(working)return
    const next=record.status==='paused'?'active':'paused'
    const ok=await confirm({title:next==='paused'?tx('Παύση Demo','Pause Demo'):tx('Ενεργοποίηση Demo','Reactivate Demo'),message:next==='paused'?tx('Η πρόσβαση του Demo χρήστη θα απενεργοποιηθεί μέχρι να την ενεργοποιήσεις ξανά.','The Demo user will lose Demo access until you reactivate it.'):tx('Να ενεργοποιηθεί ξανά η πρόσβαση Demo;','Reactivate Demo access?'),confirmLabel:next==='paused'?tx('Παύση','Pause'):tx('Ενεργοποίηση','Reactivate')})
    if(!ok)return
    setWorking(true)
    try{const updated=await setPlatformDemoStatus(record.id,next);setRecord(updated);onChanged?.(updated);notify(next==='paused'?tx('Το Demo τέθηκε σε παύση.','Demo paused.'):tx('Το Demo ενεργοποιήθηκε.','Demo reactivated.'),'success',{operation:'platform_demo_status'})}
    catch(error){notifyError(error,'action',{operation:'platform_demo_status'})}finally{setWorking(false)}
  }
    ...
    <section className="platform-center-section platform-owner-record-workspace platform-demo-record-workspace">
      <div className="platform-owner-details">
        ...
        <div className="platform-demo-record-status">
          ...
          <div><strong>{statusLabel}</strong><span>{active?`${remaining} ${tx('ημέρες υπόλοιπο','days remaining')}`:status==='paused'?tx('Η πρόσβαση έχει τεθεί σε παύση.','Access is paused.'):tx('Η πρόσβαση δεν είναι ενεργή.','Access is not active.')}</span></div>
        </div>

        <div className="platform-info-sections platform-demo-info-sections">
          <InfoSection title={tx('Demo οργανισμός','Demo organization')}>
            <InfoRow label={tx('Επωνυμία','Name')} value={org?.name||record.label}/>
            <InfoRow label={tx('Κωδικός','Code')} value={org?.code||'—'}/>
            <InfoRow label={tx('Τύπος','Type')} value={tx('Demo / Prospect','Demo / Prospect')}/>
            <InfoRow label={tx('Κατάσταση','Status')} value={statusLabel} status={active?'active':'danger'}/>
          </InfoSection>
          <InfoSection title={tx('Επικοινωνία','Contact')}>
            <InfoRow label={tx('Υπεύθυνος','Contact person')} value={record.contact_name||'—'}/>
            <InfoRow label="Email" value={record.contact_email||'—'}/>
            <InfoRow label={tx('Λογαριασμός Demo','Demo account')} value={record.demo_user_id?tx('Συνδεδεμένος','Linked'):tx('Δεν έχει συνδεθεί','Not linked')}/>
          </InfoSection>
          <InfoSection title={tx('Διάρκεια πρόσβασης','Access period')}>
            <InfoRow label={tx('Έναρξη','Start')} value={record.valid_from}/>
            <InfoRow label={tx('Λήξη','End')} value={record.valid_until}/>

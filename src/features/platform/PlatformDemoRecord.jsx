...
import { purgePlatformOrganization,resetPlatformDemoPassword,setPlatformDemoStatus,updatePlatformDemoEntitlement } from '../../core/tenant/tenantService'
...
export function PlatformDemoRecord({demo,language='el',onBack,onOpenDemo,onChanged,onDeleted}){
  ...
  async function resetPassword(){if(working)return;const target=record.contact_email||tx('το καταχωρημένο email','the registered email address');const ok=await confirm({title:tx('Επαναφορά κωδικού Demo','Reset Demo password'),message:tx(`Θα αποσταλεί email ασφαλούς επαναφοράς κωδικού στο ${target}. Ο κωδικός δεν αλλάζει μέχρι ο Demo χρήστης να ολοκληρώσει τη διαδικασία. Θέλεις να συνεχίσεις;`,`A secure password-reset email will be sent to ${target}. The password will not change until the Demo user completes the reset flow. Do you want to continue?`),confirmLabel:tx('Αποστολή email','Send reset email')});if(!ok)return;setWorking(true);try{await resetPlatformDemoPassword(record);notify(tx('Στάλθηκε email επαναφοράς κωδικού στον Demo χρήστη.','Password reset email sent to the Demo user.'),'success',{operation:'platform_demo_reset_password'})}catch(error){notifyError(error,'action',{operation:'platform_demo_reset_password'})}finally{setWorking(false)}}
  function requestDelete(){setDeleteConfirm('');setDeletePassword('');setDeleteOpen(true)}
  async function confirmDelete(){
    if(working||!record.organization_id||!deleteCode)return
    if(deleteConfirm.trim().toUpperCase()!==deleteCode.toUpperCase())return
    if(!deletePassword)return
    setWorking(true)
    try{
      await purgePlatformOrganization({organizationId:record.organization_id,password:deletePassword,confirmation:deleteConfirm.trim()})
      setDeleteOpen(false)
      notify(tx('Το Demo, ο απομονωμένος οργανισμός και τα σχετικά δεδομένα διαγράφηκαν οριστικά.','The Demo, isolated organization, and related data were permanently deleted.'),'success',{operation:'platform_demo_delete'})
      if(onDeleted)onDeleted(record.id);else onBack?.()
  ...
  return <>
    <EntityRecordShell className="platform-owner-record-shell platform-demo-record-workspace" avatar={<FlaskConical size={20}/>} eyebrow={tx('ΚΑΡΤΕΛΑ DEMO','DEMO RECORD')} title={org?.name||record.label} subtitle={tx('Χρονικά περιορισμένο και πλήρως απομονωμένο περιβάλλον επίδειξης.','Time-limited, fully isolated demonstration environment.')} status={<span className={`status-badge ${active?'active':status==='paused'?'temporary':'danger'}`}>{statusLabel}</span>} headerActions={actions} onBack={onBack} backLabel={tx('Πίσω','Back')}>
      <div className="platform-owner-details"><div className="platform-demo-record-status"><span className="platform-demo-icon"><FlaskConical size={20}/></span><div><strong>{statusLabel}</strong><span>{active?`${remaining} ${tx('ημέρες υπόλοιπο','days remaining')}`:status==='paused'?tx('Η πρόσβαση έχει τεθεί σε παύση.','Access is paused.'):tx('Η πρόσβαση δεν είναι ενεργή.','Access is not active.')}</span></div></div><div className="platform-info-sections platform-demo-info-sections"><InfoSection title={tx('Demo οργανισμός','Demo organization')}><InfoRow label={tx('Επωνυμία','Name')} value={org?.name||record.label}/><InfoRow label={tx('Κωδικός','Code')} value={org?.code||'—'}/><InfoRow label={tx('Τύπος','Type')} value="Demo / Prospect"/><InfoRow label={tx('Κατάσταση','Status')} value={statusLabel} status={active?'active':'danger'}/></InfoSection><InfoSection title={tx('Επικοινωνία','Contact')}><InfoRow label={tx('Υπεύθυνος','Contact person')} value={record.contact_name||'—'}/><InfoRow label="Email" value={record.contact_email||'—'}/><InfoRow label={tx('Λογαριασμός Demo','Demo account')} value={record.demo_user_id?tx('Συνδεδεμένος','Linked'):tx('Δεν έχει συνδεθεί','Not linked')}/></InfoSection><InfoSection title={tx('Διάρκεια πρόσβασης','Access period')}><InfoRow label={tx('Έναρξη','Start')} value={record.valid_from}/><InfoRow label={tx('Λήξη','End')} value={record.valid_until}/><InfoRow label={tx('Υπόλοιπο','Remaining')} value={`${remaining} ${tx('ημέρες','days')}`}/><InfoRow label={tx('Απομόνωση δεδομένων','Data isolation')} value={tx('Μόνο Demo δεδομένα','Demo data only')}/></InfoSection></div></div>
    </EntityRecordShell>

...
import { Building2,FlaskConical,KeyRound,LogIn,PauseCircle,PlayCircle,Save,Trash2 } from 'lucide-react'
...
export function PlatformDemoRecord({demo,language='el',onBack,onOpenDemo,onChanged,onDeleted}){
  ...
  async function saveEdit(){if(!canSave||saving)return;setSaving(true);try{const next=await savePlatformDemoRecord(record,draft);setRecord(next);setDraft(toDraft(next));onChanged?.(next);notify(tx('Η καρτέλα Demo ενημερώθηκε.','Demo record updated.'),'success',{operation:'platform_demo_update'})}catch(error){notifyError(error,'save',{operation:'platform_demo_update'})}finally{setSaving(false)}}
  ...
  const actions=<div className="platform-org-actions" aria-label={tx('Ενέργειες Demo','Demo actions')}>
    ...
    <Action icon={<Save size={17}/>} tone="edit" label={saving?tx('Αποθήκευση…','Saving…'):tx('Αποθήκευση','Save')} title={tx('Αποθήκευση αλλαγών','Save changes')} disabled={!canSave||saving||working} onClick={saveEdit}/>

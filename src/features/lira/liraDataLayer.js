...
export async function loadLiraData({isDemo=false,organizationId=null}={}){
  ...
  const [departmentsRes,patientsRes,casesRes,reassessmentsRes,isolationRes,samplesRes,resultsRes,handRes,bundleRes,incidentRes,capaRes,patientDaysRes,haiRes,devicesRes]=await Promise.all([
    q('departments','id,name,code'),q('patients','id,patient_code,first_name,last_name,department_id,status'),q('surveillance_cases','id,patient_id,department_id,status,started_at,closed_at'),q('surveillance_reassessments','id,surveillance_case_id,reassessed_at,next_review_due_at'),q('isolation_episodes','id,surveillance_case_id,review_due_at,status'),q('laboratory_samples','id,patient_id,surveillance_case_id,department_id,sample_code,sample_type,status,collected_at'),q('microbiology_results','id,sample_id,result_status,organism,resistance_class,is_critical,critical_communicated_at,resulted_at,validation_status'),q('hand_hygiene_sessions','id,department_id,observation_date,observations,compliant_observations,status'),q('prevention_bundle_assessments','id,department_id,bundle_key,assessment_date,score,criteria,status'),q('quality_incidents','id,code,title,department_id,occurred_at,severity,status'),q('quality_capa_actions','id,code,title,department_id,due_date,priority,status'),q('patient_days','id,department_id,census_date,patient_days,source,review_status'),q('hai_classifications','id,surveillance_case_id,patient_id,case_status,hai_type,definition_set,definition_version,criteria_met,classified_at'),q('surveillance_devices','id,surveillance_case_id,patient_id,department_id,device_type,inserted_at,removed_at,status'),
  ])
  const responses=[departmentsRes,patientsRes,casesRes,reassessmentsRes,isolationRes,samplesRes,resultsRes,handRes,bundleRes,incidentRes,capaRes,patientDaysRes,haiRes,devicesRes]
  ...
  return {surveillance,laboratory,handHygiene,bundles,qualityIncidents:qualityIncidentsRows,qualityCapas:qualityCapaRows,patientDays:patientDayRows,haiClassifications,devices,generatedAt:new Date().toISOString(),source:'production'}
}

...
export function filterLiraData(data,{department='all',periodDays=30,language='el',today=new Date().toISOString().slice(0,10)}={}){
  if(!data)return data
  ...
  return {...data,
    surveillance:filterRows(data.surveillance),
    laboratory:filterRows(data.laboratory),
    handHygiene:filterRows(data.handHygiene),
    bundles:filterRows(data.bundles),
    qualityIncidents:filterRows(data.qualityIncidents),
    qualityCapas:filterRows(data.qualityCapas),
  }
}

export function liraDepartments(data,language='el'){
  const values=new Set()
  for(const key of ['surveillance','laboratory','handHygiene','bundles','qualityIncidents','qualityCapas']){
    for(const row of data?.[key]||[]){const value=departmentOf(row,language);if(value&&value!=='—')values.add(value)}
  }
...
export function buildLiraAnalysis(data,language='el',{today=new Date().toISOString().slice(0,10)}={}){
 ...
 return {signals,highPriority:signals.filter(x=>['critical','high'].includes(x.severity)).length,activeSurveillance:active.length,amr:new Set([...amrSurv.map(x=>x.id),...amrLab.map(x=>x.resultId||x.id)]).size,bundleAllOrNone:bundleRate,
  domains:[{label:en?'Surveillance':'Επιτήρηση',value:en?`${active.length} active`:`${active.length} ενεργές`,note:`${amrSurv.length} ${en?'with':'με'} MDR/XDR/PDR`},{label:en?'Laboratory':'Εργαστήριο',value:en?`${laboratory.filter(x=>x.result==='positive').length} positive`:`${laboratory.filter(x=>x.result==='positive').length} θετικά`,note:en?`${criticalLab.length} critical without documented communication`:`${criticalLab.length} κρίσιμα χωρίς τεκμηριωμένη επικοινωνία`},{label:en?'Hand hygiene':'Υγιεινή χεριών',value:`${hhAverage}%`,note:en?'Average compliance in selected scope':'Μέση συμμόρφωση στο επιλεγμένο εύρος'},{label:'Bundles',value:`${bundleRate}%`,note:en?'All-or-none in selected scope':'All-or-none στο επιλεγμένο εύρος'},{label:en?'Quality':'Ποιότητα',value:en?`${highIncidents.length} serious open`:`${highIncidents.length} σοβαρά ανοικτά`,note:en?`${qualityCapas.filter(x=>x.status!=='completed').length} active CAPA`:`${qualityCapas.filter(x=>x.status!=='completed').length} ενεργές CAPA`}],
  actions:[{title:en?'Review resistant-organism signals':'Επανεκτίμηση σημάτων ανθεκτικών μικροοργανισμών',text:en?'Review temporal and spatial correlation, isolation and microbiology data before considering a possible outbreak.':'Ελέγξτε χρονική και χωρική συσχέτιση, απομόνωση και μικροβιολογικά δεδομένα πριν θεωρηθεί πιθανή έξαρση.',to:'/surveillance'},{title:en?'Close critical communications':'Κλείσιμο κρίσιμων επικοινωνιών',text:en?'Critical laboratory results require documented closed-loop communication.':'Τα κρίσιμα εργαστηριακά αποτελέσματα χρειάζονται τεκμηριωμένη επικοινωνία κλειστού κύκλου.',to:'/laboratory'},{title:en?'Targeted hand-hygiene observation':'Στοχευμένη παρατήρηση υγιεινής χεριών',text:en?'Low-compliance sessions require additional observations before drawing a reliable conclusion.':'Οι χαμηλές συνεδρίες συμμόρφωσης χρειάζονται περισσότερες παρατηρήσεις πριν εξαχθεί ασφαλές συμπέρασμα.',to:'/prevention'}]}
}

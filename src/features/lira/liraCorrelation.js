...
export function analyzeHaiContext(data,type,{department='all',window=null,today=new Date().toISOString().slice(0,10),language='el'}={}){
 ...
 if(bundle!=null)points.push(en?`Bundle all-or-none compliance in the same scope: ${bundle}%.`:`Πλήρης συμμόρφωση σε δέσμες μέτρων στο ίδιο πλαίσιο: ${bundle}%.`)
...
export function compareHaiContext(data,type,current,reference,{department='all',today=new Date().toISOString().slice(0,10),language='el'}={}){
 ...
 if(a.metrics.bundleCompliance!=null||b.metrics.bundleCompliance!=null)points.push(`${en?'Bundle all-or-none':'Πλήρης συμμόρφωση σε δέσμες μέτρων'}: ${current.label} ${a.metrics.bundleCompliance??'—'}% vs ${reference.label} ${b.metrics.bundleCompliance??'—'}%.`)

...
function operationalMetrics(data,{department='all',window=null,today}){
 ...
 return {
  active:surveillance.filter(row=>row.state==='active').length,
  overdueReviews:surveillance.filter(row=>row.reviewDue&&overdue(row.reviewDue,today)).length,
  criticalUncommunicated:laboratory.filter(row=>row.critical&&!(row.communications||[]).length).length,
  amr:laboratory.filter(row=>Boolean(row.resistance)).length,
  handHygiene:weightedHand(handRows),bundleCompliance:bundleRate(bundleRows),
  seriousIncidents:incidents.filter(row=>openStatus(row.status)&&['high','critical','major','severe'].includes(normalize(row.severity))).length,
  overdueCapas:capas.filter(row=>openStatus(row.status)&&overdue(row.dueDate,today)).length,
 }
}
...
export function compareOperationalOverview(data,spec,{department='all',today=new Date().toISOString().slice(0,10),language='el'}={}){
 ...
 const definitions=[
  ['criticalUncommunicated',en?'critical results without documented communication':'κρίσιμα αποτελέσματα χωρίς τεκμηριωμένη γνωστοποίηση','higher'],
  ['overdueReviews',en?'overdue surveillance reviews':'εκπρόθεσμες επανεκτιμήσεις επιτήρησης','higher'],
  ['seriousIncidents',en?'serious open quality incidents':'σοβαρά ανοικτά συμβάντα ποιότητας','higher'],
  ['overdueCapas',en?'overdue CAPA':'εκπρόθεσμες CAPA','higher'],
  ['amr','AMR / MDR-XDR','higher'],
  ['handHygiene',en?'hand-hygiene compliance':'συμμόρφωση υγιεινής χεριών','lower'],
  ['bundleCompliance',en?'bundle all-or-none compliance':'bundle all-or-none συμμόρφωση','lower'],
 ]
 const changes=[]
 ...
 const points=changes.map(change=>{const unit=['handHygiene','bundleCompliance'].includes(change.key)?'%':'';const direction=change.worsened?(en?'worsened':'επιδεινώθηκε'):(en?'improved':'βελτιώθηκε');return `${change.label}: ${fmt(change.reference)}${unit} → ${fmt(change.current)}${unit} · ${direction}.`})
 ...
 const worsened=changes.filter(x=>x.worsened).length;const improved=changes.filter(x=>!x.worsened).length
 points.push(en?`${worsened} domain signal(s) worsened and ${improved} improved or decreased. Interpret these as descriptive operational signals, not proof of causality.`:`${worsened} διατομεακά σήματα επιδεινώθηκαν και ${improved} βελτιώθηκαν ή μειώθηκαν. Η εικόνα είναι περιγραφική και δεν αποδεικνύει αιτιότητα.`)

...
import { ArrowLeft, BarChart3, Building2, Clock3, FlaskConical, PauseCircle, PlayCircle, Trash2, Download, Printer } from 'lucide-react'
...
export function PlatformCenterPage(){
  ...
  const [members,setMembers]=useState([]); const [demos,setDemos]=useState([]); const [loadingStats,setLoadingStats]=useState(true); const [selectedOrg,setSelectedOrg]=useState(null); const [analysisTab,setAnalysisTab]=useState('surveillance')
  ...
  if(activeKey==='demo')return <Page title="Demo" subtitle="Απομονωμένο περιβάλλον παρουσίασης. Τα demo δεδομένα υπάρχουν μόνο εδώ και δεν αναμειγνύονται με πραγματικούς οργανισμούς."><div className="platform-workspace-toolbar"><button className="platform-back-button" onClick={()=>nav('/platform')}><ArrowLeft size={16}/>Dashboard</button></div><section className="platform-center-section platform-demo-access"><div className="platform-demo-access-copy"><span className="platform-demo-icon"><FlaskConical size={22}/></span><div><h2>Demo περιβάλλον</h2><p>Άνοιξε το πλήρες Limoxis Observer με ασφαλή, αποκλειστικά demo δεδομένα. Η πραγματική βάση των οργανισμών παραμένει καθαρή.</p></div></div><Button onClick={()=>{enterPlatformDemo();nav('/')}}>Πρόσβαση Demo</Button></section></Page>

  const analysisTabs=[['surveillance','Επιτήρηση'],['laboratory','Εργαστήριο'],['prevention','Πρόληψη'],['hand-hygiene','Υγιεινή Χεριών'],['amr','AMR / Αντιμικροβιακά'],['controls','Έλεγχοι'],['quality','Ποιότητα'],['training','Εκπαίδευση'],['occupational','Υγεία Εργαζομένων'],['governance','Governance']]
  return <div className="platform-analysis-page"><div className="platform-analysis-head"><div><button className="platform-back-button" onClick={()=>nav('/platform')}><ArrowLeft size={16}/>Dashboard</button><h1>Ανάλυση</h1><p>Συγκεντρωτικά reports και συγκρίσεις με πλήρη platform φίλτρα.</p></div><div className="platform-analysis-actions"><Button variant="secondary"><Printer size={15}/> Εκτύπωση</Button><Button variant="secondary"><Download size={15}/> Export</Button></div></div><div className="platform-analysis-sticky"><div className="platform-analysis-filters"><label className="field"><span>Οργανισμός</span><select><option>Όλοι οι οργανισμοί</option>{organizations.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label><label className="field"><span>Περιφέρεια</span><select><option>Όλες οι περιφέρειες</option>{GREEK_REGIONS.map(r=><option key={r}>{r}</option>)}</select></label><label className="field"><span>ΥΠΕ</span><select><option>Όλες οι ΥΠΕ</option>{HEALTH_REGIONS.map(r=><option key={r}>{r}</option>)}</select></label><label className="field"><span>Περίοδος</span><select><option>Τρέχον έτος</option><option>Μήνας</option><option>Τρίμηνο</option><option>Εξάμηνο</option><option>Έτος</option><option>Σύγκριση ετών</option></select></label></div><div className="platform-analysis-tabs" role="tablist" aria-label="Κατηγορίες ανάλυσης">{analysisTabs.map(([id,label])=><button key={id} role="tab" aria-selected={analysisTab===id} className={analysisTab===id?'active':''} onClick={()=>setAnalysisTab(id)}>{label}</button>)}</div></div><div className="platform-analysis-scroll"><section className="platform-analysis-report"><div className="platform-section-heading"><div><h2>{analysisTabs.find(([id])=>id===analysisTab)?.[1]}</h2><p>Δείκτες, τάσεις, συγκρίσεις, compliance και αναλυτικά δεδομένα για τα ενεργά φίλτρα.</p></div></div><div className="kpi-grid platform-analysis-kpis"><article className="kpi-card"><span>Κύριος δείκτης</span><strong>—</strong><small>Αναμονή πραγματικών δεδομένων</small></article><article className="kpi-card"><span>Μεταβολή περιόδου</span><strong>—</strong><small>Σύγκριση με προηγούμενη περίοδο</small></article><article className="kpi-card"><span>Οργανισμοί στο φίλτρο</span><strong>{organizations.length}</strong><small>με βάση την τρέχουσα επιλογή</small></article><article className="kpi-card"><span>Πληρότητα δεδομένων</span><strong>—</strong><small>Data quality / completeness</small></article></div><div className="platform-analysis-chart-grid"><article className="platform-analysis-placeholder"><BarChart3 size={22}/><strong>Τάση περιόδου</strong><span>Το γράφημα θα τροφοδοτείται από τα πραγματικά δεδομένα της επιλεγμένης κατηγορίας.</span></article><article className="platform-analysis-placeholder"><BarChart3 size={22}/><strong>Σύγκριση οργανισμών</strong><span>Διαθέσιμο στον Platform Owner. Στον οργανισμό θα περιορίζεται αυτόματα στο δικό του scope.</span></article></div></section></div></div>
}

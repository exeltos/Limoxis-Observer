...
export function PlatformCenterPage(){
  ...
  async function refreshPlatformData(){
    ...
    try{const [m,d]=await Promise.all([listPlatformOrganizationMembers(),listPlatformDemos()]);setMembers(m);setDemos(d)}
  ...
  const openCreate=()=>{setDraft({...emptyOrganization,code:generateOrganizationCode()});setFormError('');setCreateOpen(true)}

  function openOrganization(org){
    setSelectedUser(null)
    nav(`/platform#organizations?organization=${org.id}&tab=details`,{state:{returnTo:`${location.pathname}${location.hash}`}})
  }
  ...
  function returnFromRecord(){const target=location.state?.returnTo;setSelectedUser(null);target?nav(target):nav('/platform#organizations')}
  ...
  function openDemoRecord(demo){nav(`/platform#demo?demo=${demo.id}`,{state:{returnTo:`${location.pathname}${location.hash}`}})}
  function returnFromDemoRecord(){const target=location.state?.returnTo;target?nav(target):nav('/platform#demo')}
  ...
  async function togglePause(org){
    ...
    const ok=await confirm({title:next==='suspended'?tx('Παύση οργανισμού','Suspend organization'):tx('Επανενεργοποίηση οργανισμού','Reactivate organization'),message:next==='suspended'?tx(`Να τεθεί σε παύση ο οργανισμός «${org.name}»;`,`Suspend “${org.name}”?`):tx(`Να ενεργοποιηθεί ξανά ο οργανισμός «${org.name}»;`,`Reactivate “${org.name}”?`),confirmLabel:next==='suspended'?tx('Παύση','Suspend'):tx('Ενεργοποίηση','Reactivate')})
    if(!ok)return
    try{await setPlatformOrganizationStatus(org.id,next);await reloadMemberships();await refreshPlatformData();notify(tx('Η κατάσταση του οργανισμού ενημερώθηκε.','Organization status updated.'),'success',{operation:'platform_organization_status'})}
  ...
  async function confirmRemoveOrganization(){
    ...
    if(deleteConfirm.trim().toUpperCase()!==deleteOrg.code?.toUpperCase()){notify(tx('Πληκτρολόγησε ακριβώς τον κωδικό του οργανισμού για επιβεβαίωση.','Type the organization code exactly to confirm.'),'warning',{operation:'platform_organization_delete'});return}
    if(!deletePassword){notify(tx('Απαιτείται ο κωδικός πρόσβασης του Platform Owner για επαναταυτοποίηση.','Platform Owner password is required for re-authentication.'),'warning',{operation:'platform_organization_delete'});return}
    setDeleting(true)
    try{await purgePlatformOrganization({organizationId:deleteOrg.id,password:deletePassword,confirmation:deleteConfirm.trim()});await reloadMemberships();await refreshPlatformData();setDeleteOrg(null);nav('/platform#organizations');notify(tx('Ο οργανισμός και τα σχετικά δεδομένα διαγράφηκαν οριστικά.','The organization and related data were permanently deleted.'),'success',{operation:'platform_organization_delete'})}
  ...
  async function beginEditOrg(org){
    ...
    try{const users=await listOrganizationMembersDetailed(org.id);const admin=users.find(user=>user.role==='hospital_admin')||null;setEditAdmin(admin);if(admin)setEditOrg(x=>x?({...x,adminFullName:admin.name||'',adminEmail:admin.email||''}):x)}catch(error){console.warn(error)}
  }
  ...
  async function saveOrgEdit({sendInitialInvitation=false}={}){
    ...
    try{
      ...
      await reloadMemberships();await refreshPlatformData();if(selectedOrg?.id===editOrg.id)await loadOrgUsers(selectedOrg);setEditOrg(null);setEditAdmin(null)
      notify(tx('Τα στοιχεία του οργανισμού ενημερώθηκαν.','Organization details updated.'),'success',{operation:'platform_organization_update'})
  ...
  async function resendHospitalAdminInvitation(){
    ...
    try{const result=await manageOrganizationUser({organizationId:editOrg.id,userId:editAdmin.userId,action:'resend_invitation'});notify(result?.emailSent?tx('Η πρόσκληση Hospital Admin επαναποστάλθηκε.','Hospital Admin invitation resent.'):tx('Η πρόσκληση ανανεώθηκε.','Invitation refreshed.'),'success',{operation:'platform_admin_invite'});const users=await listOrganizationMembersDetailed(editOrg.id);setEditAdmin(users.find(user=>user.role==='hospital_admin')||null);if(selectedOrg?.id===editOrg.id)await loadOrgUsers(selectedOrg)}
  ...
  const createDialog=createOpen?<ObserverDialog width="wide" eyebrow="Platform Owner" title={tx('Νέος οργανισμός','New organization')} subtitle={tx('Στοιχεία οργανισμού και πρόσκληση αρχικού Hospital Admin.','Organization identity and initial Hospital Admin invitation.')} onClose={()=>!saving&&setCreateOpen(false)} footer={<SaveButton loading={saving} savingLabel={tx('Αποθήκευση…','Saving…')} disabled={!formValid} onClick={submitOrganization}>{tx('Αποθήκευση & αποστολή πρόσκλησης','Save & send invitation')}</SaveButton>}>
    <div className="platform-form-shell">
      <FormSection title={tx('Ταυτότητα οργανισμού','Organization identity')} subtitle={tx('Βασικά στοιχεία αναγνώρισης και κατάστασης.','Core identity and status information.')}>
        <div className="platform-form-grid"><label className="field field-wide"><span>{tx('Επωνυμία','Name')} *</span><input autoFocus value={draft.name} onChange={e=>setField('name',e.target.value)}/></label><label className="field"><span>{tx('Κωδικός / Hospital Prefix','Code / Hospital Prefix')}</span><input value={draft.code} readOnly/><small className="field-hint">{tx('Δημιουργείται αυτόματα από την πλατφόρμα.','Generated automatically by the platform.')}</small></label><label className="field"><span>{tx('Τύπος','Type')}</span><select value={draft.type} onChange={e=>setField('type',e.target.value)}><option value="hospital">{tx('Νοσοκομείο','Hospital')}</option><option value="clinic">{tx('Κλινική','Clinic')}</option><option value="group">{tx('Όμιλος','Group')}</option><option value="other">{tx('Άλλο','Other')}</option></select></label><label className="field"><span>{tx('Κατάσταση','Status')}</span><select value={draft.status} onChange={e=>setField('status',e.target.value)}><option value="active">{tx('Ενεργός','Active')}</option><option value="suspended">{tx('Σε παύση','Suspended')}</option></select></label></div>
      </FormSection>
      <FormSection title={tx('Τοποθεσία & λειτουργία','Location & operations')}><div className="platform-form-grid"><label className="field"><span>{tx('Περιφέρεια','Region')} *</span><select value={draft.region} onChange={e=>setField('region',e.target.value)}><option value="">{tx('Επιλογή…','Select…')}</option>{GREEK_REGIONS.map(x=><option key={x}>{x}</option>)}</select></label><label className="field field-wide"><span>{tx('Υγειονομική Περιφέρεια (ΥΠΕ)','Health Region')} *</span><select value={draft.healthRegion} onChange={e=>setField('healthRegion',e.target.value)}><option value="">{tx('Επιλογή…','Select…')}</option>{HEALTH_REGIONS.map(x=><option key={x}>{x}</option>)}</select></label><label className="field"><span>{tx('Πόλη','City')} *</span><input value={draft.city} onChange={e=>setField('city',e.target.value)}/></label><label className="field"><span>{tx('Χώρα','Country')}</span><input value={draft.country} onChange={e=>setField('country',e.target.value)}/></label><label className="field"><span>{tx('Κεντρικό email','Main email')}</span><input type="email" value={draft.contactEmail} onChange={e=>setField('contactEmail',e.target.value)}/></label><label className="field"><span>{tx('Τηλέφωνο','Phone')}</span><input value={draft.contactPhone} onChange={e=>setField('contactPhone',e.target.value)}/></label><label className="field"><span>{tx('Δυναμικότητα κλινών','Bed capacity')}</span><input type="number" min="0" value={draft.bedCapacity} onChange={e=>setField('bedCapacity',e.target.value)}/></label></div></FormSection>
      <FormSection title={tx('Αρχικός Hospital Admin','Initial Hospital Admin')} subtitle={tx('Ο χρήστης θα λάβει ασφαλή πρόσκληση και θα ορίσει ο ίδιος κωδικό.','The user receives a secure invitation and sets their own password.')}><div className="platform-form-grid"><label className="field"><span>{tx('Ονοματεπώνυμο','Full name')} *</span><input value={draft.adminFullName} onChange={e=>setField('adminFullName',e.target.value)}/></label><label className="field"><span>{tx('Email πρόσκλησης','Invitation email')} *</span><input type="email" value={draft.adminEmail} onChange={e=>setField('adminEmail',e.target.value)}/><small className={draft.adminEmail&&!emailValid?'field-error':'field-hint'}>{tx('Χρησιμοποιείται μόνο για ασφαλή ενεργοποίηση.','Used only for secure activation.')}</small></label></div></FormSection>
  ...
  if(!activeKey)return <><Page title={tx('Dashboard Πλατφόρμας','Platform Dashboard')} subtitle={tx('Συνολική εικόνα του Limoxis Observer και των οργανισμών του.','Overview of Limoxis Observer and its organizations.')}><div className="kpi-grid platform-kpi-grid"><article className="kpi-card"><span>{tx('Σύνολο οργανισμών','Organizations')}</span><strong>{organizations.length}</strong><small>{activeOrganizations} {tx('ενεργοί','active')}</small></article><article className="kpi-card"><span>{tx('Σύνολο χρηστών','Users')}</span><strong>{loadingStats?'—':members.length}</strong><small>{tx('σε όλους τους οργανισμούς','across all organizations')}</small></article><article className="kpi-card"><span>{tx('Ενεργά Demo','Active demos')}</span><strong>{loadingStats?'—':activeDemos.length}</strong><small>{expiringDemos.length} {tx('λήγουν ≤14 ημέρες','expire within 14 days')}</small></article></div><section className="platform-center-section"><div className="platform-section-heading"><div><h2>{tx('Demo που βρίσκονται σε εξέλιξη','Active demos')}</h2><p>{tx('Παρακολούθηση διάρκειας και έγκαιρη ειδοποίηση πριν τη λήξη.','Track duration and upcoming expiry.')}</p></div></div>{activeDemos.length?<div className="platform-demo-list">{activeDemos.map(d=>{const p=demoProgress(d);return <button type="button" className="platform-demo-row platform-owner-clickable-row" key={d.id} onClick={()=>openDemoRecord(d)}><div><strong>{d.organization?.name||d.label}</strong><small>{d.contact_name||d.contact_email||'Demo access'} · {tx('έως','until')} {new Date(d.valid_until).toLocaleDateString(en?'en-GB':'el-GR')}</small></div><div className="platform-demo-progress"><div><span style={{width:`${p.pct}%`}}/></div><small className={p.remaining<=14?'warning':''}>{p.remaining} {tx('ημέρες υπόλοιπο','days remaining')}</small></div></button>})}</div>:<div className="empty-state platform-empty"><Clock3 size={22}/><strong>{tx('Δεν υπάρχουν ενεργά Demo','No active demos')}</strong></div>}</section></Page>{createDialog}</>

  if(activeKey==='organizations'){
    if(selectedOrg)return <><Page fill title={selectedOrg.name} subtitle={`${selectedOrg.code||'—'} · ${selectedOrg.city||'—'} · ${selectedOrg.region||'—'}`}><div className="platform-workspace-toolbar"><button className="platform-back-button" onClick={returnFromRecord}><ArrowLeft size={16}/>{tx('Πίσω','Back')}</button></div><section className="platform-center-section platform-owner-record-workspace"><div className="platform-org-tabs"><button className={orgDetailTab==='details'?'active':''} onClick={()=>changeOrgTab('details')}>{tx('Στοιχεία','Details')}</button><button className={orgDetailTab==='users'?'active':''} onClick={()=>changeOrgTab('users')}><Users size={14}/>{tx('Χρήστες','Users')} ({orgUsers.length})</button><button className={orgDetailTab==='diagnostics'?'active':''} onClick={()=>changeOrgTab('diagnostics')}><Activity size={14}/>{tx('Λειτουργία & συμβάντα','Activity & events')}</button><button className={orgDetailTab==='analysis'?'active':''} onClick={()=>changeOrgTab('analysis')}><BarChart3 size={14}/>{tx('Ανάλυση','Analytics')}</button></div>
      {orgDetailTab==='details'&&<div className="platform-owner-details"><div className="platform-record-intro"><div><span className="eyebrow">{tx('ΚΑΡΤΕΛΑ ΟΡΓΑΝΙΣΜΟΥ','ORGANIZATION RECORD')}</span><h2>{tx('Γενικά στοιχεία','General information')}</h2><p>{tx('Ταυτότητα, διοικητική υπαγωγή, στοιχεία επικοινωνίας και βασική λειτουργική εικόνα.','Identity, administrative scope, contact information and operational profile.')}</p></div><PlatformOrganizationActions organization={selectedOrg} language={language} onEnter={()=>enterOrganization(selectedOrg)} onEdit={()=>beginEditOrg(selectedOrg)} onTogglePause={()=>togglePause(selectedOrg)} onDelete={()=>requestRemoveOrganization(selectedOrg)}/></div><div className="platform-info-sections"><InfoSection title={tx('Ταυτότητα','Identity')}><InfoRow label={tx('Επωνυμία','Name')} value={selectedOrg.name}/><InfoRow label={tx('Κωδικός / Hospital Prefix','Code / Hospital Prefix')} value={selectedOrg.code}/><InfoRow label={tx('Τύπος οργανισμού','Organization type')} value={selectedOrg.type||'hospital'}/><InfoRow label={tx('Κατάσταση','Status')} value={selectedOrg.status==='active'?tx('Ενεργός','Active'):tx('Σε παύση','Suspended')} status={selectedOrg.status}/></InfoSection><InfoSection title={tx('Διοικητικά & γεωγραφικά','Administrative & location')}><InfoRow label={tx('Υγειονομική Περιφέρεια (ΥΠΕ)','Health Region')} value={selectedOrg.health_region}/><InfoRow label={tx('Περιφέρεια','Region')} value={selectedOrg.region}/><InfoRow label={tx('Πόλη','City')} value={selectedOrg.city}/><InfoRow label={tx('Χώρα','Country')} value={selectedOrg.country}/></InfoSection><InfoSection title={tx('Επικοινωνία & λειτουργία','Contact & operations')}><InfoRow label={tx('Κεντρικό email','Main email')} value={selectedOrg.contact_email}/><InfoRow label={tx('Τηλέφωνο','Phone')} value={selectedOrg.contact_phone}/><InfoRow label={tx('Δυναμικότητα κλινών','Bed capacity')} value={selectedOrg.bed_capacity}/><InfoRow label={tx('Χρήστες οργανισμού','Organization users')} value={memberCountByOrg[selectedOrg.id]||0}/><InfoRow label="Hospital Admin" value={hospitalAdminStatusByOrg[selectedOrg.id]==='active'?tx('Ενεργός','Active'):hospitalAdminStatusByOrg[selectedOrg.id]==='disabled'?tx('Σε παύση','Suspended'):hospitalAdminStatusByOrg[selectedOrg.id]==='invited'?tx('Εκκρεμής πρόσκληση','Invitation pending'):tx('Δεν έχει οριστεί','Not assigned')}/></InfoSection></div></div>}
      ...
      </section></Page>{editDialog}<PlatformUserDialog organization={selectedOrg} user={selectedUser} language={language} onChange={setSelectedUser} onAction={userAction} onDeleteConfirm={confirmUserDelete}/>{deleteDialog}</>

    return <><Page title={tx('Οργανισμοί','Organizations')} subtitle={tx('Διαχείριση οργανισμών, χρηστών, πρόσβασης και ανάλυσης.','Manage organizations, users, access and analytics.')}><div className="platform-workspace-toolbar"><button className="platform-back-button" onClick={()=>nav('/platform')}><ArrowLeft size={16}/>{tx('Dashboard','Dashboard')}</button></div><section className="platform-center-section"><div className="platform-section-heading"><div><h2>{tx('Οργανισμοί / Νοσοκομεία','Organizations / Hospitals')}</h2><p>{tx('Κλικ σε μια εγγραφή για να ανοίξεις την πλήρη καρτέλα οργανισμού.','Select a row to open the full organization record.')}</p></div><Button onClick={openCreate}>+ {tx('Νέος οργανισμός','New organization')}</Button></div>{organizations.length?<div className="platform-org-list">{organizations.map(org=><button key={org.id} type="button" className={`platform-org-row platform-org-main platform-owner-clickable-row ${org.status==='suspended'?'is-paused':''}`} onClick={()=>openOrganization(org)}><span><strong>{org.name}</strong><small>{org.code} · {org.city||'—'} · {org.region||'—'} · {org.status==='active'?tx('Ενεργός','Active'):tx('ΣΕ ΠΑΥΣΗ','SUSPENDED')} · {memberCountByOrg[org.id]||0} {tx('χρήστες','users')}{hospitalAdminStatusByOrg[org.id]==='invited'&&<span className="status-badge temporary">Admin: {tx('Εκκρεμής','Pending')}</span>}{hospitalAdminStatusByOrg[org.id]==='active'&&<span className="status-badge active">Admin: {tx('Ενεργός','Active')}</span>}{hospitalAdminStatusByOrg[org.id]==='disabled'&&<span className="status-badge danger">Admin: {tx('Σε παύση','Suspended')}</span>}</small></span></button>)}</div>:<div className="empty-state platform-empty"><Building2 size={22}/><strong>{tx('Δεν υπάρχουν οργανισμοί','No organizations')}</strong></div>}</section></Page>{createDialog}</>
  }

  if(activeKey==='demo'){
    if(selectedDemo)return <Page fill title={selectedDemo.organization?.name||selectedDemo.label} subtitle={tx('Demo οργανισμός · απομονωμένο περιβάλλον','Demo organization · isolated environment')}><PlatformDemoRecord demo={selectedDemo} language={language} onBack={returnFromDemoRecord} onOpenDemo={()=>{enterPlatformDemo();nav('/')}} onConvert={()=>setConvertDemo(selectedDemo)}/>{convertDemo&&<ObserverDialog width="wide" eyebrow="Demo → Production" title={tx('Μετατροπή σε μόνιμο οργανισμό','Convert to production organization')} subtitle={tx('Δημιουργείται καθαρός production οργανισμός. Τα synthetic demo δεδομένα δεν αντιγράφονται.','A clean production organization is created. Synthetic demo data is not copied.')} onClose={()=>setConvertDemo(null)}><div className="setup-note"><ShieldCheck size={16}/><span>{tx('Η μετατροπή ανοίγει τη φόρμα Νέου Οργανισμού με προτεινόμενη επωνυμία.','Conversion opens the New Organization form with a suggested name.')}</span></div><Button onClick={()=>{setDraft({...emptyOrganization,code:generateOrganizationCode(),name:convertDemo.label,adminEmail:convertDemo.contact_email||''});setConvertDemo(null);setCreateOpen(true);nav('/platform#organizations')}}>{tx('Συνέχεια στη δημιουργία οργανισμού','Continue to organization creation')}</Button></ObserverDialog>}</Page>

    return <><Page title="Demo" subtitle={tx('Απομονωμένο περιβάλλον παρουσίασης. Τα demo δεδομένα υπάρχουν μόνο εδώ και δεν αναμειγνύονται με πραγματικούς οργανισμούς.','Isolated presentation environment. Demo data exists only here and never mixes with production organizations.')}><div className="platform-workspace-toolbar"><button className="platform-back-button" onClick={()=>nav('/platform')}><ArrowLeft size={16}/>Dashboard</button></div><section className="platform-center-section platform-demo-access"><div className="platform-demo-access-copy"><span className="platform-demo-icon"><FlaskConical size={22}/></span><div><h2>{tx('Demo περιβάλλον','Demo environment')}</h2><p>{tx('Άνοιξε εσύ το πλήρες Limoxis Observer με ασφαλή demo δεδομένα ή ενεργοποίησε χρονικά περιορισμένο Demo για υποψήφιο οργανισμό.','Open Limoxis Observer with safe demo data or enable time-limited access for a prospect.')}</p></div></div><div className="platform-demo-actions"><Button variant="secondary" onClick={()=>setDemoOpen(true)}>+ {tx('Νέο Demo','New Demo')}</Button><Button onClick={()=>{enterPlatformDemo();nav('/')}}>{tx('Πρόσβαση Demo','Open Demo')}</Button></div></section><section className="platform-center-section"><div className="platform-section-heading"><div><h2>{tx('Demo προσβάσεις','Demo access')}</h2><p>{tx('Κλικ σε μια εγγραφή για να ανοίξεις την πλήρη καρτέλα Demo.','Select a row to open the full Demo record.')}</p></div></div>{demos.length?<div className="platform-demo-list">{demos.map(d=><button type="button" className="platform-demo-row platform-owner-clickable-row" key={d.id} onClick={()=>openDemoRecord(d)}><div><strong>{d.organization?.name||d.label}</strong><small>{d.organization?.code||'DEMO'} · {d.contact_name||d.contact_email||'—'} · {d.valid_from} → {d.valid_until}</small></div><span className={`status-badge ${d.status==='active'?'active':'danger'}`}>{d.status==='active'?tx('Ενεργό','Active'):tx('Ανενεργό','Inactive')}</span></button>)}</div>:<div className="inline-empty">{tx('Δεν υπάρχουν Demo προσβάσεις.','No demo access records.')}</div>}</section></Page>{demoOpen&&<ObserverDialog width="wide" eyebrow="Platform Owner" title={tx('Νέο Demo','New Demo')} subtitle={tx('Χρονικά περιορισμένη πρόσβαση σε αποκλειστικά synthetic demo δεδομένα.','Time-limited access to synthetic demo data only.')} onClose={()=>setDemoOpen(false)} footer={<SaveButton loading={demoSaving} disabled={!demoDraft.label.trim()||!demoDraft.contactEmail.trim()||!demoDraft.validUntil} onClick={createDemo}>{tx('Ενεργοποίηση Demo','Enable Demo')}</SaveButton>}><div className="platform-form-grid"><label className="field field-wide"><span>{tx('Οργανισμός / Prospect','Organization / Prospect')} *</span><input value={demoDraft.label} onChange={e=>setDemoDraft(x=>({...x,label:e.target.value}))}/></label><label className="field"><span>{tx('Υπεύθυνος επικοινωνίας','Contact person')}</span><input value={demoDraft.contactName} onChange={e=>setDemoDraft(x=>({...x,contactName:e.target.value}))}/></label><label className="field"><span>Email *</span><input type="email" value={demoDraft.contactEmail} onChange={e=>setDemoDraft(x=>({...x,contactEmail:e.target.value}))}/></label><ManualDateField label={tx('Έναρξη','Start')} value={demoDraft.validFrom} onChange={value=>setDemoDraft(x=>({...x,validFrom:value}))}/><ManualDateField label={`${tx('Λήξη','End')} *`} value={demoDraft.validUntil} onChange={value=>setDemoDraft(x=>({...x,validUntil:value}))}/></div></ObserverDialog>}</>
  }

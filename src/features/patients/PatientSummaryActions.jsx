...
export function PatientSummaryActions({patient,departments,onReload,onDeleted}){
  ...
  const items=[
    has(CAPABILITIES.EDIT_PATIENT)?{label:t('edit'),icon:Pencil,onSelect:()=>setEditing(true)}:null,
    has(CAPABILITIES.DELETE_PATIENT)?{label:t('delete'),icon:Trash2,danger:true,onSelect:()=>setDeleting(true)}:null,
  ].filter(Boolean)
  ...
  return <>
    ...
    <GovernedReasonDialog open={deleting} title={t('delete')} description={language==='el'?'Η διαγραφή θα καταγραφεί στο ιστορικό ελέγχου.':'The deletion will be recorded in the audit history.'} confirmLabel={t('delete')} danger onCancel={()=>setDeleting(false)} onConfirm={remove}/>
  </>
}

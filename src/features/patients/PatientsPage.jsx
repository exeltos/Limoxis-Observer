...
export function PatientFormDialog({t,language,departments,onClose,onSave,patient=null}){
  ...
  return <ObserverDialog width="wide" eyebrow={t('patients')} title={editing?t('edit'):t('newPatient')} subtitle={editing?t('patientRegistrySubtitle'):t('newPatientHelp')} onClose={onClose} footer={<DialogActions showCancel onCancel={onClose} onSave={save} disabled={disabled}/> }>
    <div className="entry-grid patient-entry-grid">
      ...
      <div><ManualDateField label={t('dateOfBirth')} value={draft.dateOfBirth} onChange={v=>set('dateOfBirth',v)}/>{birthInvalid&&<small className="field-error">{language==='el'?'Ελέγξτε την ημερομηνία γέννησης. Δεν μπορεί να είναι μελλοντική ή πριν από το 1900.':'Check the date of birth. It cannot be in the future or before 1900.'}</small>}{ageYears!=null&&!birthInvalid&&<small className="entry-detail-note">{language==='el'?`Υπολογιζόμενη ηλικία: ${ageYears} έτη`:`Calculated age: ${ageYears} years`}</small>}</div>
      ...
      <label><span>{language==='el'?'Βάρος γέννησης (g)':'Birth weight (g)'}</span><input type="number" min="1" value={draft.birthWeightGrams} onChange={e=>set('birthWeightGrams',e.target.value)} placeholder={language==='el'?'Προαιρετικό — για νεογνά':'Optional — for neonates'}/>{birthWeightInvalid&&<small className="field-error">{language==='el'?'Ελέγξτε το βάρος γέννησης (300–7000 g).':'Check birth weight (300–7000 g).'}</small>}</label>
      <label><span>{language==='el'?'Ηλικία κύησης (εβδ.)':'Gestational age (weeks)'}</span><input type="number" min="20" max="45" value={draft.gestationalAgeWeeks} onChange={e=>set('gestationalAgeWeeks',e.target.value)} placeholder={language==='el'?'Προαιρετικό — για νεογνά':'Optional — for neonates'}/>{gestationInvalid&&<small className="field-error">{language==='el'?'Ελέγξτε την ηλικία κύησης (20–45 εβδομάδες).':'Check gestational age (20–45 weeks).'}</small>}</label>

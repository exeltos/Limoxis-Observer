...
function QualityDetails({recordType,record,setRecord,t,language,locale,canManage,notify,actor,finalized,onDeleted,organizationId,navigate}){
  ...
  return <div className="record-section">
    ...
    <div className={`detail-grid quality-detail-grid ${editing?'employee-inline-edit':''}`}>
      ...
      {recordType==='incidents'&&<><EditSelect editing={editing} label={en?'Category':'Κατηγορία συμβάντος'} value={draft.category||'clinical'} onChange={v=>set('category',v)} options={[['clinical',en?'Clinical / patient safety':'Κλινικό / ασφάλεια ασθενούς'],['medication',en?'Medication':'Φάρμακο'],['equipment',en?'Equipment / device':'Εξοπλισμός / ιατροτεχνολογικό'],['process',en?'Process / operational':'Διαδικασία / λειτουργικό'],['staff',en?'Staff / occupational safety':'Προσωπικό / επαγγελματική ασφάλεια'],['facility',en?'Facility / infrastructure':'Εγκαταστάσεις / υποδομές'],['security',en?'Security':'Ασφάλεια'],['other',en?'Other':'Άλλο']]}/><EditSelect editing={editing} label={t('severity')} value={draft.severity} onChange={v=>set('severity',v)} options={['low','medium','high','critical'].map(x=>[x,t(x)])}/><Field label={t('qualityRecords.reportedBy')} value={draft.reportedBy}/><EditField editing={editing} label={t('owner')} value={draft.owner} onChange={v=>set('owner',v)}/><Field label={t('date')} value={fmt(draft.date,locale)}/><EditField editing={editing} label={en?'Time':'Ώρα'} value={draft.eventTime} onChange={v=>set('eventTime',v)} type="time"/><EditSelect editing={editing} label={en?'Actual / potential impact':'Πραγματική / δυνητική επίπτωση'} value={draft.impact||'none'} onChange={v=>set('impact',v)} options={[['none',en?'No harm / impact':'Χωρίς βλάβη / επίπτωση'],['minor',en?'Minor':'Μικρή'],['moderate',en?'Moderate':'Μέτρια'],['major',en?'Major':'Σημαντική'],['severe',en?'Severe':'Σοβαρή']]}/></>}

...
export function QualityCreatePage(){
  ...
  return <Page fill><EntityRecordShell className="quality-record-shell workspace-fill" avatar={<Icon size={19}/>} eyebrow={t('quality')} title={t(c.title)} subtitle={t('newQualityRecord')} tabs={[]} activeTab="" onTabChange={()=>{}} onBack={goBack}>
    <div className="record-section quality-create-form">
      <div className="entry-grid">
        ...
        {recordType==='findings'&&<><label><span>{t('severity')}</span><select value={draft.severity} onChange={e=>set('severity',e.target.value)}>{['low','medium','high','critical'].map(x=><option key={x} value={x}>{t(x)}</option>)}</select></label><label><span>{t('source')}</span><select value={draft.source} onChange={e=>set('source',e.target.value)}><option value="manual">{t('manual')}</option><option value="audit">{t('audit')}</option><option value="incident">{t('incident')}</option></select></label><label><span>{t('sourceId')}</span><input value={draft.sourceId} onChange={e=>set('sourceId',e.target.value)}/></label></>}
        {recordType==='capas'&&<><label><span>{t('actionType')}</span><select value={draft.actionType} onChange={e=>set('actionType',e.target.value)}><option value="corrective">{t('corrective')}</option><option value="preventive">{t('preventive')}</option></select></label><label><span>{t('priority')}</span><select value={draft.priority} onChange={e=>set('priority',e.target.value)}>{['low','medium','high','critical'].map(x=><option key={x} value={x}>{t(x)}</option>)}</select></label><ManualDateField label={t('dueDate')} value={draft.dueDate} onChange={v=>set('dueDate',v)}/><label><span>{t('sourceId')}</span><input value={draft.sourceId} onChange={e=>set('sourceId',e.target.value)}/></label></>}

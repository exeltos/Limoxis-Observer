...
import { AlertTriangle, FileClock, FlaskConical, Microscope, Paperclip, PhoneCall, ShieldAlert } from 'lucide-react'
...
import { addAstResult, communicateCriticalResult, loadLaboratorySample, saveMicrobiologyResult, updateLaboratorySampleStatus } from './laboratoryCloudService'
...
export function LaboratorySampleCloudRecordPage(){
  ...
  const tabs=useMemo(()=>[
    ...
    {id:'ast',label:t('laboratoryRecords.antimicrobialSusceptibility'),icon:ShieldAlert},
  ...
  return <Page fill><EntityRecordShell className="laboratory-record-shell workspace-fill" recordNavigation={recordNavigation} avatar={<FlaskConical size={20}/>} eyebrow={sample.id} title={t(sample.type)} subtitle={`${sample.patient} · ${sample.patientId} · ${sample.department||'—'}`} status={<><Status text={t(sample.status)} kind={sample.status}/>{sample.resistance&&<b className="amr-chip">{sample.resistance}</b>}</>} headerActions={<PrintExportActions onExport={()=>downloadRecordJson(sample,{filename:sample.id})}/>} tabs={tabs} activeTab={tab} onTabChange={setTab} onBack={goBack} backLabel={t('backToLaboratory')}>
    ...
    {tab==='summary'&&<section className="clinical-panel full-panel"><div className="record-section-header"><div><strong>{t('sampleDetails')}</strong><small>{sample.id}</small></div><div className="record-section-actions">{canManage&&sample.status==='requested'&&<Button onClick={()=>saveAndReload(()=>updateLaboratorySampleStatus(tenant?.id,sample.recordId,'received',{receivedAt:new Date()}),t('laboratoryRecords.sampleReceivedMessage'))}>{t('received')}</Button>}{canManage&&['received','collected'].includes(sample.status)&&<Button onClick={()=>saveAndReload(()=>updateLaboratorySampleStatus(tenant?.id,sample.recordId,'processing'),t('laboratoryRecords.sampleProcessingStartedMessage'))}>{t('processing')}</Button>}</div></div><div className="detail-grid patient-detail-grid"><Detail label={t('patient')} value={sample.patient}/><Detail label={t('department')} value={sample.department}/><Detail label={t('sampleType')} value={t(sample.type)}/><Detail label={t('clinicalSource')} value={sample.source}/><Detail label={t('priority')} value={t(sample.priority)}/><Detail label={t('collectedLabel')} value={fmt(sample.collectedAt)}/><Detail label={t('received')} value={fmt(sample.receivedAt)}/><Detail label={t('surveillance')} value={sample.surveillanceCase||'—'}/></div></section>}
    {tab==='result'&&<section className="clinical-panel full-panel"><div className="record-section-header"><div><strong>{t('laboratoryRecords.microbiologyResult')}</strong><small>{result?t(result.resultStatus):t('pending')}</small></div>{canManage&&<Button variant="secondary" onClick={()=>setDialog('result')}>{result?t('edit'):t('clinicalRecords.add')}</Button>}</div>{result?<div className="detail-grid patient-detail-grid"><Detail label={t('result')} value={t(result.result)}/><Detail label={t('exportOrganism')} value={result.organism}/><Detail label={t('classification')} value={result.resistance}/><Detail label={t('criticalResult')} value={result.critical?t('yes'):t('no')}/><Detail label={t('method')} value={result.method}/><Detail label={t('status')} value={t(result.resultStatus)}/></div>:<div className="inline-empty">{t('clinicalRecords.notDocumented')}</div>}</section>}
    {tab==='ast'&&<section className="clinical-panel full-panel"><div className="record-section-header"><div><strong>{t('laboratoryRecords.antimicrobialSusceptibility')}</strong><small>{ast.length}</small></div>{canManage&&result&&<Button variant="secondary" onClick={()=>setDialog('ast')}>+ {t('clinicalRecords.add')}</Button>}</div>{ast.length?<div className="record-table-wrap"><table className="record-table"><thead><tr><th>{t('antibiotic')}</th><th>S/I/R</th><th>{t('method')}</th><th>{t('version')}</th></tr></thead><tbody>{ast.map(row=><tr key={row.id}><td>{row.drug}</td><td><strong>{row.sir}</strong></td><td>{row.method}</td><td>{row.standard} {row.version}</td></tr>)}</tbody></table></div>:<div className="inline-empty">{t('clinicalRecords.notDocumented')}</div>}</section>}
    ...
    {tab==='attachments'&&<LaboratoryAttachmentsPanel organizationId={tenant?.id} sampleRecordId={sample.recordId} canManage={canManage} t={t} notify={notify}/>} 
    {tab==='history'&&<section className="clinical-panel full-panel"><div className="record-section-header"><div><FileClock size={17}/><strong>{t('history')}</strong></div></div><div className="clinical-timeline"><Timeline at={sample.requestedAt} title={t('requested')} fmt={fmt}/>{sample.receivedAt&&<Timeline at={sample.receivedAt} title={t('received')} fmt={fmt}/>} {(sample.microbiologyResults||[]).map((item,index)=><Timeline key={item.id} at={item.resultedAt} title={`${t('laboratoryRecords.microbiologyResult')} · ${t(item.resultStatus)}${index?` · v${sample.microbiologyResults.length-index}`:''}`} fmt={fmt}/>)} {communications.map(row=><Timeline key={row.id} at={row.at} title={t('laboratoryRecords.criticalCommunication')} fmt={fmt}/>)}</div></section>}
  </EntityRecordShell>
  {dialog==='result'&&<ResultDialog t={t} current={result} canValidate={canValidate} onClose={()=>setDialog(null)} onSave={draft=>saveAndReload(()=>saveMicrobiologyResult(tenant?.id,sample.recordId,draft),t('saved'))}/>} 
...
function ResultDialog({t,current,canValidate,onClose,onSave}){const [draft,setDraft]=useState({id:current?.id||null,result:current?.result||'negative',organism:current?.organism||'',resistance:current?.resistance||'',critical:Boolean(current?.critical),method:current?.method||'',preliminary:Boolean(current?.preliminary),validationStatus:current?.resultStatus||'draft',interpretationStandard:current?.interpretationStandard||'EUCAST',interpretationVersion:current?.interpretationVersion||''});return <SimpleDialog title={t('laboratoryRecords.microbiologyResult')} t={t} onClose={onClose} onSave={()=>onSave(draft)} disabled={!draft.result}><label><span>{t('result')}</span><select value={draft.result} onChange={e=>setDraft(d=>({...d,result:e.target.value}))}><option value="positive">{t('positive')}</option><option value="negative">{t('negative')}</option><option value="inconclusive">{t('inconclusive')}</option><option value="contaminated">{t('contaminated')}</option></select></label><label><span>{t('exportOrganism')}</span><input value={draft.organism} onChange={e=>setDraft(d=>({...d,organism:e.target.value}))}/></label><label><span>{t('classification')}</span><select value={draft.resistance} onChange={e=>setDraft(d=>({...d,resistance:e.target.value}))}><option value="">—</option><option value="MDR">MDR</option><option value="XDR">XDR</option><option value="PDR">PDR</option></select></label><label><span>{t('method')}</span><input value={draft.method} onChange={e=>setDraft(d=>({...d,method:e.target.value}))}/></label><label><span>{t('criticalResult')}</span><select value={draft.critical?'yes':'no'} onChange={e=>setDraft(d=>({...d,critical:e.target.value==='yes'}))}><option value="no">{t('no')}</option><option value="yes">{t('yes')}</option></select></label><label><span>{t('status')}</span><select disabled={!canValidate} value={draft.validationStatus} onChange={e=>setDraft(d=>({...d,validationStatus:e.target.value}))}><option value="draft">{t('draft')}</option>{canValidate&&<option value="validated">{t('validated')}</option>}</select></label><label><span>{t('source')}</span><input value={draft.interpretationStandard} onChange={e=>setDraft(d=>({...d,interpretationStandard:e.target.value}))}/></label><label><span>{t('version')}</span><input value={draft.interpretationVersion} onChange={e=>setDraft(d=>({...d,interpretationVersion:e.target.value}))}/></label></SimpleDialog>}

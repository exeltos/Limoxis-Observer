...
import { OrganismEditor } from './OrganismEditor'
...
export function LaboratorySampleCloudRecordPage(){
  ...
  useEffect(()=>{if(tenant?.id)loadManagementLibraries(tenant.id).then(libs=>setMicroorganismOptions((libs?.microorganisms||[]).map(([el,en])=>language==='el'?el:en))).catch(()=>setMicroorganismOptions([]))},[tenant?.id,language])
  ...
  const lockedLabel=t('laboratoryRecords.completePreviousStep')
  const tabAccess={
    summary:true,
  ...
  const tabs=useMemo(()=>[
    ...
    {id:'communication',label:t('laboratoryRecords.criticalCommunication'),icon:PhoneCall,disabled:!tabAccess.communication,lockedLabel},
  ...
  async function removeAst(row){const ok=await confirm({title:t('delete'),message:t('deleteConfirm'),confirmLabel:t('delete'),danger:true});if(!ok)return;await saveAndReload(()=>deleteAstResult(tenant?.id,row.id),t('laboratoryRecords.astDeletedMessage'))}
  ...
  return <Page fill><EntityRecordShell className="laboratory-record-shell workspace-fill" recordNavigation={recordNavigation} avatar={<FlaskConical size={20}/>} eyebrow={sample.id} title={sampleTypeLabel(sample.type,t)} subtitle={`${sample.patient} · ${sample.patientId} · ${sample.department||'—'}`} status={<><Status text={finalized?t('completed'):t(sample.status)} kind={finalized?'completed':sample.status}/>{sample.resistance&&<b className="amr-chip">{sample.resistance}</b>}</>} headerActions={<>{(canManage||canReopen)&&<button className="general-edit-button" title={finalized?text('correction'):text('generalEdit')} onClick={()=>{if(finalized)setDialog('correction');else setTab('result')}}><Pencil size={15}/><span>{finalized?text('correction'):text('generalEdit')}</span></button>}<PrintExportActions onExport={()=>downloadRecordJson(sample,{filename:sample.id})}/></>} tabs={tabs} activeTab={tab} onTabChange={setTab} onBack={goBack} backLabel={t('backToLaboratory')}>
    ...
    {tab==='result'&&<ResultDialogPanel t={t} text={text} result={result} isEnvironmental={isEnvironmental} standard={standard} canManage={canManageActive} canValidate={canValidate} canClassify={canClassify} microorganismOptions={microorganismOptions} onSave={draft=>saveAndReload(()=>saveMicrobiologyResult(tenant?.id,sample.recordId,draft),t('saved'))}/>}
    {tab==='ast'&&!isEnvironmental&&<AstPanel t={t} text={text} ast={ast} organisms={result?.organisms||[]} canManageActive={canManageActive} result={result} resultIsDraft={resultIsDraft} onAdd={()=>setDialog('ast')} onEdit={row=>setDialog({type:'ast',row})} onDelete={removeAst}/>}
  ...
  {dialog==='ast'&&<AstDialog t={t} organisms={result?.organisms||[]} onClose={()=>setDialog(null)} onSave={draft=>saveAndReload(()=>addAstResult(tenant?.id,result.id,draft),t('saved'))}/>}
  {dialog?.type==='ast'&&<AstDialog t={t} organisms={result?.organisms||[]} initial={dialog.row} onClose={()=>setDialog(null)} onSave={draft=>saveAndReload(()=>updateAstResult(tenant?.id,dialog.row.id,draft),t('saved'))}/>}
...
function ResultDialogPanel({t,text,result,isEnvironmental,standard,canManage,canValidate,canClassify,microorganismOptions,onSave}){
  const [editing,setEditing]=useState(false)
  const hasConfiguredLimit=standard?.limitCfu!=null&&standard?.limitCfu!==''
  const draftFrom=()=>({id:result?.id||null,result:result?.result||(isEnvironmental?'':'negative'),organisms:result?.organisms?.length?result.organisms:(result?.organism?[{name:result.organism,resistance:result.resistance||''}]:[]),critical:Boolean(result?.critical),method:result?.method||'',preliminary:Boolean(result?.preliminary),validationStatus:result?.resultStatus||'draft',interpretationStandard:result?.interpretationStandard||'EUCAST',interpretationVersion:result?.interpretationVersion||'',cfuCount:result?.cfuCount??''})
  const [draft,setDraft]=useState(draftFrom)
  useEffect(()=>{setDraft(draftFrom())},[result?.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function save(){
    ...
    onSave({...draft,organisms:draft.result==='positive'?draft.organisms:[],cfuCount:isEnvironmental?(draft.result==='negative'?0:(draft.cfuCount===''?null:Number(draft.cfuCount))):null,cfuLimit,withinLimit})
    setEditing(false)
  }
  ...
  return <div className="clinical-panel full-panel record-section">
    ...
    {editing?<div className="detail-grid patient-detail-grid employee-inline-edit">
      ...
      {isEnvironmental?draft.result==='positive'&&<div className="detail-item"><span>{text('cfu')}</span><input inputMode="decimal" value={draft.cfuCount} onChange={e=>setDraft(d=>({...d,cfuCount:e.target.value}))} placeholder="CFU"/></div>:draft.result==='positive'&&<div className="detail-item entry-span-2 lab-organisms-editor"><span>{t('laboratoryRecords.organisms')}</span><OrganismEditor organisms={draft.organisms} onChange={rows=>setDraft(d=>({...d,organisms:rows}))} options={microorganismOptions} t={t} canClassify={canClassify}/></div>}
      ...
      {isEnvironmental?<><Detail label={text('cfu')} value={result?.result==='negative'?'0':(result?.cfuCount??'—')}/><Detail label={text('acceptableLimit')} value={result?.cfuLimit!=null?`${result.cfuLimit} ${standard?.unit||'CFU'}`:'—'}/><Detail label={text('assessment')} value={result?.withinLimit===true?text('withinLimits'):result?.withinLimit===false?text('outsideLimits'):result?'—':'—'}/></>:<div className="detail-item entry-span-2"><span>{t('laboratoryRecords.organisms')}</span><div className="organism-read-list">{(result?.organisms?.length?result.organisms:[]).map((row,index)=><span key={`${row.name}-${index}`}><strong>{row.name}</strong>{row.resistance&&<b className="amr-chip">{row.resistance}</b>}</span>)}{!result?.organisms?.length&&<strong>—</strong>}</div></div>}
      {!isEnvironmental&&<Detail label={t('criticalResult')} value={result?.critical?t('yes'):t('no')}/>}
...
function AstPanel({t,text,ast,organisms,canManageActive,result,resultIsDraft,onAdd,onEdit,onDelete}){
  const multiOrganism=(organisms||[]).length>1
  return <section className="clinical-panel full-panel"><div className="record-section-header"><div><strong>{t('laboratoryRecords.antimicrobialSusceptibility')}</strong><small>{ast.length}</small></div>{canManageActive&&result&&resultIsDraft&&<Button variant="secondary" onClick={onAdd}>+ {t('clinicalRecords.add')}</Button>}</div>{result&&!resultIsDraft&&<div className="permission-info-banner"><ShieldAlert size={16}/><span>{text('finalizedAstNotice')}</span></div>}{ast.length?<div className="record-table-wrap"><table className="record-table"><thead><tr>{multiOrganism&&<th>{t('organism')}</th>}<th>{t('antimicrobial')}</th><th>S/I/R</th><th>{t('method')}</th><th>{t('version')}</th>{canManageActive&&result&&resultIsDraft&&<th>{t('actions')}</th>}</tr></thead><tbody>{ast.map(row=><tr key={row.id}>{multiOrganism&&<td>{row.organism||'—'}</td>}<td>{row.drug}</td><td><strong>{row.sir}</strong></td><td>{row.method}</td><td>{row.standard} {row.version}</td>{canManageActive&&result&&resultIsDraft&&<td><OverflowMenu items={[{id:'edit',label:t('edit'),icon:Pencil,onClick:()=>onEdit(row)},{id:'delete',label:t('delete'),icon:Trash2,tone:'danger',separatorBefore:true,onClick:()=>onDelete(row)}]}/></td>}</tr>)}</tbody></table></div>:<div className="inline-empty">{t('clinicalRecords.notDocumented')}</div>}</section>
}

function AstDialog({t,organisms=[],initial=null,onClose,onSave}){const [draft,setDraft]=useState({organism:initial?.organism||organisms[0]?.name||'',drug:initial?.drug||'',method:initial?.method||'MIC',sir:initial?.sir||'S',standard:initial?.standard||'EUCAST',version:initial?.version||'',mic:initial?.mic||'',operator:initial?.operator||'',zone:initial?.zone||'',notes:initial?.notes||''});return <SimpleDialog title={t('laboratoryRecords.antimicrobialSusceptibility')} t={t} onClose={onClose} onSave={()=>onSave(draft)} disabled={!draft.drug||!draft.version}>{organisms.length>1&&<label><span>{t('organism')}</span><select value={draft.organism} onChange={e=>setDraft(d=>({...d,organism:e.target.value}))}>{organisms.map(row=><option key={row.name} value={row.name}>{row.name}</option>)}</select></label>}<label><span>{t('antimicrobial')}</span><input value={draft.drug} onChange={e=>setDraft(d=>({...d,drug:e.target.value}))}/></label><label><span>S/I/R</span><select value={draft.sir} onChange={e=>setDraft(d=>({...d,sir:e.target.value}))}><option>S</option><option>I</option><option>R</option></select></label><label><span>{t('method')}</span><input value={draft.method} onChange={e=>setDraft(d=>({...d,method:e.target.value}))}/></label><label><span>{t('source')}</span><input value={draft.standard} onChange={e=>setDraft(d=>({...d,standard:e.target.value}))}/></label><label><span>{t('version')}</span><input value={draft.version} onChange={e=>setDraft(d=>({...d,version:e.target.value}))}/></label><label><span>MIC</span><input value={draft.mic} onChange={e=>setDraft(d=>({...d,mic:e.target.value}))}/></label></SimpleDialog>}

...
export function LaboratorySampleRecordView(){
  ...
  const canManageActive=canManage&&!finalized&&!rejected
  ...
  async function markDocuments(){await saveAndReload(()=>repository.markDocumentsReviewed(sample.id),text('documentsReviewed'))}
  async function finalize(){if(!readyToFinalize)return;await saveAndReload(()=>repository.finalize(sample.id),text('finalized'))}
  ...
  async function startProcessing(){
    if(finalized){setDialog('correction');return}
    if(rejected)return
    try{
      if(sample.status!=='processing')await repository.updateStatus(sample.id,'processing',{receivedAt:sample.receivedAt||new Date()})
      await reload()
      setTab('result')
      notify(text('processSampleStarted'),'success')
  ...
  return <Page fill><EntityRecordShell className="laboratory-record-shell workspace-fill" recordNavigation={recordNavigation} avatar={<FlaskConical size={20}/>} eyebrow={sample.id} title={sampleTypeLabel(sample.type,t)} subtitle={`${subjectName}${subjectCode?` · ${subjectCode}`:''} · ${sample.department||'—'}`} status={<><Status text={finalized?t('completed'):t(sample.status)} kind={finalized?'completed':sample.status}/>{sample.resistance&&<b className="amr-chip">{sample.resistance}</b>}</>} headerActions={<>{canManageActive&&<button className="general-edit-button" title={text('processSample')} onClick={startProcessing}><Pencil size={15}/><span>{text('processSample')}</span></button>}{canManageActive&&!['completed','cancelled'].includes(sample.status)&&<Button variant="secondary" onClick={()=>setDialog('reject')}>{text('rejectSample')}</Button>}{finalized&&canReopen&&<button className="general-edit-button" title={text('correction')} onClick={()=>setDialog('correction')}><Pencil size={15}/><span>{text('correction')}</span></button>}<PrintExportActions onExport={()=>downloadRecordJson(sample,{filename:sample.id})}/></>} tabs={tabs} activeTab={tab} onTabChange={setTab} onBack={goBack} backLabel={t('backToLaboratory')}>
    ...
    {rejected&&tab==='summary'&&<div className="permission-info-banner"><AlertTriangle size={16}/><span>{text('rejectedReadOnly')}</span></div>}
    {tab==='summary'&&<section className="clinical-panel full-panel record-section"><div className="record-section-header"><div><span className="eyebrow">{t('summary')}</span><h3>{t('sampleDetails')}</h3></div></div><LaboratorySampleSummary sample={sample} t={t} language={language} fmt={fmt}/>{sample.rejectionReason&&<div className="permission-info-banner"><AlertTriangle size={16}/><span><strong>{text('rejectionReason')}:</strong> {sample.rejectionReason}</span></div>}</section>}
    {tab==='result'&&<ResultDialogPanel t={t} text={text} result={result} isEnvironmental={isEnvironmental} standard={standard} canManage={canManageActive} canValidate={canValidate} onSave={draft=>saveAndReload(()=>repository.saveResult(sample.id,draft),t('saved'))}/>}
    ...
    {tab==='attachments'&&<EntityAttachmentsPanel organizationId={tenant?.id} entityType="laboratory_sample" entityRecordId={sample.recordId} category="laboratory_evidence" canManage={canManageActive} t={t} notify={notify}/>}
    {tab==='attachments'&&!finalized&&!rejected&&<div className="lab-step-footer"><Button variant="secondary" disabled={documentsReviewed} onClick={markDocuments}>{documentsReviewed?text('checkDocuments'):text('markDocumentsReviewed')}</Button></div>}
    {tab==='finalize'&&<section className="clinical-panel full-panel finalization-panel"><div className="record-section-header"><div><span className="eyebrow">{text('finalize')}</span><h3>{text('finalizeRecord')}</h3></div></div><div className="finalization-checklist"><FinalCheck ok={Boolean(resultValidated)} text={text('checkResult')}/><FinalCheck ok={astComplete} text={text('checkAst')}/><FinalCheck ok={communicationComplete} text={text('checkCommunication')}/><FinalCheck ok={documentsReviewed} text={text('checkDocuments')}/></div>{finalized?<div className="validated-result-note"><CheckCircle2 size={17}/><span>{text('finalized')}</span></div>:<div className="finalization-warning"><LockKeyhole size={17}/><div><strong>{text('finalize')}</strong><span>{text('finalizationWarning')}</span></div></div>}{canValidate&&!finalized&&!rejected&&<div className="lab-step-footer"><Button disabled={!readyToFinalize} onClick={finalize}><CheckCircle2 size={15}/>{text('finalizeRecord')}</Button></div>}</section>}
  ...
  {dialog==='ast'&&<AstDialog t={t} onClose={()=>setDialog(null)} onSave={draft=>saveAndReload(()=>repository.addAst(sample.id,result.id,draft),t('saved'))}/>}
  {dialog==='communication'&&<CommunicationDialog t={t} onClose={()=>setDialog(null)} onSave={draft=>saveAndReload(()=>repository.communicate(sample.id,result.id,draft),t('saved'))}/>}
  {dialog==='reject'&&<RejectDialog t={t} title={text('rejectSample')} reasonLabel={text('rejectionReason')} onClose={()=>setDialog(null)} onSave={reason=>saveAndReload(()=>repository.updateStatus(sample.id,'rejected',{rejectionReason:reason}),text('sampleRejected'))}/>}
...
function ResultDialogPanel({t,text,result,isEnvironmental,standard,canManage,canValidate,onSave}){
  const [editing,setEditing]=useState(()=>canManage&&!result)
  const hasConfiguredLimit=standard?.limitCfu!=null&&standard?.limitCfu!==''
  const emptyResult=''
  const makeDraft=()=>({id:result?.id||null,result:result?.result||emptyResult,organism:result?.organism||'',resistance:result?.resistance||'',critical:Boolean(result?.critical),method:result?.method||'',preliminary:Boolean(result?.preliminary),validationStatus:result?.resultStatus||'draft',interpretationStandard:result?.interpretationStandard||'EUCAST',interpretationVersion:result?.interpretationVersion||'',cfuCount:result?.cfuCount??''})
  const [draft,setDraft]=useState(makeDraft)
  useEffect(()=>{setDraft(makeDraft());setEditing(canManage&&!result)},[result?.id,canManage]) // eslint-disable-line react-hooks/exhaustive-deps
  function computeCfuLimit(){return hasConfiguredLimit?Number(standard.limitCfu):null}
  function computeWithinLimit(nextResult,cfu){if(nextResult==='negative')return true;if(nextResult==='positive')return cfu!==''&&hasConfiguredLimit&&Number.isFinite(Number(cfu))?Number(cfu)<=computeCfuLimit():null;return null}
  function save(){const cfuLimit=isEnvironmental?computeCfuLimit():null;const withinLimit=isEnvironmental?computeWithinLimit(draft.result,draft.result==='negative'?'0':draft.cfuCount):null;onSave({...draft,cfuCount:isEnvironmental?(draft.result==='negative'?0:(draft.cfuCount===''?null:Number(draft.cfuCount))):null,cfuLimit,withinLimit});setEditing(false)}
  ...
  return <div className="clinical-panel full-panel record-section"><div className="record-section-header"><div><span className="eyebrow">{t('laboratoryRecords.microbiologyResult')}</span><h3>{isEnvironmental?t('laboratoryRecords.environmentalResult'):t('laboratoryRecords.resultAndOrganism')}</h3></div>{canManage&&!editing&&<Button variant="secondary" onClick={()=>setEditing(true)}><Pencil size={14}/>{result?t('edit'):t('clinicalRecords.add')}</Button>}</div>{isEnvironmental&&<div className={`smart-protocol-strip ${standard?.limitCfu!=null?'configured':'missing'}`}><div><strong>{standard?.protocolCode||text('noProtocolConfigured')}</strong>{hasConfiguredLimit&&<span> · {standard.limitCfu} {standard.unit||'CFU'}</span>}</div><span className="smart-lock-chip">🔒 {text('protocolNote')}</span></div>}{editing?<div className="detail-grid patient-detail-grid employee-inline-edit"><div className="detail-item"><span>{t('result')}</span><select value={draft.result} onChange={e=>setDraft(d=>({...d,result:e.target.value}))}><option value="">{t('select')}</option><option value="negative">{t('negative')}</option><option value="positive">{t('positive')}</option>{!isEnvironmental&&<><option value="inconclusive">{t('inconclusive')}</option><option value="contaminated">{t('contaminated')}</option></>}</select></div>{isEnvironmental?draft.result==='positive'&&<div className="detail-item"><span>{text('cfu')}</span><input inputMode="decimal" value={draft.cfuCount} onChange={e=>setDraft(d=>({...d,cfuCount:e.target.value}))} placeholder="CFU"/></div>:<div className="detail-item"><span>{t('exportOrganism')}</span><input value={draft.organism} onChange={e=>setDraft(d=>({...d,organism:e.target.value}))}/></div>}{!isEnvironmental&&<div className="detail-item"><span>{t('classification')}</span><select value={draft.resistance} onChange={e=>setDraft(d=>({...d,resistance:e.target.value}))}><option value="">—</option><option value="MDR">MDR</option><option value="XDR">XDR</option><option value="PDR">PDR</option></select></div>}<div className="detail-item"><span>{t('method')}</span><input value={draft.method} onChange={e=>setDraft(d=>({...d,method:e.target.value}))}/></div>{!isEnvironmental&&<div className="detail-item"><span>{t('criticalResult')}</span><select value={draft.critical?'yes':'no'} onChange={e=>setDraft(d=>({...d,critical:e.target.value==='yes'}))}><option value="no">{t('no')}</option><option value="yes">{t('yes')}</option></select></div>}<div className="detail-item"><span>{t('status')}</span><select disabled={!canValidate} value={draft.validationStatus} onChange={e=>setDraft(d=>({...d,validationStatus:e.target.value}))}><option value="draft">{t('draft')}</option>{canValidate&&<option value="validated">{t('validated')}</option>}</select></div></div>:<div className="detail-grid patient-detail-grid"><Detail label={t('result')} value={result?t(result.result):'—'}/>{isEnvironmental?<><Detail label={text('cfu')} value={result?.result==='negative'?'0':(result?.cfuCount??'—')}/><Detail label={text('acceptableLimit')} value={result?.cfuLimit!=null?`${result.cfuLimit} ${standard?.unit||'CFU'}`:'—'}/><Detail label={text('assessment')} value={result?.withinLimit===true?text('withinLimits'):result?.withinLimit===false?text('outsideLimits'):'—'}/></>:<><Detail label={t('exportOrganism')} value={result?.organism}/><Detail label={t('classification')} value={result?.resistance}/><Detail label={t('criticalResult')} value={result?.critical?t('yes'):t('no')}/></>}<Detail label={t('method')} value={result?.method}/><Detail label={t('status')} value={result?t(result.resultStatus):t('pending')}/></div>}{editing&&<div className="inline-edit-footer"><Button variant="secondary" onClick={()=>setEditing(false)}>{t('cancel')}</Button><SaveButton disabled={!complete} onClick={save}>{t('save')}</SaveButton></div>}</div>
}

...
import { AlertTriangle, CheckCircle2, Clock3, FlaskConical, Microscope, ShieldAlert, UserRound, UsersRound, Building2 } from 'lucide-react'
...
export function LaboratoryWorkspace() {
  ...
  const surveillanceLabel = sample => {
    ...
    if (!value) return '—'
    return isUuid(value) ? (language === 'el' ? 'Συνδεδεμένη εγγραφή' : 'Linked record') : value
  }
  const sourceLabel = sample => {
    const value = language === 'el' ? sample.source : sample.sourceEn
    if (!value) return '—'
    const known = sampleSourceCatalog[value]
    return known ? known[language] || known.el || value : value
  }

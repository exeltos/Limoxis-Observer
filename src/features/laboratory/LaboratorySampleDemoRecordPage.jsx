...
export function LaboratorySampleRecordPage(){
  ...
  return <Page fill>
    <EntityRecordShell
      ...
      footer={<LaboratoryWorkflowNavigator active={tab} order={workflowOrder} labels={workflowLabels} canOpen={id=>Boolean(tabAccess[id])} onMove={setTab}/>}
    >
...
function AstPanel({sample,persist,t,language,canManage,notify,actorName,onNext}){
  ...
  return <div className="record-section">
    ...
    {sample.ast?.length?<div className="record-table-wrap"><table className="record-table ast-record-table"><thead><tr>{availableOrganisms.length>1&&<th>{t('organism')}</th>}<th>{t('antimicrobial')}</th><th>{t('laboratoryRecords.sir')}</th><th>{t('laboratoryRecords.micFullLabel')}</th><th>{t('laboratoryRecords.method')}</th><th>{t('laboratoryRecords.breakpointStandard')}</th>{canManage&&<th>{t('actions')}</th>}</tr></thead><tbody>{sample.ast.map((row,index)=><tr key={`${row.organism||''}-${row.drug}-${index}`}>{availableOrganisms.length>1&&<td>{row.organism||'—'}</td>}<td><strong>{row.drug}</strong></td><td><b className={`sir ${row.sir.toLowerCase()}`}>{row.sir}</b></td><td>{row.mic||'—'}</td><td>{row.method||'—'}</td><td>{row.standard||'EUCAST'} {row.version||''}</td>{canManage&&<td><OverflowMenu items={[
      {id:'edit',label:t('edit'),icon:Pencil,onClick:()=>openEdit(row,index)},
      {id:'delete',label:t('delete'),icon:Trash2,tone:'danger',separatorBefore:true,onClick:()=>remove(index)},
    ]}/></td>}</tr>)}</tbody></table></div>:available?<div className="inline-empty">{t('laboratoryRecords.noAstResults')}</div>:null}
...
function EmployeeScreeningLaboratoryRecord({sample,persist,t,language,fmt,canManage,canValidate,canAttach,canReopen,canPrint,canExport,notify,recordNavigation,actor,actorName}){
  ...
  return <Page fill><EntityRecordShell
    ...
    footer={<LaboratoryWorkflowNavigator active={tab} order={workflowOrder} labels={workflowLabels} canOpen={id=>Boolean(access[id])} onMove={setTab}/>}
  >
...
function EnvironmentalLaboratoryRecord({sample,persist,t,language,fmt,canManage,canValidate,canAttach,canReopen,canPrint,canExport,notify,recordNavigation,actor,actorName}){
  ...
  return <Page fill>
    <EntityRecordShell
      ...
      backLabel={t('backToLaboratory')}
      footer={<LaboratoryWorkflowNavigator active={tab} order={order} labels={workflowLabels} canOpen={id=>Boolean(access[id])} onMove={setTab}/>}
    >

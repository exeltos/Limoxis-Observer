...
function ExposureIncidents({rows,employeeMap,name,en,fmt,openEmployee,highlightId}){
 ...
 return <table className="data-table sticky-table"><thead><tr><th>{en?'Date':'Ημερομηνία'}</th><th>{en?'Employee':'Εργαζόμενος'}</th><th>{en?'Exposure type':'Τύπος έκθεσης'}</th><th>{en?'Follow-up':'Παρακολούθηση'}</th><th>{en?'Status':'Κατάσταση'}</th></tr></thead><tbody>{rows.map(x=>{const e=employeeMap[x.employeeId];return <tr key={x.id} data-record-id={x.id} className={`clickable-row${highlightId===x.id?' registry-row-returned':''}`} tabIndex={0} onClick={()=>openEmployee(e.id,x.id)} onKeyDown={ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();openEmployee(e.id,x.id)}}}><td>{fmt(x.incidentDate)}</td><td><strong>{name(e)}</strong><small>{e.id}</small></td><td>{EXPOSURE_TYPE_LABELS[x.exposureType]?.[en?1:0]||x.exposureType}</td><td>{FOLLOW_UP_LABELS[x.followUpStatus]?.[en?1:0]||x.followUpStatus}</td><td><span className={`status-badge ${x.status==='open'?'temporary':''}`}>{x.status==='open'?(en?'Open':'Ανοικτό'):(en?'Closed':'Κλειστό')}</span></td></tr>})}</tbody></table>
}

...
export function OccupationalHealthPage(){
 ...
 const reloadExposureIncidents=useCallback(async()=>{if(!tenant?.id)return;setExposureLoading(true);try{setExposureRows(await loadAllExposureIncidentsAsync(tenant.id))}catch(error){notify(error?.message||(translate('copy.occupationalCopy.couldNotLoadExposureIncidents',en?'en':'el')),'error')}finally{setExposureLoading(false)}},[tenant?.id,notify,en])
 ...
 function action(a){
  ...
  if(a===UI_ACTIONS.EXPORT){
   if(section==='exposure'){downloadCsv('limoxis-occupational-exposure-incidents.csv',[t('date'),t('employee'),translate('copy.occupationalCopy.exposureType',en?'en':'el'),translate('copy.occupationalCopy.followUp',en?'en':'el'),t('status')],exposureFiltered.map(x=>{const e=employeeMap[x.employeeId];return [x.incidentDate,name(e),x.exposureType,x.followUpStatus,x.status]}));notify(t('currentListExported'),'success');return}
 ...
 const sectionTabs=[{id:'visits',label:t('occupationalHealth'),icon:HeartPulse},{id:'exposure',label:translate('copy.occupationalCopy.exposureIncidents',en?'en':'el'),icon:ShieldAlert}]
 const createLabel=section==='exposure'?(translate('copy.occupationalCopy.newExposureIncident',en?'en':'el')):t('newOccupationalVisit')
...
function Visits({rows,employeeMap,name,t,fmt,openEmployee,highlightId}){return <table className="data-table sticky-table"><thead><tr><th>{t('date')}</th><th>{t('employee')}</th><th>{t('visitType')}</th><th>{t('status')}</th><th>{t('fitnessStatus')}</th><th>{t('followUp')}</th></tr></thead><tbody>{rows.map(x=>{const e=employeeMap[x.employeeId];return <tr key={x.id} data-record-id={x.id} className={`clickable-row${highlightId===x.id?' registry-row-returned':''}`} tabIndex={0} onClick={()=>openEmployee(e.id,x.id)} onKeyDown={ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();openEmployee(e.id,x.id)}}}><td>{fmt(x.date)}</td><td><strong>{name(e)}</strong><small>{e.id}</small></td><td>{t(x.type)}</td><td><span className="status-badge active">{t(x.status)}</span></td><td>{t(x.fitStatus)}</td><td>{fmt(x.followUpDate)}</td></tr>})}</tbody></table>}
...
function ExposureIncidents({rows,employeeMap,name,en,fmt,openEmployee,highlightId}){
 if(!rows.length)return <div className="inline-empty">{translate('copy.occupationalCopy.noExposureIncidentsRecorded',en?'en':'el')}</div>
 return <table className="data-table sticky-table"><thead><tr><th>{translate('copy.occupationalCopy.date',en?'en':'el')}</th><th>{translate('copy.occupationalCopy.employee',en?'en':'el')}</th><th>{translate('copy.occupationalCopy.exposureType',en?'en':'el')}</th><th>{translate('copy.occupationalCopy.followUp',en?'en':'el')}</th><th>{translate('copy.occupationalCopy.status',en?'en':'el')}</th></tr></thead><tbody>{rows.map(x=>{const e=employeeMap[x.employeeId];return <tr key={x.id} data-record-id={x.id} className={`clickable-row${highlightId===x.id?' registry-row-returned':''}`} tabIndex={0} onClick={()=>openEmployee(e.id,x.id)} onKeyDown={ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();openEmployee(e.id,x.id)}}}><td>{fmt(x.incidentDate)}</td><td><strong>{name(e)}</strong><small>{e.id}</small></td><td>{EXPOSURE_TYPE_LABELS[x.exposureType]?.[en?1:0]||x.exposureType}</td><td>{FOLLOW_UP_LABELS[x.followUpStatus]?.[en?1:0]||x.followUpStatus}</td><td><span className={`status-badge ${x.status==='open'?'temporary':''}`}>{x.status==='open'?(translate('copy.occupationalCopy.open',en?'en':'el')):(translate('copy.occupationalCopy.closed',en?'en':'el'))}</span></td></tr>})}</tbody></table>
}

...
export function CommitteeRecordPage(){
  ...
  async function addMeeting(draft){
    ...
    if(result){if(!isDemo)await reload();setDialog({type:'meeting',id})}
  }
  async function saveMeeting(draft,finalize=false){
    ...
    const result=await execute({operation:()=>saveCommitteeMeetingAsync(organizationId,record,next,{finalize}),local:()=>({...record,meetings:record.meetings.map(x=>x.id===next.id?{...next,status:localStatus,finalizedAt:finalize?new Date().toISOString():x.finalizedAt}:x)}),success:finalize?(en?'Minutes submitted/finalized.':'Τα πρακτικά υποβλήθηκαν / οριστικοποιήθηκαν.'):(en?'Meeting saved.':'Η συνεδρίαση αποθηκεύτηκε.'),close:finalize})
...
function Members({rows,canManage,onAdd,onEdit,onEnd,en}){
  return <section className="record-section"><Head title={en?'Committee members':'Μέλη επιτροπής'} subtitle={en?'Membership is ended, not deleted, preserving history.':'Η συμμετοχή λήγει αντί να διαγράφεται, ώστε να διατηρείται ιστορικότητα.'} action={canManage&&<Button onClick={onAdd}><Plus size={15}/>{en?' Add member':' Προσθήκη μέλους'}</Button>}/><div className="scroll-table"><table className="data-table"><thead><tr><th>{en?'Member':'Μέλος'}</th><th>{en?'Role':'Ιδιότητα'}</th><th>{en?'Responsibilities':'Αρμοδιότητες'}</th><th>{en?'Vote':'Ψήφος'}</th><th>{en?'Period':'Περίοδος'}</th>{canManage&&<th/>}</tr></thead><tbody>{rows.map(m=><tr key={m.id} className={m.active===false?'committee-member-inactive':''}><td><strong>{m.name}</strong><small>{[m.profession,m.department].filter(Boolean).join(' · ')||'—'}</small></td><td>{m.committeeTitle||'—'}</td><td>{m.responsibilities||'—'}</td><td>{m.voting?(en?'Yes':'Ναι'):(en?'No':'Όχι')}</td><td>{fmtDate(m.startedAt)} → {m.endedAt?fmtDate(m.endedAt):(en?'today':'σήμερα')}</td>{canManage&&<td><div className="record-inline-actions committee-member-actions">{m.active!==false&&<button type="button" className="lo-icon-button lo-icon-button-edit lo-icon-button-sm" title={en?'Edit member':'Επεξεργασία μέλους'} aria-label={en?'Edit member':'Επεξεργασία μέλους'} onClick={()=>onEdit(m)}><Pencil size={15}/></button>}{m.active!==false&&<button type="button" className="lo-icon-button lo-icon-button-danger lo-icon-button-sm" title={en?'End membership':'Λήξη συμμετοχής'} aria-label={en?'End membership':'Λήξη συμμετοχής'} onClick={()=>onEnd(m)}><Trash2 size={15}/></button>}</div></td>}</tr>)}</tbody></table>{!rows.length&&<div className="inline-empty">{en?'No members yet. Add the committee composition from here.':'Δεν υπάρχουν ακόμη μέλη. Προσθέστε τη σύνθεση της επιτροπής από εδώ.'}</div>}</div></section>
}

function Meetings({rows,canCreate,canCancel,onAdd,onOpen,onCancel,en}){
  return <section className="record-section"><Head title={en?'Meetings & minutes':'Συνεδριάσεις & πρακτικά'} subtitle={en?'Attendance, quorum, agenda, minutes and governed approvals.':'Παρουσίες, απαρτία, θέματα, πρακτικά και ελεγχόμενες εγκρίσεις.'} action={canCreate&&<Button onClick={onAdd}><Plus size={15}/>{en?' New meeting':' Νέα συνεδρίαση'}</Button>}/><div className="scroll-table"><table className="data-table record-table-clickable"><thead><tr><th>{en?'Date':'Ημερομηνία'}</th><th>{en?'Meeting':'Συνεδρίαση'}</th><th>{en?'Minutes no.':'Αρ. πρακτικού'}</th><th>{en?'Quorum':'Απαρτία'}</th><th>{en?'Status':'Κατάσταση'}</th>{canCancel&&<th/>}</tr></thead><tbody>{rows.map(x=><tr key={x.id} onClick={()=>onOpen(x)}><td>{fmtDate(x.date)}<small>{x.time||''}</small></td><td><strong>{x.title}</strong><small>{x.meetingType==='extraordinary'?(en?'Extraordinary':'Έκτακτη'):(en?'Regular':'Τακτική')}{x.location?` · ${x.location}`:''}</small>{x.status==='cancelled'&&x.cancellationReason&&<small>{en?'Reason':'Αιτιολογία'}: {x.cancellationReason}</small>}</td><td>{x.minutesNo||'—'}</td><td>{x.quorum===true?(en?'Yes':'Ναι'):x.quorum===false?(en?'No':'Όχι'):'—'}</td><td><span className={`status-badge ${x.status==='finalized'?'active':x.status==='approval_pending'?'temporary':''}`}>{statusLabel(x.status,en)}</span></td>{canCancel&&<td>{['draft','planned','in_progress'].includes(x.status)&&<button className="danger" onClick={e=>{e.stopPropagation();onCancel(x)}} title={en?'Cancel meeting':'Ακύρωση συνεδρίασης'}><XCircle size={15}/></button>}</td>}</tr>)}</tbody></table>{!rows.length&&<div className="inline-empty">{en?'No meetings yet.':'Δεν υπάρχουν ακόμη συνεδριάσεις.'}</div>}</div></section>
}
...
function Decisions({rows,canManage,onAdd,onEdit,onStatus,en}){
  ...
  return <section className="record-section"><Head title={en?'Decisions & actions':'Αποφάσεις & ενέργειες'} action={canManage&&<Button onClick={onAdd}><Plus size={15}/>{en?' New decision':' Νέα απόφαση'}</Button>}/><FilterBar query={q} onQueryChange={setQ} onClear={()=>{setQ('');setStatus('all')}} activeAdvancedCount={status==='all'?0:1}><FilterSelect label={en?'Status':'Κατάσταση'} value={status} onChange={setStatus}><option value="all">{en?'All':'Όλες'}</option><option value="open">{en?'Open':'Ανοιχτή'}</option><option value="in_progress">{en?'In progress':'Σε εξέλιξη'}</option><option value="completed">{en?'Completed':'Ολοκληρωμένη'}</option></FilterSelect></FilterBar><div className="scroll-table"><table className="data-table"><thead><tr><th>{en?'Decision':'Απόφαση'}</th><th>{en?'Owner':'Υπεύθυνος'}</th><th>{en?'Due':'Προθεσμία'}</th><th>{en?'Status':'Κατάσταση'}</th>{canManage&&<th/>}</tr></thead><tbody>{filtered.map(x=><tr key={x.id}><td><strong>{x.title}</strong><small>{x.action||'—'}</small></td><td>{x.owner||'—'}</td><td>{fmtDate(x.dueDate)}</td><td><span className="status-badge">{x.status}</span></td>{canManage&&<td><div className="record-inline-actions"><button onClick={()=>onEdit(x)}><Pencil size={15}/></button>{x.status!=='completed'&&<button onClick={()=>onStatus(x,'completed')}><CheckCircle2 size={15}/></button>}</div></td>}</tr>)}</tbody></table>{!filtered.length&&<div className="inline-empty">{en?'No decisions or actions yet.':'Δεν υπάρχουν ακόμη αποφάσεις ή ενέργειες.'}</div>}</div></section>
}

function Plan({rows,canManage,onAdd,onEdit,onDelete,en}){
  return <section className="record-section"><Head title={en?'Annual action plan':'Ετήσιο σχέδιο δράσης'} action={canManage&&<Button onClick={onAdd}><Plus size={15}/>{en?' New objective':' Νέος στόχος'}</Button>}/><div className="scroll-table"><table className="data-table"><thead><tr><th>{en?'Objective':'Στόχος'}</th><th>{en?'Indicator':'Δείκτης'}</th><th>{en?'Baseline → target':'Baseline → στόχος'}</th><th>{en?'Owner':'Υπεύθυνος'}</th><th>{en?'Due':'Προθεσμία'}</th><th>{en?'Status':'Κατάσταση'}</th>{canManage&&<th/>}</tr></thead><tbody>{rows.map(x=><tr key={x.id}><td><strong>{x.title}</strong></td><td>{x.indicator||'—'}</td><td>{x.baseline||'—'} → {x.target||'—'}</td><td>{x.owner||'—'}</td><td>{fmtDate(x.dueDate)}</td><td><span className={`status-badge ${workflowStatusClass(x.status)}`}>{workflowStatusLabel(x.status,en)}</span></td>{canManage&&<td><div className="record-inline-actions"><button type="button" className="lo-icon-button lo-icon-button-edit lo-icon-button-sm" onClick={()=>onEdit(x)} title={en?'Edit objective':'Επεξεργασία στόχου'} aria-label={en?'Edit objective':'Επεξεργασία στόχου'}><Pencil size={15}/></button><button type="button" className="lo-icon-button lo-icon-button-danger lo-icon-button-sm" onClick={()=>onDelete(x)} title={en?'Delete objective':'Διαγραφή στόχου'} aria-label={en?'Delete objective':'Διαγραφή στόχου'}><Trash2 size={15}/></button></div></td>}</tr>)}</tbody></table>{!rows.length&&<div className="inline-empty">{en?'No annual-plan objectives yet.':'Δεν υπάρχουν ακόμη στόχοι ετήσιου σχεδίου.'}</div>}</div></section>
}

function Framework({record,canManage,onEdit,en}){
  return <section className="record-section"><Head title={en?'Role & institutional framework':'Ρόλος & θεσμικό πλαίσιο'} action={canManage&&<button type="button" className="lo-icon-button lo-icon-button-edit" onClick={onEdit} title={en?'Edit institutional framework':'Επεξεργασία θεσμικού πλαισίου'} aria-label={en?'Edit institutional framework':'Επεξεργασία θεσμικού πλαισίου'}><Pencil size={16}/></button>}/><div className="details-grid"><div><span>{en?'Decision number':'Αρ. πράξης σύστασης'}</span><strong>{record.decisionNumber||'—'}</strong></div><div><span>{en?'Meeting frequency':'Συχνότητα συνεδριάσεων'}</span><strong>{frequencyLabel(record.meetingFrequency,en)}</strong></div></div><div className="source-truth-note"><strong>{en?'Committee role':'Ρόλος επιτροπής'}</strong><p>{record.committeeRole||'—'}</p></div><div className="source-truth-note"><strong>{en?'Legal basis':'Νομική / θεσμική βάση'}</strong><p>{record.legalBasis||'—'}</p></div><div className="source-truth-note"><strong>{en?'Mandate':'Αρμοδιότητα / σκοπός'}</strong><p>{record.mandate||'—'}</p></div></section>
}

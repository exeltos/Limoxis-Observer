...
export function CommitteeCreatePage(){
 ...
 const set=(k,v)=>setDraft(x=>({...x,[k]:v})),template=ipcCommitteeById(draft.templateId),manual=template.id==='custom',datesValid=!draft.termStart||!draft.termEnd||new Date(draft.termEnd)>=new Date(draft.termStart),valid=draft.name.trim()&&draft.committeeRole.trim()&&draft.mandate.trim()&&draft.termStart&&draft.termEnd&&datesValid
 function chooseTemplate(id){const t=ipcCommitteeById(id);setDraft(x=>({...x,templateId:id,name:t.id==='custom'?'':t.name,shortName:t.id==='custom'?'':t.code,committeeRole:t.id==='custom'?'':t.role,mandate:t.id==='custom'?'':t.duties.join('\n'),legalBasis:t.id==='custom'?'':t.source}))}
 async function save(){
 ...
 return <Page><EntityRecordShell className="committee-create-shell" avatar={<BookOpenCheck size={19}/>} eyebrow={en?'Committees':'Επιτροπές'} title={en?'New committee / group':'Νέα επιτροπή / ομάδα'} subtitle={en?'Select a predefined committee or enter one manually.':'Επιλέξτε έτοιμη επιτροπή ή καταχωρήστε μία χειροκίνητα.'} tabs={[]} activeTab="" onTabChange={()=>{}} onBack={goBack}>
  <div className="record-section committee-create-form">
   <div className="entry-grid committee-create-grid">
    <label className="committee-type-field"><span>{en?'Committee / group type *':'Τύπος επιτροπής / ομάδας *'}</span><select value={draft.templateId} onChange={e=>chooseTemplate(e.target.value)}>{IPC_COMMITTEE_CATALOG.map(x=><option key={x.id} value={x.id}>{x.code?`${x.code} — `:''}{x.name}</option>)}</select></label>
    ...
    {manual&&<div className="committee-manual-textarea-row"><label><span>{en?'Committee role *':'Ρόλος επιτροπής *'}</span><textarea rows="2" value={draft.committeeRole} onChange={e=>set('committeeRole',e.target.value)}/></label><label><span>{en?'Responsibilities *':'Αρμοδιότητες *'}</span><textarea rows="2" value={draft.mandate} onChange={e=>set('mandate',e.target.value)}/></label></div>}
   ...
   <div className="source-truth-note">{manual?(en?'For a manual committee, complete its role and responsibilities. Members and governance details are added after saving.':'Για χειροκίνητη επιτροπή συμπληρώστε ρόλο και αρμοδιότητες. Τα μέλη και τα στοιχεία διακυβέρνησης προστίθενται μετά την αποθήκευση.'):(en?'The selected template supplies the committee role, responsibilities and institutional basis automatically.':'Η έτοιμη επιτροπή συμπληρώνει αυτόματα ρόλο, αρμοδιότητες και θεσμική βάση.')}</div>

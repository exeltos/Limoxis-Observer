...
export async function updateCommitteeMemberAsync(organizationId,committee,member,patch={}){
  ...
  await appendHistory(organizationId,committee,'Επεξεργασία στοιχείων μέλους',`${data.member_name} — ${data.title}`,{member_id:data.id});return {...next,dbId:data.id,userId:data.user_id||null,committeeTitle:data.title,responsibilities:data.responsibilities||'',memberType:data.member_type,voting:data.has_vote,approvalStatus:data.approval_status}
}
export async function endCommitteeMemberAsync(organizationId,committee,member,reason){
  requireProduction(organizationId,committee,'member.end');if(!member?.dbId)throw new Error('PRODUCTION_COMMITTEE_MEMBER_DB_ID_REQUIRED');const endedAt=new Date().toISOString().slice(0,10)
...
export async function createCommitteeMeetingAsync(organizationId,committee,draft){
  ...
  await appendHistory(organizationId,committee,'Δημιουργία συνεδρίασης',draft.title,{meeting_id:data.id,client_key:clientKey});return {...draft,id:clientKey,dbId:data.id,scheduledAt:data.scheduled_at,status:data.status,meetingType:data.meeting_type,location:data.location||'',topics:Array.isArray(data.agenda)?data.agenda:[],attendanceRecords:[],quorum:null,minutesNo:'',generalNotes:'',approvalState:'not_started'}
}
...
export async function saveCommitteeMeetingAsync(organizationId,committee,meeting,{finalize=false}={}){
  requireProduction(organizationId,committee,finalize?'meeting.finalize':'meeting.save');if(!meeting?.dbId)throw new Error('PRODUCTION_COMMITTEE_MEETING_DB_ID_REQUIRED:meeting.save')
  await syncAttendance(organizationId,committee,meeting,meeting.attendanceRecords||[])
  const editableStatus=['draft','planned','in_progress'].includes(meeting.status)?meeting.status:'draft'
  const payload={title:meeting.title,scheduled_at:scheduledAt(meeting),meeting_type:meeting.meetingType==='extraordinary'?'extraordinary':'regular',location:meeting.location||null,minutes_number:meeting.minutesNo||null,quorum_met:meeting.quorum??null,agenda:Array.isArray(meeting.topics)?meeting.topics:[],minutes:meeting.generalNotes||meeting.notes||null,status:editableStatus,updated_at:new Date().toISOString()}
  const {data:saved,error:saveError}=await supabase.from('committee_meetings').update(payload).eq('organization_id',organizationId).eq('committee_id',committee.dbId).eq('id',meeting.dbId).select('id,client_key,title,scheduled_at,meeting_type,location,status,minutes_number,quorum_met,agenda,minutes,finalized_at,finalized_by,created_at,updated_at').single();if(saveError)throw saveError
  if(!finalize){await appendHistory(organizationId,committee,'Ενημέρωση συνεδρίασης',meeting.title,{meeting_id:meeting.dbId,status:saved.status});return {...meeting,dbId:saved.id,scheduledAt:saved.scheduled_at,meetingType:saved.meeting_type,location:saved.location||'',status:saved.status,minutesNo:saved.minutes_number||'',quorum:saved.quorum_met,topics:Array.isArray(saved.agenda)?saved.agenda:[],generalNotes:saved.minutes||'',finalizedAt:saved.finalized_at||null}}
  const {data:submission,error:submissionError}=await supabase.rpc('submit_committee_minutes_for_approval',{p_meeting_id:meeting.dbId});if(submissionError)throw submissionError
  void supabase.functions.invoke('process-notification-outbox',{body:{organizationId}}).catch(()=>{})
  const approvalCount=Number(submission?.approvalCount||0),status=submission?.status||'approval_pending'
  return {...meeting,dbId:saved.id,scheduledAt:saved.scheduled_at,meetingType:saved.meeting_type,location:saved.location||'',status,minutesNo:saved.minutes_number||'',quorum:saved.quorum_met,topics:Array.isArray(saved.agenda)?saved.agenda:[],generalNotes:saved.minutes||'',finalizedAt:submission?.finalizedAt||null,approvalState:approvalCount?'pending':'not_required',approvals:[]}
}
export async function answerCommitteeMinutesApprovalAsync(approvalId,status,comment=''){
  if(isDemoDataEnvironment())throw new Error('DEMO_COMMITTEE_APPROVAL_LOCAL_ONLY');if(!hasSupabaseConfig||!supabase)throw new Error('PRODUCTION_COMMITTEES_SUPABASE_REQUIRED:approval.answer');if(!['approved','rejected'].includes(status))throw new Error('COMMITTEE_APPROVAL_STATUS_INVALID')
  const {data,error}=await supabase.from('committee_minutes_approvals').update({status,comment:comment||null,decided_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',approvalId).eq('status','pending').select('id,meeting_id,committee_id,organization_id,status,comment,decided_at').single();if(error)throw error;return data
}
...
export async function createCommitteeDecisionAsync(organizationId,committee,draft){
  ...
  const clientKey=draft.id||`DEC-${Date.now()}`;const {data,error}=await supabase.from('committee_decisions').insert({organization_id:organizationId,committee_id:committee.dbId,meeting_id:meetingDbId,client_key:clientKey,topic_key:draft.topicId||null,title:draft.title,action:draft.action||null,owner_id:draft.ownerId||null,owner_label:draft.owner||null,due_date:draft.dueDate||null,priority:normalizePriority(draft.priority),status:normalizeWorkflowStatus(draft.status)}).select('id,client_key,meeting_id,topic_key,title,action,owner_id,owner_label,due_date,priority,status,created_at,updated_at').single();if(error)throw error
  await appendHistory(organizationId,committee,'Καταχώρηση απόφασης',draft.title,{decision_id:data.id,client_key:clientKey});return {...draft,id:clientKey,dbId:data.id,meetingDbId:data.meeting_id||null,topicId:data.topic_key||draft.topicId||null,ownerId:data.owner_id||null,owner:data.owner_label||'',dueDate:data.due_date||'',priority:data.priority,status:data.status}
}
export async function updateCommitteeDecisionAsync(organizationId,committee,decision,patch={}){
  ...
  const {data,error}=await supabase.from('committee_decisions').update({title:next.title,action:next.action||null,owner_id:next.ownerId||null,owner_label:next.owner||null,due_date:next.dueDate||null,priority:normalizePriority(next.priority),status:normalizeWorkflowStatus(next.status),updated_at:new Date().toISOString()}).eq('organization_id',organizationId).eq('committee_id',committee.dbId).eq('id',decision.dbId).select('id,client_key,meeting_id,topic_key,title,action,owner_id,owner_label,due_date,priority,status,created_at,updated_at').single();if(error)throw error
  await appendHistory(organizationId,committee,'Ενημέρωση απόφασης / ενέργειας',next.title,{decision_id:data.id,status:data.status});return {...next,dbId:data.id,ownerId:data.owner_id||null,owner:data.owner_label||'',dueDate:data.due_date||'',priority:data.priority,status:data.status}
}

...
export async function saveCommitteeMeetingAsync(organizationId,committee,meeting,{finalize=false}={}){
  ...
  const attendance=await syncAttendance(organizationId,committee,meeting,meeting.attendanceRecords||[]);const present=attendance.filter(x=>x.attendance_status==='present');let approvals=[]
  if(finalize)approvals=await requestMinutesApprovals(organizationId,committee,meeting,present)
  const status=finalize?(approvals.length?'approval_pending':'finalized'):(meeting.status==='draft'?'draft':'planned');const payload={title:meeting.title,scheduled_at:scheduledAt(meeting),meeting_type:meeting.meetingType==='extraordinary'?'extraordinary':'regular',location:meeting.location||null,minutes_number:meeting.minutesNo||null,quorum_met:meeting.quorum??null,agenda:Array.isArray(meeting.topics)?meeting.topics:[],minutes:meeting.generalNotes||meeting.notes||null,status,updated_at:new Date().toISOString()};if(finalize&&!approvals.length)payload.finalized_at=new Date().toISOString()
  ...
  await appendHistory(organizationId,committee,finalize?(approvals.length?'Υποβολή πρακτικών για έγκριση':'Οριστικοποίηση πρακτικών'):'Ενημέρωση συνεδρίασης',meeting.title,{meeting_id:meeting.dbId,status:data.status,approval_count:approvals.length});return {...meeting,dbId:data.id,scheduledAt:data.scheduled_at,meetingType:data.meeting_type,location:data.location||'',status:data.status,minutesNo:data.minutes_number||'',quorum:data.quorum_met,topics:Array.isArray(data.agenda)?data.agenda:[],generalNotes:data.minutes||'',finalizedAt:data.finalized_at||null,approvalState:approvals.length?'pending':(finalize?'not_required':meeting.approvalState),approvals}
}

...
export function IndicatorCreatePage(){
 ...
 const [v,setV]=useState({titleEl:'',titleEn:'',indicatorKey:'',category:'quality',calculationType:'manual',numeratorMetric:'active_surveillance',denominatorMetric:'',multiplier:'1',unit:'',unitEn:'',targetValue:'',direction:'context',sourceAuthority:'Hospital-defined',version:'1.0',effectiveFrom:today()})
 const set=(k,value)=>setV(s=>({...s,[k]:value}))
 const valid=v.titleEl.trim()&&v.effectiveFrom&&(v.calculationType==='manual'||v.numeratorMetric)
 ...
 return <Page fill title={el?'Νέος δείκτης':'New indicator'} subtitle={el?'Δημιουργία οργανωτικού δείκτη με πραγματικά δεδομένα ή χειροκίνητη τιμή.':'Create an organization indicator from live data or a manual value.'} actions={<div className="row-actions"><Button variant="secondary" onClick={()=>navigate('/indicators')}><ArrowLeft size={15}/>{el?'Ακύρωση':'Cancel'}</Button><Button onClick={save} disabled={!valid||saving}><Save size={15}/>{saving?(el?'Αποθήκευση…':'Saving…'):(el?'Αποθήκευση':'Save')}</Button></div>}>
  <div className="workspace-column workspace-fill create-page-form">
   <section className="surface form-page-section"><div className="record-section-header"><div><h3>{el?'Βασικά στοιχεία':'Basic details'}</h3><p>{el?'Ο δείκτης θα ανήκει στο τρέχον νοσοκομείο και θα είναι διαθέσιμος σύμφωνα με τα δικαιώματα ρόλου.':'The indicator belongs to the current hospital and follows role permissions.'}</p></div></div><div className="entry-grid compact">
    <label className="entry-span-2"><span>{el?'Τίτλος *':'Title *'}</span><input value={v.titleEl} onChange={e=>set('titleEl',e.target.value)}/></label>
    <label className="entry-span-2"><span>{el?'Αγγλικός τίτλος':'English title'}</span><input value={v.titleEn} onChange={e=>set('titleEn',e.target.value)}/></label>
   ...
   {v.calculationType==='auto'&&<section className="surface form-page-section"><div className="record-section-header"><div><h3>{el?'Υπολογισμός':'Calculation'}</h3><p>{el?'Επίλεξε μόνο canonical μετρικές που παράγονται ήδη από το Limoxis.':'Choose only canonical metrics already produced by Limoxis.'}</p></div></div><div className="entry-grid compact">
    <label><span>{el?'Αριθμητής / κύρια μετρική *':'Numerator / primary metric *'}</span><select value={v.numeratorMetric} onChange={e=>set('numeratorMetric',e.target.value)}>{INDICATOR_METRICS.map(x=><option key={x} value={x}>{metricLabels[x]?.[el?0:1]||x}</option>)}</select></label>
    <label><span>{el?'Παρονομαστής':'Denominator'}</span><select value={v.denominatorMetric} onChange={e=>set('denominatorMetric',e.target.value)}><option value="">{el?'Χωρίς παρονομαστή':'No denominator'}</option>{INDICATOR_METRICS.map(x=><option key={x} value={x}>{metricLabels[x]?.[el?0:1]||x}</option>)}</select></label>
    <label><span>{el?'Πολλαπλασιαστής':'Multiplier'}</span><input type="number" step="any" value={v.multiplier} onChange={e=>set('multiplier',e.target.value)}/></label>
    <label><span>{el?'Μονάδα':'Unit'}</span><input value={v.unit} onChange={e=>set('unit',e.target.value)} placeholder="%, /1.000, L"/></label>
   </div></section>}
   <section className="surface form-page-section"><div className="record-section-header"><div><h3>{el?'Στόχος & τεκμηρίωση':'Target & governance'}</h3></div></div><div className="entry-grid compact">

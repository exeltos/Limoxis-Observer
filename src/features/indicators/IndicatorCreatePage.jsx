 ...
 return <Page fill><EntityRecordShell className="indicator-create-shell workspace-fill" avatar={<Gauge size={19}/>} eyebrow={el?'Δείκτες':'Indicators'} title={el?'Νέος δείκτης':'New indicator'} subtitle={el?'Δημιουργία οργανωτικού δείκτη':'Create organization indicator'} tabs={[]} activeTab="" onTabChange={()=>{}} onBack={()=>navigate('/indicators')}>
  <div className="record-section indicator-create-form">
   <div className="entry-grid">
    <label><span>{el?'Τίτλος *':'Title *'}</span><input autoFocus value={v.titleEl} onChange={e=>set('titleEl',e.target.value)}/></label>
    <label><span>{el?'Αγγλικός τίτλος':'English title'}</span><input value={v.titleEn} onChange={e=>set('titleEn',e.target.value)}/></label>
    ...
    {v.calculationType==='auto'&&<><label><span>{el?'Αριθμητής / κύρια μετρική *':'Numerator / primary metric *'}</span><select value={v.numeratorMetric} onChange={e=>set('numeratorMetric',e.target.value)}>{INDICATOR_METRICS.map(x=><option key={x} value={x}>{metricLabels[x]?.[el?0:1]||x}</option>)}</select></label><label><span>{el?'Παρονομαστής':'Denominator'}</span><select value={v.denominatorMetric} onChange={e=>set('denominatorMetric',e.target.value)}><option value="">{el?'Χωρίς παρονομαστή':'No denominator'}</option>{INDICATOR_METRICS.map(x=><option key={x} value={x}>{metricLabels[x]?.[el?0:1]||x}</option>)}</select></label><label><span>{el?'Πολλαπλασιαστής':'Multiplier'}</span><input type="number" step="any" value={v.multiplier} onChange={e=>set('multiplier',e.target.value)}/></label><label><span>{el?'Μονάδα':'Unit'}</span><input value={v.unit} onChange={e=>set('unit',e.target.value)} placeholder="%, /1.000, L"/></label></>}

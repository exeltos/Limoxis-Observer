...
export function IndicatorsPage(){
 ...
 function updateManualValue(row,value){
  setManualValues(current=>({...current,[row.id]:value}))
  setRows(current=>current.map(item=>item.id===row.id?calculateCloudDefinition({...item,manualValue:value},{}):item))
 }
 ...
 return <Page fill title={t('indicators')} subtitle={t('indicatorsRecords.operationalSubtitle')} actions={<div className="row-actions"><Button variant="secondary" onClick={calculate} disabled={loading}><RefreshCcw size={15}/>{t('recalculate')}</Button>{canManage&&<Button variant="secondary" onClick={savePeriod} disabled={saving||loading||!savableRows.length}><Save size={15}/>{t('indicatorsRecords.savePeriod')}</Button>}{canManage&&<Button onClick={()=>navigate('/indicators/new')}><Plus size={16}/>{t('indicatorsRecords.newIndicatorTitle')}</Button>}</div>}>
  ...
  <section className="surface registry-workspace workspace-column workspace-fill indicator-registry">
   ...
   <div className="scroll-table"><table className="data-table sticky-table indicator-click-table"><thead><tr><th>{t('indicatorsRecords.tableIndicator')}</th><th>{t('category')}</th><th>{t('indicatorsRecords.tableResult')}</th><th>{t('indicatorsRecords.numeratorDenominatorLabel')}</th><th>{t('indicatorsRecords.sourceVersionLabel')}</th><th>{t('status')}</th><th>{t('indicatorsRecords.approvalLabel')}</th></tr></thead><tbody>{pagedRows.map(r=>{const snapshot=snapshotFor(r),approved=snapshot?.status==='approved';return <tr key={r.definitionId||r.id} tabIndex="0" role="button" onClick={()=>r.definitionId&&navigate(`/indicators/${r.definitionId}`)} onKeyDown={e=>{if((e.key==='Enter'||e.key===' ')&&r.definitionId){e.preventDefault();navigate(`/indicators/${r.definitionId}`)}}}><td><strong>{el?r.titleEl:r.titleEn}</strong><small>{from} – {to}{r.calculation==='manual'?(el?' · χειροκίνητος':' · manual'):''}</small></td><td>{categoryLabel(r.category,t)}</td><td className="indicator-result-cell" onClick={event=>event.stopPropagation()} onKeyDown={event=>event.stopPropagation()}>{r.calculation==='manual'&&canManage?<label className="indicator-manual-value"><span>{el?'Χειροκίνητη τιμή':'Manual value'}</span><input type="number" step="any" inputMode="decimal" value={manualValues[r.id]??''} disabled={approved||saving||loading} onChange={event=>updateManualValue(r,event.target.value)} placeholder="—"/><small>{el?r.unit:(r.unitEn||r.unit)}</small></label>:<><strong>{r.value??'—'}</strong><small>{el?r.unit:(r.unitEn||r.unit)}</small></>}</td><td>{r.calculation==='manual'?(el?'Χειροκίνητη καταχώρηση':'Manual entry'):r.evidence}</td><td>{r.source}<small>{r.version}</small></td><td><span className={`indicator-status ${r.status}`}>{statusText(r.status,t)}</span></td><td onClick={event=>event.stopPropagation()} onKeyDown={event=>event.stopPropagation()}>{(()=>{

...
export const INDICATOR_RATIO_RULES=Object.freeze({
 resistant_active_surveillance:Object.freeze({denominator:'active_surveillance',multiplier:100,unit:'%'}),
 hh_compliant_actions:Object.freeze({denominator:'hh_opportunities',multiplier:100,unit:'%'}),
 bundle_all_or_none_pass:Object.freeze({denominator:'bundle_executions',multiplier:100,unit:'%'}),
 active_staff_with_vaccination:Object.freeze({denominator:'active_staff',multiplier:100,unit:'%'}),
 training_completed:Object.freeze({denominator:'training_assignments',multiplier:100,unit:'%'}),
 mdro_bsi:Object.freeze({denominator:'patient_days',multiplier:1000,unit:'/1.000 νοσηλευτικές ημέρες'}),
 abhr_litres:Object.freeze({denominator:'patient_days',multiplier:1000,unit:'L/1.000 νοσηλευτικές ημέρες'}),
})

export function indicatorMetricRule(numeratorMetric){return INDICATOR_RATIO_RULES[numeratorMetric]||null}
export function allowedIndicatorDenominators(numeratorMetric){const rule=indicatorMetricRule(numeratorMetric);return rule?[rule.denominator]:[]}
export function indicatorMetricCombinationIsValid(numeratorMetric,denominatorMetric=''){
 if(!numeratorMetric||!INDICATOR_METRICS.includes(numeratorMetric))return false
 if(!denominatorMetric)return true
 if(!INDICATOR_METRICS.includes(denominatorMetric))return false
 return allowedIndicatorDenominators(numeratorMetric).includes(denominatorMetric)
}
export function normalizeIndicatorMetricPresentation(item){
 const next={...item}
 if(next.calculationType!=='auto')return next
 const rule=indicatorMetricRule(next.numeratorMetric)
 if(next.denominatorMetric&&rule&&next.denominatorMetric===rule.denominator){
  next.multiplier=rule.multiplier
  next.unit=rule.unit
 }
 return next
}
...
export async function saveIndicatorDefinition(organizationId,item){
 ...
 if(item.calculationType==='auto'){
  if(!INDICATOR_METRICS.includes(item.numeratorMetric))throw new Error('Unsupported numerator metric.')
  ...
  if(!indicatorMetricCombinationIsValid(item.numeratorMetric,item.denominatorMetric||''))throw new Error('Unsupported numerator/denominator combination.')
 }
 const normalized=normalizeIndicatorMetricPresentation(item)

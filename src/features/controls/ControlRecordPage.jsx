...
export function ControlRecordPage(){
 ...
 async function removeDefinition(){
  const ok=await confirm({title:en?'Delete control':'Διαγραφή ελέγχου',message:en?'The control and its schedule will be deleted. Continue?':'Ο έλεγχος θα διαγραφεί μαζί με το πρόγραμμά του. Θέλετε να συνεχίσετε;',confirmLabel:en?'Delete':'Διαγραφή',danger:true})
  if(!ok)return
  try{await deleteControlDefinition(tenant.id,record);notify(en?'Control deleted.':'Ο έλεγχος διαγράφηκε.','success');navigate('/controls')}
  catch(error){notifyError(error,'delete',{operation:'control_definition_delete'})}
 }

...
export function ControlRecordPage(){
 ...
 const [historyPage,setHistoryPage]=useState(1),[historyPageSize,setHistoryPageSize]=useState(15)
 ...
 async function saveExecution(payload){
  try{await completeControlExecution(tenant.id,record,department,payload);setEntryOpen(false);setTab('history');setHistoryPage(1);await reload();notify(en?'Control recorded.':'Ο έλεγχος καταχωρήθηκε.','success')}
 ...
 return <Page fill><EntityRecordShell className="control-record-shell workspace-fill" avatar={<ClipboardCheck size={19}/>} eyebrow={record.id} title={language==='el'?record.title:record.titleEn} subtitle={department} status={<div className="control-status-stack">{hasDraft&&<span className="status-badge temporary">{en?'Draft':'Προσωρινή'}</span>}<span className={`status-badge ${status==='overdue'?'danger':status==='dueSoon'?'warning':'active'}`}>{status==='overdue'?(en?'Overdue':'Εκπρόθεσμος'):status==='dueSoon'?(en?'Due soon':'Πλησιάζει'):(en?'On schedule':'Εντός προγράμματος')}</span></div>} recordNavigation={recordNavigation} tabs={[{id:'details',label:en?'Control details':'Στοιχεία ελέγχου',icon:LockKeyhole},{id:'history',label:en?'Execution history':'Ιστορικό εκτελέσεων',icon:FileClock}]} activeTab={tab} onTabChange={next=>{setTab(next);if(next==='history')setHistoryPage(1)}}>
  {tab==='details'&&<div className="record-section control-details-overview"><div className="record-section-header"><div><span className="eyebrow">{en?'CONTROL DETAILS':'ΣΤΟΙΧΕΙΑ ΕΛΕΓΧΟΥ'}</span><h3>{en?'Basic details':'Βασικά στοιχεία'}</h3></div>{(canModifyDefinition||canRemoveDefinition)&&<div className="record-inline-actions control-definition-actions">{canModifyDefinition&&<button type="button" className="edit" title={en?'Edit control':'Επεξεργασία ελέγχου'} aria-label={en?'Edit control':'Επεξεργασία ελέγχου'} onClick={()=>setEditOpen(true)}><Pencil size={16}/></button>}{canRemoveDefinition&&<button type="button" className="danger" title={canDeleteDraft?(en?'Delete control':'Διαγραφή ελέγχου'):(en?'Archive control':'Αρχειοθέτηση ελέγχου')} aria-label={canDeleteDraft?(en?'Delete control':'Διαγραφή ελέγχου'):(en?'Archive control':'Αρχειοθέτηση ελέγχου')} onClick={removeDefinition}><Trash2 size={16}/></button>}</div>}</div><div className="detail-grid quality-detail-grid"><D l={en?'Category':'Κατηγορία'} v={record.category}/><D l={en?'Department':'Τμήμα'} v={department}/><D l={en?'Frequency':'Συχνότητα'} v={frequencyLabel(record.frequency,language)}/><D l={en?'Times':'Ώρες'} v={record.frequency.times?.join(' · ')||'—'}/><D l={en?'Last control':'Τελευταίος έλεγχος'} v={fmt(assignment?.lastCompletedAt)}/><D l={en?'Next control':'Επόμενος έλεγχος'} v={fmt(assignment?.nextDueAt)}/><D l={en?'Owner':'Υπεύθυνος'} v={record.owner||'—'}/><D l={en?'Source':'Προέλευση'} v={sourceLabel}/><D l={en?'Created by':'Δημιουργήθηκε από'} v={record.createdBy||sourceLabel}/>{record.updatedBy&&<D l={en?'Last changed by':'Τελευταία αλλαγή από'} v={record.updatedBy}/>}</div><div className="quality-description"><span>{en?'Description / instructions':'Περιγραφή / οδηγίες'}</span><p>{record.description||'—'}</p></div>{record.frequency.kind==='daily'&&record.frequency.timesPerDay>1&&<div className="governance-banner"><CalendarClock size={17}/><span>{record.frequency.timesPerDay} {en?'scheduled executions per day':'προγραμματισμένες εκτελέσεις την ημέρα'}: {record.frequency.times.join(', ')}.</span></div>}{canExecute&&<div className="record-edit-footer"><Button onClick={()=>setEntryOpen(true)}><PlayCircle size={15}/>{en?'Record control':'Καταχώρηση ελέγχου'}</Button></div>}</div>}
  {tab==='history'&&<div className="record-section workspace-column workspace-fill"><section className="surface registry-workspace workspace-column workspace-fill"><div className="scroll-table"><table className="data-table sticky-table"><thead><tr><th>{en?'Date / time':'Ημερομηνία / ώρα'}</th><th>{en?'Result':'Αποτέλεσμα'}</th><th>{en?'Recorded by':'Καταχώρησε'}</th><th>{en?'Notes':'Σημειώσεις'}</th><th></th></tr></thead><tbody>{pagedHistory.map(h=><tr key={h.id} className={h.status==='cancelled'?'control-history-cancelled':''}><td><strong>{fmt(h.at)}</strong>{h.editedAt&&<small>{en?'Edited':'Επεξεργάστηκε'} {fmt(h.editedAt)}</small>}{h.status==='cancelled'&&<small>{en?'Voided':'Ακυρώθηκε'} {fmt(h.cancelledAt)}</small>}</td><td>{h.status==='cancelled'?<span className="status-badge danger">{en?'Voided':'Ακυρώθηκε'}</span>:structuredSummary(h)}</td><td><strong>{h.by||'—'}</strong><small>{h.email||''}</small>{h.editedBy&&<small>{en?'Last change':'Τελευταία αλλαγή'}: {h.editedBy}</small>}{h.status==='cancelled'&&<small>{en?'Voided by':'Αναίρεση'}: {h.cancelledBy||'—'}</small>}</td><td>{h.status==='cancelled'?h.cancellationReason:(h.notes||'—')}</td><td className="open-record-cell"><div className="record-inline-actions control-history-actions">{h.structuredData?.rows?.length>0&&<button type="button" className="lo-icon-button lo-icon-button-sm control-history-action control-history-action-view" title={en?'Print entry':'Εκτύπωση καταχώρησης'} aria-label={en?'Print entry':'Εκτύπωση καταχώρησης'} onClick={()=>printControlForm({record,department,execution:h})}><Printer size={14}/></button>}{canEditHistory(h)&&<button type="button" className="lo-icon-button lo-icon-button-sm control-history-action control-history-action-edit" title={en?'Edit entry':'Επεξεργασία καταχώρησης'} aria-label={en?'Edit entry':'Επεξεργασία καταχώρησης'} onClick={()=>setEditExecution(h)}><Pencil size={14}/></button>}{canCancelHistory(h)&&<button type="button" className="lo-icon-button lo-icon-button-sm control-history-action control-history-action-danger" title={en?'Void entry':'Αναίρεση καταχώρησης'} aria-label={en?'Void entry':'Αναίρεση καταχώρησης'} onClick={()=>setCancelExecution(h)}><RotateCcw size={14}/></button>}</div></td></tr>)}</tbody></table>{!historyRows.length&&<div className="registry-empty-state"><strong>{en?'No executions yet':'Δεν υπάρχουν ακόμη καταχωρήσεις'}</strong></div>}</div>{historyRows.length>0&&<RegistryPagination language={language} page={historySafePage} totalPages={historyTotalPages} totalItems={historyRows.length} pageSize={historyPageSize} onPageChange={setHistoryPage} onPageSizeChange={size=>{setHistoryPageSize(size);setHistoryPage(1)}}/>}</section></div>}
 </EntityRecordShell>

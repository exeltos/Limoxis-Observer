...
export function ControlExecutionModal({organizationId,record,department,onClose,onSave,onDraftSaved,initialExecution=null}){
 ...
 const savedDraft=isEditing?null:assignment?.draft||null
 ...
 async function persistDraft(){
  ...
  try{
   ...
   setDraftSaved(true);onDraftSaved?.(draft);return draft
  }catch(error){console.error(error);notify(en?'The draft could not be saved.':'Δεν ήταν δυνατή η προσωρινή αποθήκευση.','error');return null}
 ...
 function printDraft(){printControlForm({record,department,execution:{at:initialExecution?.at||new Date().toISOString(),value,notes,structuredData:response.mode==='list'?{template:response.template,rows}:null,by:actor.name},actorName:actor.name})}
 ...
 return <ObserverDialog width={response.mode==='list'?'wide':'standard'} className="control-execution-card" eyebrow={isEditing?(en?'Edit entry':'Επεξεργασία καταχώρησης'):(en?'Control entry':'Καταχώρηση ελέγχου')} title={en?(record.titleEn||record.title):record.title} subtitle={`${department} · ${frequencyLabel(record.frequency,language)}`} onClose={onClose} footer={<DialogActions onCancel={onClose} onSave={submit} disabled={!confirmed||!valid} saveLabel={isEditing?(en?'Save changes':'Αποθήκευση αλλαγών'):(en?'Confirm & save':'Επιβεβαίωση & αποθήκευση')}/> }>
  ...
  <div className="control-execution-tools">
   {!isEditing&&<><Button variant="quiet" disabled={draftSaving} onClick={persistDraft}><Save size={15}/>{draftSaving?(en?' Saving…':' Αποθήκευση…'):(en?' Save draft':' Αποθήκευση προσωρινά')}</Button>{draftSaved&&<span className="control-temp-save-state">{en?'Draft saved · it will be restored when you reopen the entry':'Αποθηκευμένο προσωρινά · θα επανέλθει όταν ξανανοίξετε την καταχώρηση'}</span>}</>}

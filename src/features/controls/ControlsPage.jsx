...
export function ControlsPage(){
  ...
  return <Page fill title={t('controls')} subtitle={canManage?t('controlsManageSubtitle'):t('controlsAssignedSubtitle')}>
    <section className="surface controls-surface workspace-fill workspace-column">
      <div className="section-toolbar"><div><h2>{canManage?t('activeAssignments'):t('toExecute')}</h2><p>{t('controlsTrackingHint')}</p></div>{canManage&&<button className="button button-primary">+ {t('newControl')}</button>}</div>

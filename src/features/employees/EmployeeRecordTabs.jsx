...
export function EmployeeExposureIncidentsTab({employee,language,fmt,organizationId}){
  ...
  return <section className="record-section employee-secondary-registry">
    ...
    <State {...state} language={language} onRetry={state.reload}/>
    {!state.loading&&!state.error&&(state.data.length?<><div className="scroll-table"><table className="data-table sticky-table record-table-clickable"><thead><tr><th>{en?'Date':'Ημερομηνία'}</th><th>{en?'Exposure type':'Τύπος έκθεσης'}</th><th>{en?'Follow-up':'Παρακολούθηση'}</th><th>{en?'Status':'Κατάσταση'}</th></tr></thead><tbody>{paging.paged.map(row=><tr key={row.id} tabIndex={0} onClick={()=>setSelected(row)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelected(row)}}}><td>{fmt(row.incidentDate)}</td><td><strong>{typeLabel(row.exposureType)}</strong></td><td>{followUpLabel(row.followUpStatus)}</td><td><span className={`status-badge ${row.status==='open'?'temporary':''}`}>{row.status==='open'?(en?'Open':'Ανοικτό'):(en?'Closed':'Κλειστό')}</span></td></tr>)}</tbody></table></div><Pager paging={paging} total={state.data.length} language={language}/></>:<Empty language={language} title={en?'No exposure incidents recorded':'Δεν έχουν καταγραφεί περιστατικά έκθεσης'}/>) }

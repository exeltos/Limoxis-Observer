...
export function EmployeeEvaluationsTab({employee,language,fmt,organizationId,canCreate=false,canHrApprove=false,canAdminApprove=false,selfReadOnly=false}){
  ...
  return <section className="record-section record-secondary-registry">
    ...
    {creating&&<ObserverDialog width="wide" eyebrow={en?'Performance evaluation':'Αξιολόγηση απόδοσης'} title={en?'New employee evaluation':'Νέα αξιολόγηση εργαζομένου'} onClose={()=>setCreating(false)} footer={<div className="dialog-actions"><Button variant="secondary" onClick={()=>setCreating(false)} disabled={saving}>{en?'Cancel':'Ακύρωση'}</Button><Button onClick={create} disabled={saving||!draft.period}>{en?'Save':'Αποθήκευση'}</Button></div>}>
      ...
      <div className="evaluation-criteria evaluation-criteria-edit">{draft.criteria.map((item,index)=><div className="evaluation-criterion" key={item.id}><strong>{item.name}</strong><div className="evaluation-rating">{[1,2,3,4,5].map(n=><button type="button" key={n} className={item.score===n?'is-selected':''} aria-pressed={item.score===n} onClick={()=>setDraft(v=>({...v,criteria:v.criteria.map((x,i)=>i===index?{...x,score:n}:x)}))}>{n}</button>)}</div></div>)}</div>
    ...
    {selected&&<ObserverDialog width="wide" eyebrow={en?'Employee evaluation':'Αξιολόγηση εργαζομένου'} title={en?selected.titleEn:selected.titleEl} subtitle={`${selected.period||''} · ${fmt(selected.date)}`} onClose={()=>setSelected(null)}>
      {selected.source==='training'&&<><div className="source-truth-note"><div><strong>{en?'Source':'Πηγή'}</strong><span>{en?'Training knowledge assessment':'Αξιολόγηση γνώσεων εκπαίδευσης'}</span></div></div>{selected.score!=null&&<div className="evaluation-score-hero"><strong>{selected.score}%</strong><span>{selected.competent===true?(en?'Successful':'Επιτυχής'):selected.competent===false?(en?'Retraining required':'Απαιτείται επανεκπαίδευση'):'—'}</span></div>}<section className="record-section training-review-section"><div className="record-section-header"><div><span className="eyebrow">{en?'Assessment form':'Φόρμα αξιολόγησης'}</span><h3>{en?'Questions & submitted answers':'Ερωτήσεις & υποβληθείσες απαντήσεις'}</h3></div></div><div className="training-answer-list">{(selected.assessmentQuestions||[]).map((question,index)=><article className="training-answer-card" key={question.id||index}><div className="training-answer-question"><span className="training-answer-index">{index+1}</span><strong>{question.text||'—'}</strong></div><div className="training-answer-value">{Array.isArray(selected.assessmentAnswers?.[question.id])?selected.assessmentAnswers[question.id].join(', '):trainingAnswerText(question,selected.assessmentAnswers?.[question.id],en)}</div></article>)}</div></section>{selected.certificate&&<div className="training-review-certificate-row"><Button variant="secondary" onClick={()=>downloadCertificatePdf({certificate:selected.certificate,program:selected.program,participantName:[employee.firstName,employee.lastName].filter(Boolean).join(' ')||employee.id,en})}><Download size={14}/>{en?'Print certificate':'Εκτύπωση πιστοποιητικού'}</Button></div>}</>}
      {selected.source!=='training'&&<div className="source-truth-note"><div><strong>{en?'Supervisor / evaluator':'Προϊστάμενος / αξιολογητής'}</strong><span>{selected.evaluatorName||'—'}</span></div></div>}{selected.overallScore!=null&&<div className="evaluation-score-hero"><strong>{selected.overallScore.toFixed(2)} / 5</strong><span>{statusLabel(selected.status)}</span></div>}

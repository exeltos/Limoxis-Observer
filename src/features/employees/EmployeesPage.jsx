...
export function EmployeesPage(){
  const {t,language}=useLanguage(); const {notify}=useFeedback(); const navigate=useNavigate()
  ...
  const rows=useMemo(()=>employeeRows.filter(x=>`${x.id} ${x.firstName} ${x.firstNameEn} ${x.lastName} ${x.lastNameEn} ${x.email}`.toLowerCase().includes(query.toLowerCase())).filter(x=>department==='all'||(language==='el'?x.department:x.departmentEn)===department).filter(x=>status==='all'||x.employmentStatus===status),[query,department,status,language])
  const displayName=x=>language==='el'?`${x.lastName} ${x.firstName}`:`${x.firstNameEn} ${x.lastNameEn}`
  function action(a){if(a===UI_ACTIONS.CREATE)notify(t('newEmployee'),'info'); else if(a===UI_ACTIONS.PRINT)window.print(); else notify(t('actionCompleted'),'success')}
  return <Page fill title={t('employees')} subtitle={t('employeesRegistrySubtitle')} actions={<RecordActions actions={[UI_ACTIONS.CREATE,UI_ACTIONS.PRINT,UI_ACTIONS.EXPORT]} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.MANAGE_STAFF_ADMIN}} onAction={action}/> }>
    <div className="workspace-summary"><div className="employee-kpis"><Kpi icon={Users} value={employeeRows.filter(x=>x.employmentStatus==='active').length} label={t('activeEmployees')}/><Kpi icon={Users} value={departments.length} label={t('departments')}/></div><div className="governance-banner"><ShieldCheck size={17}/><span>{t('employeeAdminGovernance')}</span></div></div>

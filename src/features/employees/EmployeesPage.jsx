...
export function EmployeesPage(){
  ...
  return <Page fill title={t('employees')} subtitle={t('employeesRecords.employeesRegistrySubtitle')} actions={<RecordActions actions={[UI_ACTIONS.CREATE,UI_ACTIONS.PRINT,UI_ACTIONS.EXPORT]} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.MANAGE_STAFF_ADMIN}} onAction={action}/> }>
    ...
    {createOpen&&<EmployeeCreateDialog rows={employeeRows} actor={actor} onClose={()=>setCreateOpen(false)} onSave={row=>{const next=[row,...employeeRows];setEmployeeRows(next);saveEmployees(next);setCreateOpen(false);notify('Ο εργαζόμενος δημιουργήθηκε.','success');navigate(`/employees/${row.id}`)}}/>}
  </Page>
}

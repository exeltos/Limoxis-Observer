...
import { Activity, BriefcaseBusiness, FileCheck2, FileSignature, GraduationCap, HeartPulse, KeyRound, Pencil, ShieldCheck, Syringe, Trash2, UserRound } from 'lucide-react'
...
function Certificates({employee,t,language,fmt,selfMode,canAdmin,notify,organizationId}){
  ...
  return <div className="record-section employee-secondary-registry"><div className="record-section-header"><SectionTitle t={t} title="employeesRecords.certificatesDocuments"/>{canAdmin&&!selfMode&&<Button onClick={beginNew}>+ {language==='en'?'Add':'Προσθήκη'}</Button>}</div>{loading?<div className="registry-loading-state">…</div>:rows.length?<><div className="scroll-table"><table className="data-table sticky-table"><thead><tr><th>{language==='en'?'Document':'Έγγραφο'}</th><th>{t('issuer')}</th><th>{language==='en'?'Issue date':'Ημερομηνία έκδοσης'}</th><th>{t('validUntil')}</th></tr></thead><tbody>{pagedRows.map(x=><tr key={x.id} className="clickable-row" onClick={()=>openExisting(x)}><td><strong>{x.titleEl||x.titleEn||'—'}</strong>{x.certificateNumber&&<small>{x.certificateNumber}</small>}</td><td>{x.issuer||'—'}</td><td>{fmt(x.issueDate)}</td><td>{fmt(x.validUntil)}</td></tr>)}</tbody></table></div><RegistryPagination language={language} page={safePage} totalPages={totalPages} totalItems={rows.length} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={size=>{setPageSize(size);setPage(1)}}/></>:<RegistryEmpty language={language} title={language==='en'?'No certificates or documents':'Δεν υπάρχουν πιστοποιήσεις ή έγγραφα'} subtitle={language==='en'?'Add a document when one becomes available.':'Προσθέστε ένα έγγραφο όταν υπάρχει διαθέσιμο.'}/>} {open&&<ObserverDialog width="wide" eyebrow={t('employeesRecords.certificatesDocuments')} title={editingId?t('edit'):(language==='en'?'Add document':'Προσθήκη εγγράφου')} onClose={close} footer={<DialogActions onCancel={close} onSave={save} disabled={saving}/>}> <div className="entry-grid"><label className="entry-span-2"><span>{language==='en'?'Title':'Τίτλος'}</span><input value={draft.title} onChange={e=>set('title',e.target.value)}/></label><label><span>{t('issuer')}</span><input value={draft.issuer} onChange={e=>set('issuer',e.target.value)}/></label><ManualDateField label={language==='en'?'Issue date':'Ημερομηνία έκδοσης'} value={draft.issueDate} onChange={v=>set('issueDate',v)}/><ManualDateField label={t('validUntil')} value={draft.validUntil} onChange={v=>set('validUntil',v)}/><div className="entry-span-2"><AttachmentField value={draft.attachments} onChange={v=>set('attachments',v)}/></div></div></ObserverDialog>}</div>
}

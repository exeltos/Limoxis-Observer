...
export function EmployeeRecordPage({selfMode=false}){
  ...
  return <Page fill title={name} subtitle={t('employeesRecords.employeeFullRecordSubtitle')}>
    <EntityRecordShell className={`employee-record-shell workspace-fill${selfReadOnly?' employee-self-readonly':''}`} avatar={`${employee.firstName?.[0]||''}${employee.lastName?.[0]||''}`} eyebrow={employee.id} title={name} subtitle={`${language==='el'?employee.profession:employee.professionEn} · ${language==='el'?employee.department:employee.departmentEn}`} status={<span className={`status-badge ${employee.employmentStatus==='active'?'active':''}`}>{t(employee.employmentStatus)}</span>} recordNavigation={selfMode?null:recordNavigation} headerActions={headerActions} tabs={tabs} activeTab={tab} onTabChange={setTab} onBack={selfMode?()=>navigate('/'):goBack} backLabel={t('back')}>
      ...
      {tab==='details'&&<Details employee={employee} t={t} language={language} fmt={fmt} canAdmin={canAdmin} canManageUsers={canManageUsers} onCreateAccount={()=>!selfReadOnly&&setAccountOpen(true)} deleteEmployee={deleteEmployee} notify={notify}/>} 
      {tab==='occupational'&&<Occupational employee={employee} t={t} fmt={fmt} organizationId={tenant?.id} readOnly={selfReadOnly}/>} {tab==='vaccinations'&&<Vaccinations employee={employee} t={t} fmt={fmt} organizationId={tenant?.id}/>} {tab==='surveillance'&&<EmployeeSurveillance employee={employee} t={t} language={language} fmt={fmt} version={surveillanceVersion} readOnly={selfReadOnly} onNew={()=>!selfReadOnly&&setSurveillanceOpen(true)}/>} {tab==='training'&&<Training employee={employee} t={t} language={language} fmt={fmt} organizationId={tenant?.id}/>} {tab==='evaluations'&&<Evaluations employee={employee} t={t} language={language} fmt={fmt} selfMode={selfReadOnly} organizationId={tenant?.id}/>} {tab==='certificates'&&<Certificates employee={employee} t={t} language={language} selfMode={selfReadOnly} canAdmin={canAdmin} organizationId={tenant?.id}/>} {tab==='history'&&<History employee={employee} t={t} language={language}/>} 
    </EntityRecordShell>
...
function Evaluations({employee,t,language,fmt,selfMode,organizationId}){
  ...
  return <div className="record-section employee-secondary-registry">
    ...
    <div className="source-truth-note">{selfMode?t('employeesRecords.selfEvaluationReadOnly'):t('employeesRecords.evaluationGovernance')}</div>
    {rows.length?<div className="scroll-table"><table className="data-table sticky-table record-table-clickable"><thead><tr><th>{language==='en'?'Evaluation':'Αξιολόγηση'}</th><th>{t('date')}</th><th>{language==='en'?'Result':'Αποτέλεσμα'}</th></tr></thead><tbody>{rows.map(x=><tr key={x.id} tabIndex={0} onClick={()=>setSelected(x)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelected(x)}}}><td><strong>{titleOf(x)||'—'}</strong></td><td>{fmt(x.date)}</td><td>{resultOf(x)||'—'}</td></tr>)}</tbody></table></div>:<RegistryEmpty language={language} title={language==='en'?'No evaluations':'Δεν υπάρχουν αξιολογήσεις'}/>} 
...
function Certificates({employee,t,language,selfMode,canAdmin,organizationId}){
...
function History({employee,t,language}){
  const rows=[]
  if(employee.createdAt)rows.push({id:'created',at:employee.createdAt,event:language==='en'?'Employee record created':'Δημιουργία καρτέλας εργαζομένου'})
  if(employee.updatedAt&&employee.updatedAt!==employee.createdAt)rows.push({id:'updated',at:employee.updatedAt,event:language==='en'?'Employee record updated':'Τελευταία ενημέρωση καρτέλας'})
  if(employee.accountLinked)rows.push({id:'account',at:null,event:language==='en'?'Linked user account':'Συνδεδεμένος λογαριασμός χρήστη'})
  const formatDateTime=value=>{if(!value)return '—';const d=new Date(value);return Number.isNaN(d.getTime())?'—':new Intl.DateTimeFormat(language==='en'?'en-GB':'el-GR',{dateStyle:'medium',timeStyle:'short'}).format(d)}
  return <div className="record-section employee-secondary-registry"><SectionTitle t={t} title="history"/>{rows.length?<div className="scroll-table"><table className="data-table sticky-table"><thead><tr><th>{language==='en'?'Event':'Ενέργεια'}</th><th>{language==='en'?'Date / time':'Ημερομηνία / ώρα'}</th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td><strong>{row.event}</strong></td><td>{formatDateTime(row.at)}</td></tr>)}</tbody></table></div>:<RegistryEmpty language={language} title={language==='en'?'No lifecycle history is available for this record':'Δεν υπάρχουν διαθέσιμες εγγραφές ιστορικού για αυτή την καρτέλα'}/>}</div>
}

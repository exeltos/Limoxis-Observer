...
export function EmployeeRecordPage({selfMode=false}){
  ...
  return <Page fill title={name} subtitle={t('employeesRecords.employeeFullRecordSubtitle')}>
    <EntityRecordShell className="employee-record-shell workspace-fill" avatar={`${employee.firstName?.[0]||''}${employee.lastName?.[0]||''}`} eyebrow={employee.id} title={name} subtitle={`${language==='el'?employee.profession:employee.professionEn} · ${language==='el'?employee.department:employee.departmentEn}`} status={<span className={`status-badge ${employee.employmentStatus==='active'?'active':''}`}>{t(employee.employmentStatus)}</span>} recordNavigation={selfMode?null:recordNavigation} headerActions={headerActions} tabs={tabs} activeTab={tab} onTabChange={setTab} onBack={selfMode?()=>navigate('/'):goBack} backLabel={t('back')}>
      <div className="employee-record-facts"><RecordFact label={t('department')} value={language==='el'?employee.department:employee.departmentEn}/><RecordFact label={t('professionalCategory')} value={language==='el'?employee.profession:employee.professionEn}/><RecordFact label={t('employeesRecords.hireDate')} value={fmt(employee.hireDate)}/><RecordFact label={t('status')} value={t(employee.employmentStatus)} kind={employee.employmentStatus==='active'?'active':''}/></div>
...
function Certificates({employee,t,language,fmt,selfMode,canAdmin,notify,organizationId}){const emptyDraft={titleEl:'',titleEn:'',issuer:'',issueDate:'',validUntil:'',certificateNumber:'',attachments:[]};const cloud=certificatesCloudEnabled(employee.dbId);const [allRows,setAllRows]=useState(cloud?[]:loadCertificates),[loading,setLoading]=useState(cloud);useEffect(()=>{if(!cloud)return;let cancelled=false;setLoading(true);loadCertificatesAsync(organizationId,employee.dbId,employee.id).then(rows=>{if(!cancelled)setAllRows(rows)}).catch(()=>{}).finally(()=>{if(!cancelled)setLoading(false)});return ()=>{cancelled=true}},[cloud,organizationId,employee.dbId,employee.id]);const rows=cloud?allRows:allRows.filter(x=>x.employeeId===employee.id);const [open,setOpen]=useState(false),[editingId,setEditingId]=useState(null),[draft,setDraft]=useState(emptyDraft),[saving,setSaving]=useState(false);const set=(k,v)=>setDraft(d=>({...d,[k]:v})),beginNew=()=>{setEditingId(null);setDraft(emptyDraft);setOpen(true)},openExisting=row=>{setEditingId(row.id);setDraft({...emptyDraft,...row,attachments:row.attachments||[]});setOpen(true)},close=()=>{setOpen(false);setEditingId(null);setDraft(emptyDraft)};async function save(){if(saving)return;setSaving(true);try{let attachments=draft.attachments||[];if(cloud){attachments=await Promise.all(attachments.map(async a=>a?.file?uploadAttachment(a.file,{organizationId,scope:'employee-certificates'}):a));const payload={...draft,attachments};if(editingId){const updated=await updateCertificateAsync(organizationId,employee.dbId,editingId,payload);setAllRows(rows=>rows.map(r=>r.id===editingId?updated:r))}else{const created=await createCertificateAsync(organizationId,employee.dbId,payload);setAllRows(rows=>[created,...rows])}}else{const payload={...draft,attachments};const next=editingId?allRows.map(r=>r.id===editingId?{...r,...payload}:r):[{...payload,id:`CERT-${Date.now()}`,employeeId:employee.id},...allRows];setAllRows(next);saveCertificatesLocalFallback(next)}notify(t('employeesRecords.certificateSaved'),'success');close()}catch(error){notify(error?.message||t('employeesRecords.certificateSaveFailed'),'error')}finally{setSaving(false)}}return <div className="record-section"><div className="record-section-header"><SectionTitle t={t} title="employeesRecords.certificatesDocuments"/>{canAdmin&&!selfMode&&<Button onClick={beginNew}>+ {t('add')}</Button>}</div>{loading?<div className="inline-empty">…</div>:<div className="record-card-list">{rows.length?rows.map(x=><article key={x.id} className="record-subcard clickable" onClick={()=>openExisting(x)}><strong>{language==='el'?x.titleEl:x.titleEn}</strong><span>{x.issuer}</span><small>{fmt(x.issueDate)} · {t('validUntil')}: {fmt(x.validUntil)}</small></article>):<Empty t={t}/>}</div>}{open&&<ObserverDialog width="wide" eyebrow={t('employeesRecords.certificatesDocuments')} title={editingId?t('edit'):t('add')} onClose={close} footer={<DialogActions onCancel={close} onSave={save} disabled={saving}/>}> <div className="entry-grid"><label><span>{t('title')} EL</span><input value={draft.titleEl} onChange={e=>set('titleEl',e.target.value)}/></label><label><span>{t('title')} EN</span><input value={draft.titleEn} onChange={e=>set('titleEn',e.target.value)}/></label><label><span>{t('issuer')}</span><input value={draft.issuer} onChange={e=>set('issuer',e.target.value)}/></label><ManualDateField label={t('date')} value={draft.issueDate} onChange={v=>set('issueDate',v)}/><ManualDateField label={t('validUntil')} value={draft.validUntil} onChange={v=>set('validUntil',v)}/><AttachmentField value={draft.attachments} onChange={v=>set('attachments',v)}/></div></ObserverDialog>}</div>}
...
function History({t}){return <div className="record-section"><SectionTitle t={t} title="history"/><Empty t={t}/></div>}
function SectionTitle({t,title}){return <div><span className="eyebrow">{t(title)}</span><h3>{t(title)}</h3></div>}

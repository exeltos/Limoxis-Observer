...
export function EmployeeCertificatesTab({employee,language,organizationId,canEdit=false}){
  ...
  const load=async()=>{setLoading(true);setError('');try{setRows(await loadCertificatesAsync(organizationId,employee.dbId,employee.id))}catch(err){setError(err?.message||'LOAD_FAILED')}finally{setLoading(false)}}
  ...
  async function save(){if(saving||!draft.titleEl.trim())return;setSaving(true);try{if(selected?.id)await updateCertificateAsync(organizationId,employee.dbId,selected.id,draft);else await createCertificateAsync(organizationId,employee.dbId,draft);await load();close();notify(language==='en'?'Certification saved.':'Η πιστοποίηση αποθηκεύτηκε.','success')}catch(err){notify(err?.message||(language==='en'?'Could not save certification.':'Δεν ήταν δυνατή η αποθήκευση της πιστοποίησης.'),'error')}finally{setSaving(false)}}
  async function remove(){if(!selected?.id||!canEdit)return;const ok=await confirm({title:language==='en'?'Delete certification':'Διαγραφή πιστοποίησης',message:language==='en'?'The certification record will be deleted.':'Η εγγραφή της πιστοποίησης θα διαγραφεί.',confirmLabel:language==='en'?'Delete':'Διαγραφή',danger:true});if(!ok)return;try{await deleteCertificateAsync(organizationId,employee.dbId,selected.id);await load();close();notify(language==='en'?'Certification deleted.':'Η πιστοποίηση διαγράφηκε.','success')}catch(err){notify(err?.message||(language==='en'?'Could not delete certification.':'Δεν ήταν δυνατή η διαγραφή της πιστοποίησης.'),'error')}}
  return <section className="record-section employee-secondary-registry">
    ...
    {editing&&<ObserverDialog width="wide" eyebrow={language==='en'?'Employee certification':'Πιστοποίηση εργαζομένου'} title={selected?titleOf(selected):(language==='en'?'New certification':'Νέα πιστοποίηση')} onClose={close} footer={<div className="dialog-actions">{selected&&canEdit&&<Button variant="danger" onClick={remove}><Trash2 size={15}/>{language==='en'?'Delete':'Διαγραφή'}</Button>}<Button variant="secondary" disabled={saving} onClick={close}>{language==='en'?'Cancel':'Ακύρωση'}</Button>{canEdit&&<SaveButton loading={saving} disabled={saving||!draft.titleEl.trim()} onClick={save}>{language==='en'?'Save':'Αποθήκευση'}</SaveButton>}</div>}>
      ...
      {selected?.id&&<section className="documents-workspace"><header className="documents-workspace-header"><h3>{language==='en'?'Attachments':'Συνημμένα'}</h3><p>{language==='en'?'Files supporting this certification.':'Αρχεία που τεκμηριώνουν τη συγκεκριμένη πιστοποίηση.'}</p></header><AttachmentField disabled={!canEdit} organizationId={organizationId} entityType="employee-certificate" entityId={selected.id}/></section>}
    </ObserverDialog>}
  </section>
}

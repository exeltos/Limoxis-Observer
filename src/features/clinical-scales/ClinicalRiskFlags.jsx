...
const tone=risk=>['high','severe','critical'].includes(risk)?'danger':['medium','moderate'].includes(risk)?'warning':'active'
export function clinicalRiskFlags(rows=[]){const latest=new Map();for(const row of rows){const k=kind(row);if(!latest.has(k))latest.set(k,row)}return [...latest].map(([k,row])=>({kind:k,row,config:CONFIG[k],tone:tone(row.interpretation)}))}
export function ClinicalRiskFlags({rows=[],language='el',compact=false,onSelect}){const flags=clinicalRiskFlags(rows);if(!flags.length)return null;return <div className={'clinical-risk-flags'+(compact?' compact':'')} aria-label={language==='en'?'Clinical alerts':'Κλινικές επισημάνσεις'}>{flags.map(({kind,row,config,tone})=>{const Icon=config.Icon;const label=language==='en'?config.labelEn:config.labelEl;const title=`${label} · ${row.scale_key?.toUpperCase()} ${row.score??'—'}`;return <button key={kind} type="button" className={`clinical-risk-flag ${tone}`} title={title} aria-label={title} onClick={onSelect?()=>onSelect(row):undefined}><Icon size={compact?15:16}/>{!compact&&<><span>{label}</span><strong>{row.score??'—'}</strong></>}</button>})}</div>}

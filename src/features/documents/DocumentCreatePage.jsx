...
export function DocumentCreatePage(){
 const navigate=useNavigate();const {tenant,isDemo}=useTenant();const {language}=useLanguage();const en=language==='en';const {notify}=useFeedback();const actor=useAuditActor()
 ...
 // loadDocumentsAsync/createDocumentAsync already branch on isDemoDataEnvironment()
 // internally, but loadDepartments (departmentsService.js) is a plain cloud
 // call with no demo awareness — calling it with tenant.id='demo-hospital'
 // (not a real UUID) fails with a Postgres 400. Use demoLibrarySeed instead,
 // the same fixture used for departments elsewhere in demo mode.
 useEffect(()=>{let active=true;const departmentsRequest=isDemo?Promise.resolve(demoLibrarySeed.departments.map(([elName,enName])=>({id:elName,name:elName,nameEn:enName}))):(tenant?.id?loadDepartments(tenant.id):Promise.resolve([]));if(!tenant?.id&&!isDemo)return;Promise.all([departmentsRequest,loadDocumentsAsync(tenant?.id)]).then(([departmentRows,documentRows])=>{if(!active)return;setDepartments((departmentRows||[]).filter(x=>x.is_active!==false));setExisting(documentRows||[])}).catch(()=>{if(active){setDepartments([]);setExisting([])}});return()=>{active=false}},[isDemo,tenant?.id])

...
export function DocumentRecordPage(){
 ...
 async function publish(){const ok=await confirm({title:en?'Publish document':'Δημοσίευση εγγράφου',message:en?'The current version will be marked as published and available for use. Continue?':'Η τρέχουσα έκδοση θα χαρακτηριστεί ως δημοσιευμένη και διαθέσιμη για χρήση. Θέλετε να συνεχίσετε;',confirmLabel:en?'Publish':'Δημοσίευση'});if(!ok)return;persist(audit({...record,status:'published',publishedAt:new Date().toISOString(),publishedBy:actor.name},'Δημοσίευση εγγράφου',`Έκδοση ${record.version}`));notify(en?'Document published.':'Το έγγραφο δημοσιεύτηκε.','success')}
 async function archive(){const ok=await confirm({title:en?'Archive document':'Αρχειοθέτηση εγγράφου',message:en?'The document will no longer be active for current use. History will be preserved.':'Το έγγραφο θα πάψει να θεωρείται ενεργό για τρέχουσα χρήση. Το ιστορικό θα διατηρηθεί.',confirmLabel:en?'Archive':'Αρχειοθέτηση',danger:true});if(!ok)return;persist(audit({...record,status:'archived'},'Αρχειοθέτηση εγγράφου',`Έκδοση ${record.version}`));notify(en?'Document archived.':'Το έγγραφο αρχειοθετήθηκε.','success')}
 ...
 return <Page fill><EntityRecordShell avatar={<BookOpenCheck size={19}/>} eyebrow={record.id} title={record.title} subtitle={`${typeLabels[record.type]||record.type} · {en?'Version':'Έκδοση'} ${record.version||'—'}`} status={<span className={`status-badge ${record.status==='published'?'active':record.status==='draft'?'temporary':''}`}>{statusLabels[record.status]||record.status}</span>} onBack={()=>navigate('/documents')} headerActions={<>{canManage&&<button className="general-edit-button" onClick={()=>setEditOpen(true)}><Pencil size={15}/>{en?' Edit':' Επεξεργασία'}</button>}{canManage&&record.status==='draft'&&<Button onClick={publish}>{en?'Publish':'Δημοσίευση'}</Button>}{canManage&&record.status==='published'&&<button className="entity-record-icon-button danger" onClick={archive} title={en?'Archive':'Αρχειοθέτηση'} aria-label={en?'Archive':'Αρχειοθέτηση'}><Archive size={15}/></button>}<PrintExportActions onExport={()=>downloadRecordJson(record,{filename:record.id})}/></>} tabs={tabs} activeTab={tab} onTabChange={setTab}>
  ...
  {tab==='files'&&<section className="record-section"><div className="record-section-header"><div><span className="eyebrow">{en?'Documents':'Έγγραφα'}</span><h3>{en?'Files & attachments':'Αρχεία & συνημμένα'}</h3><p>{en?'Preview opens the file itself. Every addition or change is recorded in history.':'Η προβολή ανοίγει το ίδιο το αρχείο. Κάθε προσθήκη/μεταβολή καταγράφεται στο ιστορικό.'}</p></div></div><AttachmentField disabled={!canManage||record.status==='archived'} value={record.attachments||[]} onChange={attachments}/></section>}

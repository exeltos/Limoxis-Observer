...
export function DocumentRecordPage(){
 ...
 const [tab,setTab]=useState('overview'),[editOpen,setEditOpen]=useState(false),[busy,setBusy]=useState(false)
 ...
 async function run(operation,successMessage){
  ...
  try{
    ...
    notify(en?'The action could not be completed.':'Η ενέργεια δεν ήταν δυνατό να ολοκληρωθεί.','danger')
    return null
  }finally{setBusy(false)}
 }

 async function saveEdit(data){
  const result=await run(()=>updateDocumentAsync(organizationId,record,data,actor),en?'Document updated.':'Το έγγραφο ενημερώθηκε.')
 ...
 async function publish(){
  ...
  const ok=await confirm({title:en?'Publish document':'Δημοσίευση εγγράφου',message:en?'The approved version will become active for use. Continue?':'Η εγκεκριμένη έκδοση θα γίνει ενεργή για χρήση. Θέλετε να συνεχίσετε;',confirmLabel:en?'Publish':'Δημοσίευση'})
 ...
 async function archive(){
  ...
  const ok=await confirm({title:en?'Archive document':'Αρχειοθέτηση εγγράφου',message:en?'The document will no longer be active for current use. History will be preserved.':'Το έγγραφο θα πάψει να θεωρείται ενεργό για τρέχουσα χρήση. Το ιστορικό θα διατηρηθεί.',confirmLabel:en?'Archive':'Αρχειοθέτηση',danger:true})
 ...
 async function createRevision(){
  ...
  const ok=await confirm({title:en?'Create new revision':'Νέα αναθεώρηση',message:en?'A new draft version will be created. The published version stays active until the revision is published.':'Θα δημιουργηθεί νέα πρόχειρη έκδοση. Η δημοσιευμένη έκδοση παραμένει ενεργή μέχρι να δημοσιευτεί η αναθεώρηση.',confirmLabel:en?'Create revision':'Δημιουργία αναθεώρησης'})
  if(!ok)return
  const next=await run(()=>createDocumentRevisionAsync(organizationId,record,actor,rows),en?'New draft revision created.':'Δημιουργήθηκε νέα πρόχειρη αναθεώρηση.')
 ...
 </EntityRecordShell>{editOpen&&<DocumentEditDialog record={record} language={language} busy={busy} onClose={()=>setEditOpen(false)} onSave={saveEdit}/>}</Page>
}

function DocumentEditDialog({record,onClose,onSave,language,busy}){
 const en=language==='en',typeLabels=labels[language].types
 const [v,setV]=useState({title:record.title||'',type:record.type||'policy',version:record.version||'0.1',audience:record.audience||'organization',effectiveDate:record.effectiveDate||'',reviewDate:record.reviewDate||'',description:record.description||''})
 const set=(k,x)=>setV(s=>({...s,[k]:x}))
 const valid=v.title.trim()&&v.version.trim()
 return <ObserverDialog width="wide" eyebrow={en?'Documents':'Έγγραφα'} title={en?'Edit draft':'Επεξεργασία πρόχειρου'} subtitle={en?'Only draft documents can be edited.':'Μόνο τα πρόχειρα έγγραφα μπορούν να επεξεργαστούν.'} onClose={onClose} footer={<DialogActions onCancel={onClose} disabled={!valid||busy} onSave={()=>onSave(v)} saveLabel={busy?(en?'Saving…':'Αποθήκευση…'):(en?'Save':'Αποθήκευση')}/>}><div className="observer-form-section"><div className="entry-grid compact"><label className="entry-span-2"><span>{en?'Title *':'Τίτλος *'}</span><input value={v.title} onChange={e=>set('title',e.target.value)}/></label><label><span>{en?'Type':'Τύπος'}</span><select value={v.type} onChange={e=>set('type',e.target.value)}>{Object.entries(typeLabels).map(([k,l])=><option key={k} value={k}>{l}</option>)}</select></label><label><span>{en?'Version':'Έκδοση'}</span><input value={v.version} onChange={e=>set('version',e.target.value)}/></label><ManualDateField label={en?'Effective date':'Ημερομηνία ισχύος'} value={v.effectiveDate} onChange={x=>set('effectiveDate',x)} optional/><ManualDateField label={en?'Review date':'Ημερομηνία επανεξέτασης'} value={v.reviewDate} onChange={x=>set('reviewDate',x)} optional/><label className="entry-span-2"><span>{en?'Description':'Περιγραφή'}</span><textarea rows="4" value={v.description} onChange={e=>set('description',e.target.value)}/></label></div></div></ObserverDialog>
}

...
export function DocumentsPage(){
 ...
 const {data:rows,setData:setRows,loading,error,reload,organizationId}=useDocumentsData()
 const [query,setQuery]=useState(''),[status,setStatus]=useState('all'),[type,setType]=useState('all'),[createOpen,setCreateOpen]=useState(false),[saving,setSaving]=useState(false)
 ...
 async function create(data){
  ...
  setSaving(true)
  try{
   const record=await createDocumentAsync(organizationId,data,actor,rows)
   setRows(current=>[record,...current])
   setCreateOpen(false)
   notify(en?'Document created.':'Το έγγραφο δημιουργήθηκε.','success')
   navigate(`/documents/${record.id}`)
  }catch{
   notify(en?'The document could not be saved. No local fallback was used.':'Το έγγραφο δεν αποθηκεύτηκε. Δεν χρησιμοποιήθηκε τοπική εναλλακτική αποθήκευση.','danger')
 ...
 return <Page fill title={en?'Documents':'Έγγραφα'} subtitle={en?'Central library of controlled documents, versions and distributions.':'Κεντρική βιβλιοθήκη ελεγχόμενων εγγράφων, εκδόσεων και κοινοποιήσεων.'} actions={<RecordActions actions={[...(canManage?[UI_ACTIONS.CREATE]:[]),UI_ACTIONS.PRINT,UI_ACTIONS.EXPORT]} resourceCapability={CAPABILITIES.VIEW_DOCUMENTS} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.MANAGE_DOCUMENTS,[UI_ACTIONS.PRINT]:CAPABILITIES.PRINT_RECORDS,[UI_ACTIONS.EXPORT]:CAPABILITIES.EXPORT_RECORDS}} onAction={action}/>}>
  ...
  {createOpen&&<DocumentCreateDialog language={language} saving={saving} onClose={()=>setCreateOpen(false)} onSave={create}/>}
 </Page>
}

function Metric({icon:Icon,label,value}){return <MetricCard icon={Icon} value={value} label={label}/>}

function DocumentCreateDialog({onClose,onSave,language,saving}){
 ...
 return <ObserverDialog width="wide" eyebrow={en?'Documents':'Έγγραφα'} title={en?'New document':'Νέο έγγραφο'} subtitle={en?'Basic details, version and initial file.':'Βασικά στοιχεία, έκδοση και αρχικό αρχείο.'} onClose={onClose} footer={<DialogActions onCancel={onClose} disabled={!valid||saving} onSave={()=>onSave(v)} saveLabel={saving?(en?'Saving…':'Αποθήκευση…'):(en?'Save':'Αποθήκευση')}/>}>

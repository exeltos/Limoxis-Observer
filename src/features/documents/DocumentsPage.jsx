...
export function DocumentsPage(){
 const navigate=useNavigate(),registry=useRegistryMemory('documents'),{notify}=useFeedback(),{role,membership}=useTenant(),{language}=useLanguage();const en=language==='en',typeLabels=labels[language].types,statusLabels=labels[language].statuses
 ...
 function action(a){if(a===UI_ACTIONS.CREATE){navigate('/documents/new');return}if(a===UI_ACTIONS.PRINT){window.print();return}if(a===UI_ACTIONS.EXPORT){exportCsv();notify(en?'Current list exported.':'Η τρέχουσα λίστα εξήχθη.','success')}}

...
import { groupDocumentFamilies } from './documentService'
...
export function DocumentsPage(){
 const navigate=useNavigate(),registry=useRegistryMemory('documents'),{role,actualRole,membership}=useTenant(),{language}=useLanguage();const en=language==='en',typeLabels=labels[language].types,statusLabels=labels[language].statuses
 ...
 const addOns=membership?.capabilities??[],custom=membership?.customCapabilities??[],permissionRole=role||actualRole;const canManage=can(permissionRole,CAPABILITIES.MANAGE_DOCUMENTS,addOns,custom)
 const families=useMemo(()=>groupDocumentFamilies(rows),[rows])
 const filtered=useMemo(()=>families.filter(family=>{const x=family.current,searchText=`${family.root?.id||''} ${family.versions.map(v=>v.id).join(' ')} ${x?.title||''} ${x?.owner||''} ${x?.department||''}`.toLowerCase();return (status==='all'||x?.status===status)&&(type==='all'||x?.type===type)&&searchText.includes(query.toLowerCase())}),[families,query,status,type])
 ...
 const reviewDue=families.filter(family=>{const x=family.activePublished;return x?.reviewDate&&x.reviewDate<=new Date(Date.now()+30*86400000).toISOString().slice(0,10)}).length

...
import { AlertTriangle, CheckCircle2, Clock3, ExternalLink, History, RefreshCcw, ShieldAlert, XCircle } from 'lucide-react'
...
const DAY=86400000
const copy={
 el:{title:'Κέντρο ενημερώσεων πλατφόρμας',subtitle:'Ελεγχόμενες κλινικές πηγές, αλλαγές που περιμένουν έλεγχο και ιστορικό ελέγχων.',pending:'Προς έλεγχο',lastCheck:'Τελευταίος έλεγχος',sources:'Πηγές ελέγχθηκαν',failures:'Αποτυχίες 30 ημερών',never:'Ποτέ',runNow:'Έλεγχος τώρα',running:'Έλεγχος σε εξέλιξη…',changesTitle:'Αλλαγές προς έλεγχο',changesHint:'Η πηγή άλλαξε από τον προηγούμενο έλεγχο. Ελέγξτε αν επηρεάζει κριτήρια, δείκτες ή κλίμακες πριν τη σημειώσετε ως ελεγμένη.',noChanges:'Όλες οι πηγές είναι ενημερωμένες',noChangesText:'Δεν υπάρχει αλλαγή που να περιμένει έλεγχο.',detected:'Εντοπίστηκε',openSource:'Άνοιγμα πηγής',deferredBadge:'Σε αναβολή',defer:'Αναβολή 7 ημερών',reviewed:'Ελέγχθηκε',historyTitle:'Ιστορικό ελέγχων',manual:'Χειροκίνητος',automatic:'Αυτόματος',sourcesN:n=>`${n} ${n===1?'πηγή':'πηγές'}`,changesN:n=>`${n} ${n===1?'αλλαγή':'αλλαγές'}`,completed:'Ολοκληρώθηκε',failed:'Απέτυχε',runningStatus:'Σε εξέλιξη',showAll:n=>`Προβολή όλων (${n})`,showLess:'Λιγότερα',noHistory:'Δεν έχει γίνει ακόμη έλεγχος.',seconds:'δευτ.',failedNote:'Ο έλεγχος διακόπηκε πριν ολοκληρωθεί.',ago:{now:'μόλις τώρα',min:n=>`πριν ${n} λεπ.`,hour:n=>`πριν ${n} ώρ.`,day:n=>`πριν ${n} ${n===1?'ημέρα':'ημέρες'}`}},
 en:{title:'Platform update center',subtitle:'Governed clinical sources, changes awaiting review and check history.',pending:'To review',lastCheck:'Last check',sources:'Sources checked',failures:'Failures, 30 days',never:'Never',runNow:'Check now',running:'Checking…',changesTitle:'Changes to review',changesHint:'The source changed since the previous check. Review whether it affects criteria, indicators or scales before marking it as reviewed.',noChanges:'All sources are up to date',noChangesText:'No change is waiting for review.',detected:'Detected',openSource:'Open source',deferredBadge:'Postponed',defer:'Postpone 7 days',reviewed:'Reviewed',historyTitle:'Check history',manual:'Manual',automatic:'Automatic',sourcesN:n=>`${n} ${n===1?'source':'sources'}`,changesN:n=>`${n} ${n===1?'change':'changes'}`,completed:'Completed',failed:'Failed',runningStatus:'Running',showAll:n=>`Show all (${n})`,showLess:'Show less',noHistory:'No check has run yet.',seconds:'s',failedNote:'The check stopped before completing.',ago:{now:'just now',min:n=>`${n} min ago`,hour:n=>`${n} h ago`,day:n=>`${n} ${n===1?'day':'days'} ago`}},
}
...
export function PlatformUpdateCenter({language='el',pending=[],history=[],reviewingId=null,onReview,onRun,running=null,onClose}){
 ...
 return <ObserverDialog className="platform-update-center-dialog" width="wide" title={c.title} subtitle={c.subtitle} onClose={onClose}>
  <div className="puc">
   ...
   <section className="puc-section">
    ...
    {pending.length?<div className="puc-changes">{pending.map(item=>{const source=item.source||{};const deferred=item.review_status==='deferred';return <article key={item.id} className={`puc-change${deferred?' is-deferred':''}`}>
     <div className="puc-change-main">
      ...
      {item.note&&<p className="puc-note">{item.note}</p>}
   ...
   <section className="puc-section">
    ...
    {history.length?<ol className="puc-history">{visibleHistory.map(run=>{const [tone,label,Icon]=runStatus(run);const seconds=duration(run);const error=run.summary?.error;return <li key={run.id} className={`puc-run-row is-${tone}`}>
     ...
     <div className="puc-run-counts"><span>{c.sourcesN(run.checked_count||0)}</span><span className={run.changed_count?'has-changes':''}>{c.changesN(run.changed_count||0)}</span></div>

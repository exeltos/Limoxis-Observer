...
import { Activity,Bell,Building2,Database,Globe2,KeyRound,Layers3,Pencil,ShieldCheck,X,ClipboardList,Wind,Users } from 'lucide-react'
...
export function ManagementPage(){
 ...
 const tabs=useMemo(()=>{const ok=cap=>can(role,cap,addOns,customCaps);return [
  ...
  ...(ok(CAPABILITIES.MANAGE_INDICATORS)?[{id:'indicators',label:t('indicators'),icon:Activity}]:[]),...(ok(CAPABILITIES.MANAGE_BED_DAYS)?[{id:'patientDays',label:t('patientDays'),icon:Database}]:[]),...(ok(CAPABILITIES.MANAGE_HOSPITAL_STRUCTURE)?[{id:'hospitalStructure',label:language==='en'?'Structural indicators':'Δομικοί δείκτες',icon:Building2}]:[]),...(ok(CAPABILITIES.MANAGE_EXTERNAL_REFERENCES)?[{id:'references',label:t('externalReferences'),icon:Globe2}]:[]),...(ok(CAPABILITIES.MANAGE_ROLES)?[{id:'roles',label:t('rolesPermissions'),icon:ShieldCheck}]:[]),...(isPlatformOwner?[{id:'platform',label:t('managementPlatformCenterLabel'),icon:KeyRound}]:[])]},[role,addOns,customCaps,t,isPlatformOwner,language])

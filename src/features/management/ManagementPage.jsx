...
export function ManagementPage(){
  ...
  return <Page fill title={t('management')} subtitle={t('managementSubtitle')}>
    ...
    {createdUser&&<ObserverDialog eyebrow={t('managementPanel.createUser')} title={t('managementPanel.userCreatedTitle')} subtitle={t('managementPanel.userCreatedHint')} onClose={()=>setCreatedUser(null)} footer={<Button onClick={()=>setCreatedUser(null)}>{t('close')}</Button>}>
      <div className="entry-grid"><label><span>{t('automaticUsername')}</span><input readOnly value={createdUser.username||'—'}/></label><label><span>{t('invitationEmail')}</span><input readOnly value={createdUser.emailSent?t('invitationSent'):t('invitationPending')}/></label></div>
    </ObserverDialog>}
    {selectedUser&&<ObserverDialog width="wide" eyebrow={t('userManagementEyebrow')} title={selectedUser.name} subtitle={`${selectedUser.username} · ${selectedUser.email||t('withoutEmail')}`} onClose={()=>setSelectedUser(null)} footer={<Button variant="secondary" onClick={()=>setSelectedUser(null)}>{t('close')}</Button>}><div className="entry-grid compact"><label className="field"><span>{t('roleLabel')}</span><select value={selectedUser.role} onChange={e=>setSelectedUser(u=>({...u,role:e.target.value}))}>{creatableRoles.map(key=><option key={key} value={key}>{t(roleNames[key]??key)}</option>)}</select></label><label className="field"><span>{t('jobTitleLabel')}</span><input value={selectedUser.jobTitle||''} onChange={e=>setSelectedUser(u=>({...u,jobTitle:e.target.value}))}/></label><label className="field"><span>Email</span><input value={selectedUser.email||''} readOnly/></label><label className="field"><span>{t('phoneLabel')}</span><input value={selectedUser.phone||''} readOnly/></label></div><div className="user-management-actions"><Button onClick={async()=>{await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'update',role:selectedUser.role,jobTitle:selectedUser.jobTitle}});notify(t('userChangesSaved'),'success')}}>{t('save')}</Button><Button variant="secondary" onClick={async()=>{await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'reset_password'}});notify(t('resetEmailSent'),'success')}}>{t('resetPassword')}</Button><Button variant="secondary" onClick={async()=>{const action=selectedUser.status==='disabled'?'reactivate':'suspend';await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action}});setSelectedUser(u=>({...u,status:action==='suspend'?'disabled':'active'}));notify(t('userStatusUpdated'),'success')}}>{selectedUser.status==='disabled'?t('reactivate'):t('pause')}</Button><Button variant="secondary" onClick={async()=>{const ok=await confirm({title:t('permanentUserDelete'),message:`${t('deleteUserMessage')} ${selectedUser.username};`,confirmLabel:t('delete'),danger:true});if(ok){await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'delete'}});setSelectedUser(null);notify(t('userDeleted'),'success')}}}>{t('delete')}</Button></div></ObserverDialog>}
  </Page>
}


function CreateUserDialog({t,onClose,onCreate}){const [fullName,setFullName]=useState(''),[email,setEmail]=useState(''),[phone,setPhone]=useState(''),[jobTitle,setJobTitle]=useState(''),[newRole,setNewRole]=useState(creatableRoles[0]),[submitting,setSubmitting]=useState(false);const valid=fullName.trim().length>1&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim());async function submit(){if(!valid)return;setSubmitting(true);try{await onCreate({fullName:fullName.trim(),email:email.trim(),phone:phone.trim(),jobTitle:jobTitle.trim(),role:newRole})}finally{setSubmitting(false)}}return <ObserverDialog width="wide" eyebrow={t('organizationUsers')} title={t('managementPanel.createUser')} subtitle={t('createUserInviteSubtitle')} onClose={onClose} footer={<DialogActions onCancel={onClose} onSave={submit} disabled={!valid||submitting}/>}><div className="entry-grid"><label className="entry-span-2"><span>{t('fullNameRequired')}</span><input value={fullName} onChange={e=>setFullName(e.target.value)}/></label><label><span>{t('invitationEmailRequired')}</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><label><span>{t('phoneLabel')}</span><input value={phone} onChange={e=>setPhone(e.target.value)}/></label><label><span>{t('jobTitleExtendedLabel')}</span><input value={jobTitle} onChange={e=>setJobTitle(e.target.value)}/></label><label><span>{t('roleLabel')}</span><select value={newRole} onChange={e=>setNewRole(e.target.value)}>{creatableRoles.map(key=><option key={key} value={key}>{t(roleNames[key]??key)}</option>)}</select></label></div></ObserverDialog>}

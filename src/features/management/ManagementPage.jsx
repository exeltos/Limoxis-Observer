...
import { Activity,Bell,Building2,Database,Globe2,KeyRound,Layers3,Pencil,ShieldCheck,X,ClipboardList,Search,Wind,Users } from 'lucide-react'
...
export function ManagementPage({global=false}={}){
 ...
 const tabs=useMemo(()=>{
  if(global)return [{id:'libraries',label:t('libraries'),icon:Database},{id:'bundles',label:t('managementPanel.preventionBundlesLabel'),icon:Layers3},{id:'indicators',label:t('indicators'),icon:Activity},{id:'references',label:t('externalReferences'),icon:Globe2}]
 ...
 return <Page fill title={global?t('managementGlobalTitle'):t('management')} subtitle={global?t('managementGlobalSubtitle'):t('managementSubtitle')}><div className="management-shell workspace-fill"><ModuleTabs className="management-tabs" activeId={tab} onChange={openTab} ariaLabel={t('management')} tabs={tabs}/>
 {tab==='users'&&<ManagementUsersPanel/>}{tab==='announcements'&&<AnnouncementsPanel/>}{tab==='libraries'&&<LibrariesPanel global={global}/>}{tab==='questionnaires'&&<section className="management-section management-scroll-section"><QuestionnairesPanel/></section>}{tab==='environmentTemplates'&&<section className="management-section management-scroll-section"><EnvironmentalStandardsPanel embedded/></section>}{tab==='bundles'&&<section className="management-section management-scroll-section"><BundleLibraryPanel global={global}/></section>}{tab==='indicators'&&<IndicatorsPanel global={global}/>}{tab==='patientDays'&&<BedDaysPanel/>}{tab==='hospitalStructure'&&<HospitalStructurePanel/>}{tab==='prevalenceSurvey'&&<PrevalenceSurveyPanel/>}{tab==='roles'&&<ManagementRolesPanel/>}

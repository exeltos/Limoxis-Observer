...
export function ManagementPage(){
 ...
 const tabs=useMemo(()=>{const ok=cap=>can(role,cap,addOns,customCaps);return [
  ...
  ...(ok(CAPABILITIES.MANAGE_LIBRARIES)?[{id:'libraries',label:t('libraries'),icon:Database},{id:'questionnaires',label:language==='en'?'Questionnaires':'Ερωτηματολόγια',icon:ClipboardList},{id:'environmentTemplates',label:language==='en'?'Environment templates':'Πρότυπα περιβάλλοντος',icon:Wind},{id:'bundles',label:t('managementPanel.preventionBundlesLabel'),icon:Layers3}]:[]),
  ...(ok(CAPABILITIES.MANAGE_INDICATORS)?[{id:'indicators',label:language==='en'?'Indicators':'Δείκτες',icon:Activity}]:[]),...(ok(CAPABILITIES.MANAGE_BED_DAYS)?[{id:'patientDays',label:t('patientDays'),icon:Database}]:[]),...(ok(CAPABILITIES.MANAGE_EXTERNAL_REFERENCES)?[{id:'references',label:t('externalReferences'),icon:Globe2}]:[]),...(ok(CAPABILITIES.MANAGE_ROLES)?[{id:'roles',label:t('rolesPermissions'),icon:ShieldCheck}]:[]),...(isPlatformOwner?[{id:'platform',label:language==='en'?'Platform Center':'Κέντρο Πλατφόρμας',icon:KeyRound}]:[])]},[role,addOns,customCaps,t,isPlatformOwner,language])
 ...
 async function saveReference(){if(referenceEditor?.isGlobal&&!isPlatformOwner){notify(language==='en'?'System references can only be changed by the Platform Owner.':'Οι αναφορές συστήματος μπορούν να αλλάξουν μόνο από τον Platform Owner.','warning');return}try{const saved=isDemo?referenceEditor:await updateExternalReference(tenant.id,referenceEditor);setReferences(rows=>mergeExternalReferences(rows.some(row=>(row.sourceKey||row.id)===(saved.sourceKey||saved.id))?rows.map(row=>(row.sourceKey||row.id)===(saved.sourceKey||saved.id)?saved:row):[...rows,saved]));setReferenceEditor(null);notify(t('managementPanel.externalSourceUpdated'),'success')}catch(error){notify(error?.message||t('managementPanel.saveFailed'),'error')}}
 ...
 {tab==='references'&&<section className="management-section management-scroll-section"><div className="section-toolbar"><div><h2>{t('externalReferences')}</h2><p>{t('externalReferenceNote')}</p></div><Button variant="secondary" onClick={refreshReferences}>{language==='en'?'Reload':'Επαναφόρτωση'}</Button></div>{managementLoading&&<div className="inline-empty">{t('loading')}</div>}<RegistryTable

...
export function ManagementPage({global=false,onBack}={}){
 ...
 const tabs=useMemo(()=>{
  ...
  ...(ok(CAPABILITIES.MANAGE_USERS)?[{id:'users',label:t('users'),icon:Users},{id:'liraAi',label:'LIRA & AI',icon:KeyRound},{id:'liraKnowledge',label:'LIRA Knowledge',icon:BookOpen},{id:'liraOutbreaks',label:language==='en'?'Outbreak investigations':'Διερευνήσεις εξάρσεων',icon:ClipboardList}]:[]),
  ...(ok(CAPABILITIES.MANAGE_ANNOUNCEMENTS)?[{id:'announcements',label:t('announcements'),icon:Bell}]:[]),
  ...(ok(CAPABILITIES.MANAGE_LIBRARIES)?[{id:'libraries',label:t('libraries'),icon:Database},{id:'questionnaires',label:t('managementQuestionnairesLabel'),icon:ClipboardList},{id:'environmentTemplates',label:t('managementEnvironmentTemplatesLabel'),icon:Wind},{id:'bundles',label:t('managementPanel.preventionBundlesLabel'),icon:Layers3},{id:'haiCriteria',label:t('managementPanel.haiCriteriaLabel'),icon:ShieldAlert}]:[]),
 ...
 {tab==='users'&&<ManagementUsersPanel/>}{tab==='announcements'&&<AnnouncementsPanel/>}{tab==='libraries'&&<LibrariesPanel global={global}/>}{tab==='questionnaires'&&<section className="management-section management-scroll-section"><QuestionnairesPanel/></section>}{tab==='environmentTemplates'&&<section className="management-section management-scroll-section"><EnvironmentalStandardsPanel embedded/></section>}{tab==='bundles'&&<section className="management-section management-scroll-section"><BundleLibraryPanel global={global}/></section>}{tab==='haiCriteria'&&<section className="management-section management-scroll-section"><HaiCriteriaLibraryPanel global={global}/></section>}{tab==='indicators'&&<IndicatorsPanel global={global}/>}{tab==='patientDays'&&<BedDaysPanel/>}{tab==='hospitalStructure'&&<HospitalStructurePanel/>}{tab==='prevalenceSurvey'&&<PrevalenceSurveyPanel/>}{tab==='roles'&&<ManagementRolesPanel/>}{tab==='liraAi'&&<LiraAiSettingsPanel/>}{tab==='liraKnowledge'&&<LiraKnowledgePanel/>}{tab==='liraOutbreaks'&&<LiraOutbreakInvestigationsPanel/>}

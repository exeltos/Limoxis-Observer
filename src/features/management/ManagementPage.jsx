...
export function ManagementPage(){
  ...
  async function createUser({fullName,role:newRole,email,phone,jobTitle}){
    ...
    const {data,error}=await supabase.functions.invoke('create-organization-user',{body:{organizationId:tenant.id,fullName,role:newRole,email,phone,jobTitle}})
  ...
  return <Page fill title={t('management')} subtitle={t('managementSubtitle')}>
    <div className="management-shell workspace-fill"><div className="tabs canonical-module-tabs management-tabs">{tabs.map(({id,label,icon:Icon})=><button key={id} className={`tab ${tab===id?'active':''}`} onClick={()=>setTab(id)}><Icon size={16}/>{label}</button>)}</div>
      ...
      {tab==='users'&&<section className="management-section"><div className="section-toolbar"><div><h2>{t('organizationUsers')}</h2><p>{tenant?.name} · δημιουργία, ρόλοι, παύση και ανάκτηση πρόσβασης</p></div>{allowed(CAPABILITIES.MANAGE_USERS)&&!isDemo&&<Button onClick={()=>setCreateUserOpen(true)}><Plus size={15}/>{t('managementPanel.createUser')}</Button>}</div><div className="table-wrap scroll-table"><table className="data-table sticky-table"><thead><tr><th>{t('users')}</th><th>Username</th><th>{t('roleLabel')}</th><th>{t('status')}</th><th/></tr></thead><tbody>{isDemo?demoUsers.map(user=><tr key={user.id}><td><strong>{user.name}</strong><small>{user.email}</small></td><td>DEMO</td><td>{user.role}</td><td><span className="status-badge active">active</span></td><td/></tr>):orgUsers.map(user=><tr key={user.id}><td><strong>{user.name}</strong><small>{user.email||'—'}</small></td><td>{user.username}</td><td><span className="role-badge">{t(roleNames[user.role]??user.role)}</span></td><td><span className={`status-badge ${user.status==='active'?'active':'temporary'}`}>{user.status}</span></td><td><button className="text-button" onClick={()=>setSelectedUser(user)}>Διαχείριση</button></td></tr>)}</tbody></table>{!isDemo&&!orgUsersLoading&&orgUsers.length===0&&<div className="inline-empty">{t('noConnectedUsers')}</div>}</div></section>}
    ...
    {createdUser&&<ObserverDialog eyebrow={t('managementPanel.createUser')} title={t('managementPanel.userCreatedTitle')} subtitle={t('managementPanel.userCreatedHint')} onClose={()=>setCreatedUser(null)} footer={<Button onClick={()=>setCreatedUser(null)}>{t('close')}</Button>}>
      <div className="entry-grid"><label><span>Αυτόματο username</span><input readOnly value={createdUser.username||'—'}/></label><label><span>Email πρόσκλησης</span><input readOnly value={createdUser.emailSent?'Στάλθηκε':'Εκκρεμεί / μη ρυθμισμένο'}/></label></div>
    </ObserverDialog>}
    {selectedUser&&<ObserverDialog width="wide" eyebrow="Hospital Admin · User Management" title={selectedUser.name} subtitle={`${selectedUser.username} · ${selectedUser.email||'χωρίς email'}`} onClose={()=>setSelectedUser(null)} footer={<Button variant="secondary" onClick={()=>setSelectedUser(null)}>Κλείσιμο</Button>}><div className="entry-grid compact"><label className="field"><span>Ρόλος</span><select value={selectedUser.role} onChange={e=>setSelectedUser(u=>({...u,role:e.target.value}))}>{creatableRoles.map(key=><option key={key} value={key}>{t(roleNames[key]??key)}</option>)}</select></label><label className="field"><span>Ιδιότητα</span><input value={selectedUser.jobTitle||''} onChange={e=>setSelectedUser(u=>({...u,jobTitle:e.target.value}))}/></label><label className="field"><span>Email</span><input value={selectedUser.email||''} readOnly/></label><label className="field"><span>Τηλέφωνο</span><input value={selectedUser.phone||''} readOnly/></label></div><div className="user-management-actions"><Button onClick={async()=>{await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'update',role:selectedUser.role,jobTitle:selectedUser.jobTitle}});notify('Οι αλλαγές αποθηκεύτηκαν.','success')}}>Αποθήκευση</Button><Button variant="secondary" onClick={async()=>{await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'reset_password'}});notify('Στάλθηκε email επαναφοράς.','success')}}>Επαναφορά κωδικού</Button><Button variant="secondary" onClick={async()=>{const action=selectedUser.status==='disabled'?'reactivate':'suspend';await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action}});setSelectedUser(u=>({...u,status:action==='suspend'?'disabled':'active'}));notify('Η κατάσταση ενημερώθηκε.','success')}}>{selectedUser.status==='disabled'?'Επανενεργοποίηση':'Παύση'}</Button><Button variant="secondary" onClick={async()=>{const ok=await confirm({title:'Οριστική διαγραφή χρήστη',message:`Να διαγραφεί ο λογαριασμός ${selectedUser.username};`,confirmLabel:'Διαγραφή',danger:true});if(ok){await supabase.functions.invoke('manage-organization-user',{body:{organizationId:tenant.id,userId:selectedUser.userId,action:'delete'}});setSelectedUser(null);notify('Ο χρήστης διαγράφηκε.','success')}}}>Διαγραφή</Button></div></ObserverDialog>}
  </Page>
}


function CreateUserDialog({t,onClose,onCreate}){const [fullName,setFullName]=useState(''),[email,setEmail]=useState(''),[phone,setPhone]=useState(''),[jobTitle,setJobTitle]=useState(''),[newRole,setNewRole]=useState(creatableRoles[0]),[submitting,setSubmitting]=useState(false);const valid=fullName.trim().length>1&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim());async function submit(){if(!valid)return;setSubmitting(true);try{await onCreate({fullName:fullName.trim(),email:email.trim(),phone:phone.trim(),jobTitle:jobTitle.trim(),role:newRole})}finally{setSubmitting(false)}}return <ObserverDialog width="wide" eyebrow={t('organizationUsers')} title={t('managementPanel.createUser')} subtitle="Το username δημιουργείται αυτόματα. Ο χρήστης λαμβάνει email και ορίζει ο ίδιος ισχυρό password." onClose={onClose} footer={<DialogActions onCancel={onClose} onSave={submit} disabled={!valid||submitting}/>}><div className="entry-grid"><label className="entry-span-2"><span>Ονοματεπώνυμο *</span><input value={fullName} onChange={e=>setFullName(e.target.value)}/></label><label><span>Email πρόσκλησης *</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><label><span>Τηλέφωνο</span><input value={phone} onChange={e=>setPhone(e.target.value)}/></label><label><span>Ιδιότητα / τίτλος</span><input value={jobTitle} onChange={e=>setJobTitle(e.target.value)}/></label><label><span>{t('roleLabel')}</span><select value={newRole} onChange={e=>setNewRole(e.target.value)}>{creatableRoles.map(key=><option key={key} value={key}>{t(roleNames[key]??key)}</option>)}</select></label></div></ObserverDialog>}\n

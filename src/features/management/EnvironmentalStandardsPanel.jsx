...
export function EnvironmentalStandardsPanel({embedded=false}){
  ...
  return <section className={embedded?"environmental-embedded management-scroll-section":"management-section management-scroll-section"}>
    ...
    <RegistryTable
      ...
      rows={filtered}
      rowKey={item=>item.id}
      renderRow={item=>{const systemLocked=item.system&&!isPlatformOwner;return <><td><strong>{item.protocolCode}</strong>{item.system&&<small>{isPlatformOwner?t('librariesPanel.systemOwnerBadge'):t('librariesPanel.systemReadOnlyBadge')}</small>}</td><td>{sampleTypeLabel(item.subjectType,t)}</td><td>{environmentalMethodLabel(item.sourceCode,t)}</td><td>{item.unit}</td><td><strong>{item.limitCfu??t('notConfigured')}</strong></td><td><span className={`status-badge ${item.active?'active':''}`}>{item.active?t('active'):t('environmentalStandards.protocolInactive')}</span></td><td><OverflowMenu items={[
    ...
    {draft&&<div className="modal-backdrop"><div className="role-editor environmental-standard-editor" role="dialog" aria-modal="true"><header><div><h3>{draft.id?t('environmentalStandards.editEnvironmentalProtocol'):t('environmentalStandards.newEnvironmentalProtocol')}</h3><p>{draft.system?(language==='en'?'Centrally governed Limoxis system protocol.':'Κεντρικά διαχειριζόμενο πρωτόκολλο συστήματος Limoxis.'):t('environmentalStandards.environmentalProtocolEditorHelp')}</p></div><button className="icon-button" onClick={()=>setDraft(null)}><X size={17}/></button></header>
      <div className="form-grid two-col">
        ...
        <div className="field environmental-active-field"><label className="environmental-active-toggle"><input disabled={readOnlySystem} type="checkbox" checked={draft.active} onChange={e=>setDraft({...draft,active:e.target.checked})}/><span>{t('active')}</span></label></div>
      </div>

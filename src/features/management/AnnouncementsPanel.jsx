  ...
  {editor&&<div className="modal-backdrop"><div className="announcement-editor announcement-editor-wide" role="dialog" aria-modal="true"><header><div><h3>{editor.id?'Επεξεργασία ανακοίνωσης':'Νέα ανακοίνωση'}</h3><p>Περιεχόμενο, παραλήπτες και ακριβές χρονικό παράθυρο εμφάνισης.</p></div><button className="icon-button" onClick={()=>setEditor(null)}><X size={17}/></button></header>
   <div className="announcement-editor-body">
    <section className="announcement-form-section"><header><strong>Περιεχόμενο</strong><span>Το μήνυμα που θα εμφανιστεί στους επιλεγμένους χρήστες.</span></header><div className="entry-form-grid">
     <label className="field entry-span-2"><span>Τίτλος</span><input value={editor.title} onChange={e=>setEditor(x=>({...x,title:e.target.value}))}/></label>
     <label className="field entry-span-2"><span>Μήνυμα</span><textarea rows="5" value={editor.message} onChange={e=>setEditor(x=>({...x,message:e.target.value}))}/></label>
     <label className="field"><span>Προτεραιότητα</span><select value={editor.priority} onChange={e=>setEditor(x=>({...x,priority:e.target.value}))}><option value="normal">Κανονική</option><option value="high">Υψηλή</option><option value="critical">Κρίσιμη</option></select></label>
     <label className="field"><span>Τύπος παραληπτών</span><select value={editor.audienceType} onChange={e=>{setRecipientQuery('');setEditor(x=>({...x,audienceType:e.target.value,audienceValues:[]}))}}><option value="all">Όλο το νοσοκομείο</option><option value="role">Ένας ή περισσότεροι ρόλοι</option><option value="department">Ένα ή περισσότερα τμήματα</option><option value="user">Ένας ή περισσότεροι χρήστες</option></select></label>
    </div></section>
    ...
    <section className="announcement-form-section"><header><strong>Πότε θα εμφανίζεται</strong><span>Αφήστε κενή την έναρξη για άμεση εμφάνιση και τη λήξη για να παραμένει ενεργή.</span></header><div className="announcement-schedule-grid">
     <label className="field"><span>Ημερομηνία έναρξης</span><input type="date" value={editor.startDate} onChange={e=>setEditor(x=>({...x,startDate:e.target.value}))}/></label>
     <label className="field"><span>Ώρα έναρξης</span><input type="time" value={editor.startTime} onChange={e=>setEditor(x=>({...x,startTime:e.target.value}))}/></label>
     <label className="field"><span>Ημερομηνία λήξης</span><input type="date" value={editor.endDate} onChange={e=>setEditor(x=>({...x,endDate:e.target.value}))}/></label>
     <label className="field"><span>Ώρα λήξης</span><input type="time" value={editor.endTime} onChange={e=>setEditor(x=>({...x,endTime:e.target.value}))}/></label>

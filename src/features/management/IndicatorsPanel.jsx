...
export function IndicatorsPanel(){
 const {language}=useLanguage();const {tenant,role,membership}=useTenant();const {notify,confirm}=useFeedback();const el=language==='el';const isOwner=role===ROLES.PLATFORM_OWNER;const canManage=can(role,CAPABILITIES.MANAGE_INDICATORS,membership?.capabilities||[],membership?.customCapabilities||[])
 const [rows,setRows]=useState([]),[loading,setLoading]=useState(false),[editor,setEditor]=useState(null)
 useEffect(()=>{if(!tenant?.id){setRows([]);return}let active=true;setLoading(true);loadIndicatorDefinitions(tenant.id).then(data=>active&&setRows(data)).catch(error=>active&&notify(error?.message||(el?'Αποτυχία φόρτωσης δεικτών.':'Failed to load indicators.'),'error')).finally(()=>active&&setLoading(false));return()=>{active=false}},[tenant?.id,notify,el])

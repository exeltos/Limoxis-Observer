...
export function IndicatorsPanel({global=false}={}){
 ...
 async function save(){if(!indicatorDefinitionIsValid(editor)){notify(el?'Συμπληρώστε τα υποχρεωτικά πεδία.':'Complete required fields.','error');return}try{const saved=await saveIndicatorDefinition(global?null:tenant.id,editor);setRows(current=>current.some(x=>x.id===saved.id)?current.map(x=>x.id===saved.id?saved:x):[...current,saved]);setEditor(null);notify(el?'Ο ορισμός δείκτη αποθηκεύτηκε.':'Indicator definition saved.','success')}catch(error){notify(error?.message||(el?'Αποτυχία αποθήκευσης.':'Save failed.'),'error')}}
 async function retire(item){if(item.system&&!isOwner)return;const ok=await confirm({title:el?'Απόσυρση δείκτη':'Retire indicator',message:`${el?'Να αποσυρθεί':'Retire'} «${item.titleEl}»;`,confirmLabel:el?'Απόσυρση':'Retire',danger:true});if(!ok)return;try{await retireIndicatorDefinition(global?null:tenant.id,item);setRows(current=>current.map(x=>x.id===item.id?{...x,status:'retired'}:x));notify(el?'Ο δείκτης αποσύρθηκε.':'Indicator retired.','success')}catch(error){notify(error?.message||(el?'Αποτυχία απόσυρσης.':'Retire failed.'),'error')}}
 return <section className="management-section management-scroll-section"><div className="section-toolbar"><div><h2>{el?'Ορισμοί δεικτών':'Indicator definitions'}</h2><p>{global?(el?'Κεντρικοί ορισμοί δεικτών της πλατφόρμας — εφαρμόζονται σε όλα τα νοσοκομεία.':'Platform-wide indicator definitions — applied to every hospital.'):(el?'Ορισμοί δεικτών συστήματος και νοσοκομείου, με εκδόσεις, που τροφοδοτούν απευθείας την οθόνη Δεικτών.':'Versioned system and hospital indicator definitions that feed the Indicators screen directly.')}</p></div>{canManage&&<Button onClick={()=>setEditor(createEmptyIndicatorDefinition())}><Plus size={15}/>{el?'Νέος δείκτης':'New indicator'}</Button>}</div>{loading&&<div className="inline-empty">{el?'Φόρτωση…':'Loading…'}</div>}<RegistryTable

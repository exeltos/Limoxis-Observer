...
export function LibrariesPanel(){
 ...
 useEffect(()=>{if(isDemo){setRows(loadDemoState());setLoading(false);return}if(!tenant?.id){setRows({});setLoading(false);return}let mounted=true;setLoading(true);loadManagementLibraries(tenant.id).then(data=>{if(mounted)setRows(data)}).catch(error=>{if(mounted)notify(error?.message||t('loadFailed'),'error')}).finally(()=>{if(mounted)setLoading(false)});return()=>{mounted=false}},[isDemo,tenant?.id,notify,t])
 ...
 async function saveEditor(){const el=draft.el.trim(),en=(draft.en||draft.el).trim();if(!el||!editor)return;if(editor.row?.[2]?.system&&!isPlatformOwner){notify(language==='en'?'System records can only be changed by the Platform Owner.':'Τα στοιχεία συστήματος μπορούν να αλλάξουν μόνο από τον Platform Owner.','warning');return}try{if(isDemo){if(editor.mode==='add')persistDemo({...rows,[active]:[...(rows[active]||[]),newLocalLibraryItem(el,en)]});else persistDemo({...rows,[active]:(rows[active]||[]).map(x=>x===editor.row?[el,en,{...(x[2]||{}),source:x[2]?.system?'Limoxis System':(x[2]?.source||'Hospital')}]:x)})}else{const saved=editor.mode==='add'?await createManagementLibraryItem(tenant.id,active,{nameEl:el,nameEn:en}):await updateManagementLibraryItem(tenant.id,active,editor.row,{nameEl:el,nameEn:en});setRows(current=>({...current,[active]:editor.mode==='add'?[...(current[active]||[]),saved]:(current[active]||[]).map(row=>row===editor.row?saved:row)}))}notify(editor.mode==='add'?t('libraryItemCreated'):t('librariesPanel.libraryItemUpdated'),'success');setEditor(null)}catch(error){notify(error?.message||t('saveFailed'),'error')}}
 async function remove(row){if(row?.[2]?.system&&!isPlatformOwner){notify(language==='en'?'System records can only be deleted by the Platform Owner.':'Τα στοιχεία συστήματος μπορούν να διαγραφούν μόνο από τον Platform Owner.','warning');return}const ok=await confirm({title:t('librariesPanel.removeFromLibraryTitle'),message:row[2]?.system?t('librariesPanel.removeSystemLibraryMessage'):t('librariesPanel.removeLocalLibraryMessage'),confirmLabel:t('librariesPanel.removeLabel'),danger:true});if(!ok)return;try{if(isDemo)persistDemo({...rows,[active]:(rows[active]||[]).filter(x=>x!==row)});else{await removeManagementLibraryItem(tenant.id,active,row);setRows(current=>({...current,[active]:(current[active]||[]).filter(x=>x!==row)}))}notify(t('librariesPanel.libraryItemRemoved'),'success')}catch(error){notify(error?.message||t('deleteFailed'),'error')}}
 return <section className="management-section management-scroll-section">
  <div className="section-toolbar"><div><h2>{t('libraries')}</h2><p>{t('librariesPanel.librariesSubtitle')}</p></div><Button onClick={openAdd}><Plus size={15}/>{t('newLibraryItem')}</Button></div>
  <div className="library-governance-strip"><LockKeyhole size={15}/><div><strong>{language==='en'?'System baseline + hospital records':'Βασική βιβλιοθήκη συστήματος + στοιχεία νοσοκομείου'}</strong><span>{language==='en'?'System-managed records are read-only for hospital users. Only the Platform Owner can change them.':'Τα system-managed στοιχεία είναι μόνο για προβολή στους χρήστες νοσοκομείου. Μόνο ο Platform Owner μπορεί να τα αλλάξει.'}</span></div></div>

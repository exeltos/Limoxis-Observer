...
export function LibrariesPanel(){
 const {language,t}=useLanguage();const {notify,confirm}=useFeedback();const {tenant,isDemo}=useTenant()
 ...
 async function saveEditor(){
  ...
  try{
   if(isDemo){
    ...
    else persistDemo({...rows,[active]:(rows[active]||[]).map(x=>x===editor.row?[el,en,{...(x[2]||{}),system:false,locked:false,source:x[2]?.system?'Hospital override':(x[2]?.source||'Hospital'),version:'local'}]:x)})
   }else{

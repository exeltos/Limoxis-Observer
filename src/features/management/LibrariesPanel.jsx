...
export function LibrariesPanel(){
 ...
 async function saveEditor(){
  ...
  if(editor.row?.[2]?.system&&!isPlatformOwner){notify(t('systemLibraryEditDenied'),'warning');return}
  try{
   if(isDemo){
 ...
 async function remove(row){
  if(row?.[2]?.system&&!isPlatformOwner){notify(t('systemLibraryDeleteDenied'),'warning');return}
 ...
 return <section className="management-section management-scroll-section">
  ...
  {active!=='environmentalProtocols'&&<div className="library-governance-strip"><LockKeyhole size={15}/><div><strong>{t('systemBaselineAndHospitalRecords')}</strong><span>{t('systemLibraryGovernance')}</span></div></div>}

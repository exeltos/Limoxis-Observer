...
function answerText(question,answer,en){
 ...
 if(q.type==='true_false')return Boolean(answer)?(en?'True':'Σωστό'):(en?'False':'Λάθος')
...
export function TrainingResultsReview({program,rows,state,onPersist,busy=false,en=false,language='el'}){
 ...
 return <div className="workspace-column workspace-fill">
  ...
  {selected&&<ObserverDialog width="wide" eyebrow={en?'Participant assessment':'Αξιολόγηση συμμετέχοντα'} title={selected.employeeName||'—'} onClose={()=>setSelected(null)} footer={<DialogActions showCancel onCancel={()=>setSelected(null)} cancelLabel={en?'Close':'Κλείσιμο'} onSave={saveReview} saveLabel={selected.assessmentReviewedAt?(en?'Reviewed':'Ελεγμένη'):(en?'Mark as reviewed':'Σήμανση ως ελεγμένη')} disabled={busy||!reviewed||Boolean(selected.assessmentReviewedAt)}/>}>

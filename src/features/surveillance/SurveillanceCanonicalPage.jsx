...
import { employeeSurveillanceBatches,employeeSurveillanceRecords,getEmployeeSurveillanceKpis as getDemoEmployeeSurveillanceKpis,updateEmployeeSurveillanceRecord } from './employeeSurveillanceData'
...
const preloadPatientClinicalRoute=()=>import('./PatientClinicalRecordRoute')
...
export function SurveillanceCanonicalPage(){
  ...
  const [mode,setMode]=useState(restored.mode==='batches'?'employees':(restored.mode||'patients')),[query,setQuery]=useState(restored.query||''),[department,setDepartment]=useState(restored.department||'all'),[status,setStatus]=useState(restored.status||'all'),[page,setPage]=useState(restored.page||1),[pageSize,setPageSize]=useState(restored.pageSize||15),[creation,setCreation]=useState(null)
  const [openEmployee,setOpenEmployee]=useState(null),[openBatch,setOpenBatch]=useState(null),[openingCaseId,setOpeningCaseId]=useState('')
  ...
  async function load(){
    ...
    try{
      ...
      if(isDemo){
        ...
        setDepartments(unique(surveillanceDemoData.map(x=>language==='el'?x.department:x.departmentEn)).map(name=>({id:name,name,nameEn:name})))
      }else if(tenant?.id){
  ...
  },[mode,cases,employeeRows,environmentRows,query,department,status,language,canAccessRecord])
  ...
  async function openCase(item){
    ...
    const patient=patients.find(row=>(item.patientRecordId&&String(row.recordId)===String(item.patientRecordId))||String(row.id)===String(item.patientId))
    if(!patient){registry.openRecord(navigate,`/surveillance/${item.id}`,String(item.id),rows.map(x=>String(x.id)),{returnState:{surveillanceView:saveView()}});return}
    setOpeningCaseId(String(item.id))
    try{
      const [,episodes,admissions]=await Promise.all([
        preloadPatientClinicalRoute(),
        clinical.loadForPatient(patient),
        patient.recordId?loadAdmissions(patient.recordId):Promise.resolve([]),
      ])
      const admissionId=item.admissionId||admissions.find(admission=>{
        const started=String(item.startedAt||'').slice(0,10)
        const inDates=started&&started>=String(admission.admissionDate||'')&&(!admission.dischargeDate||started<=String(admission.dischargeDate))
        const sameDepartment=!admission.departmentId||!item.departmentId||String(admission.departmentId)===String(item.departmentId)
        return inDates&&sameDepartment
      })?.id||''
      registry.saveViewState(saveView())
      registry.openRecord(navigate,`/patients/${patient.id}`,String(patient.id),patients.map(x=>String(x.id)),{state:{patientPrefetchComplete:true,prefetchedPatients:patients,prefetchedPatient:patient,prefetchedEpisodes:episodes,prefetchedAdmissions:admissions,prefetchedDepartments:departments,admissionId,openTab:admissionId?'surveillanceJourney':'summary',surveillanceView:saveView()}})
    }catch(error){notifyError(error,'load',{operation:'surveillance_patient_prefetch'});setOpeningCaseId('')}
  }
  ...
  if(platformOwnerNoTenant)return <Page fill title={t('clinicalRecords.surveillanceCenter')} subtitle={language==='en'?'Choose an organization to open its live surveillance workspace.':'Επιλέξτε οργανισμό για να ανοίξετε το ζωντανό Κέντρο Επιτήρησης.'}><div className="surface surveillance-org-context"><div className="surveillance-org-context-copy"><span className="eyebrow">PLATFORM OWNER · {language==='en'?'ORGANIZATION CONTEXT':'ΠΛΑΙΣΙΟ ΟΡΓΑΝΙΣΜΟΥ'}</span><h2>{language==='en'?'Select organization':'Επιλογή οργανισμού'}</h2><p>{language==='en'?'Surveillance data are isolated by organization. Select the hospital or clinic you want to work in; your role remains Platform Owner.':'Τα δεδομένα επιτήρησης είναι απομονωμένα ανά οργανισμό. Επιλέξτε το νοσοκομείο ή την κλινική όπου θέλετε να εργαστείτε· ο ρόλος σας παραμένει Platform Owner.'}</p></div><div className="surveillance-org-grid">{productionOrganizations.map(item=><button key={item.id} type="button" className="surveillance-org-card" onClick={()=>setTenantByMembership(item.id)}><strong>{item.organization.name}</strong><span>{item.organization.code||'—'} · {item.organization.type||'—'}</span><small>{language==='en'?'Open live surveillance':'Άνοιγμα ζωντανής επιτήρησης'}</small></button>)}{!productionOrganizations.length&&<div className="registry-empty-state"><strong>{language==='en'?'No organizations available':'Δεν υπάρχουν διαθέσιμοι οργανισμοί'}</strong></div>}</div></div></Page>
  ...
  return <Page fill title={t('clinicalRecords.surveillanceCenter')} subtitle={t('surveillanceSubtitleV051')} actions={<RecordActions actions={[UI_ACTIONS.CREATE]} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.CREATE_SURVEILLANCE}} onAction={()=>setCreation('chooser')}/>}>
    ...
    {openBatch&&<BatchDialog batch={openBatch} language={language} fmt={fmt} onClose={()=>setOpenBatch(null)} onOpenRecord={record=>{setOpenBatch(null);setOpenEmployee(record)}}/>}
  </Page>
}
...
function EmployeeRows({rows,registry,language,fmt,onOpen}){const rowId=x=>String(x.recordId||x.id);return <div className="scroll-table" ref={registry.scrollRef}><RegistryTable bare columns={[{key:'name',label:language==='en'?'Employee':'Εργαζόμενος'},{key:'department',label:language==='en'?'Department':'Τμήμα'},{key:'date',label:language==='en'?'Started':'Έναρξη'},{key:'screening',label:language==='en'?'Screening':'Έλεγχος'},{key:'result',label:language==='en'?'Result':'Αποτέλεσμα'}]} rows={rows} rowKey={rowId} rowProps={x=>({...registry.rowProps(rowId(x),()=>onOpen(x)),className:'clickable-row'})} renderRow={x=><><td><strong>{language==='en'?x.employeeNameEn:x.employeeName}</strong><small>{x.employeeId||'—'}</small></td><td>{language==='en'?x.departmentEn:x.department}</td><td>{fmt(x.startedAt)}</td><td>{(x.screeningTypes||[]).join(', ')}</td><td><span className={`status-badge ${x.resultStatus==='positive'?'danger':''}`}>{x.resultStatus||'pending'}</span></td></>}/></div>}
function BatchDialog({batch,language,fmt,onClose,onOpenRecord}){const en=language==='en';return <ObserverDialog eyebrow={en?'Bulk surveillance':'Μαζική επιτήρηση'} title={batch.id} subtitle={`${fmt(batch.startedAt)} · ${(batch.screeningTypes||[]).join(', ')}`} width="wide" onClose={onClose}><div className="employee-batch-record-list">{(batch.records||[]).map(record=><button type="button" key={record.recordId||record.id} className="employee-batch-record-row" onClick={()=>onOpenRecord(record)}><div><strong>{en?record.employeeNameEn:record.employeeName}</strong><span>{record.employeeId||'—'} · {en?record.departmentEn:record.department}</span></div><div><span className="status-badge">{record.resultStatus||'pending'}</span><b>›</b></div></button>)}</div></ObserverDialog>}

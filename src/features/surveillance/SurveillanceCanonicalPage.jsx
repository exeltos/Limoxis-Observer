...
export function SurveillanceCanonicalPage(){
  ...
  const [openEmployee,setOpenEmployee]=useState(null),[openBatch,setOpenBatch]=useState(null)
  ...
  return <Page fill title={t('clinicalRecords.surveillanceCenter')} subtitle={t('surveillanceSubtitleV051')} actions={<RecordActions actions={[UI_ACTIONS.CREATE]} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.CREATE_SURVEILLANCE}} onAction={()=>setCreation('chooser')}/>}>
    ...
    {creation==='employee'&&canEmployees&&(isDemo?<EmployeeSurveillanceFlow onClose={()=>setCreation(null)} onCreated={load}/>:<ProductionEmployeeSurveillanceFlow mode="single" onClose={()=>setCreation(null)} onCreated={load}/>)}
    {creation==='bulk'&&canEmployees&&(isDemo?<BulkEmployeeSurveillanceFlow onClose={()=>setCreation(null)} onCreated={load}/>:<ProductionEmployeeSurveillanceFlow mode="bulk" onClose={()=>setCreation(null)} onCreated={load}/>)}
    ...
    {openEmployee&&(!isDemo?<EmployeeSurveillanceRecordDialog organizationId={tenant?.id} record={openEmployee} samples={[]} canManage={canManageEmployeeFollowup} t={t} language={language} fmt={fmt} onClose={()=>setOpenEmployee(null)} onUpdated={updated=>{setEmployeeRows(current=>current.map(row=>(row.recordId||row.id)===(updated.recordId||updated.id)?updated:row));setOpenEmployee(updated)}}/>:<DemoEmployeeDialog record={openEmployee} language={language} fmt={fmt} onClose={()=>setOpenEmployee(null)}/>)}
...
function BatchRows({rows,registry,language,fmt,onOpen}){const rowId=x=>String(x.recordId||x.id);return <div className="scroll-table" ref={registry.scrollRef}><RegistryTable bare columns={[{key:'batch',label:language==='en'?'Batch':'Ομάδα'},{key:'department',label:language==='en'?'Department':'Τμήμα'},{key:'date',label:language==='en'?'Started':'Έναρξη'},{key:'count',label:language==='en'?'Employees':'Εργαζόμενοι'},{key:'positive',label:language==='en'?'Positive':'Θετικά'}]} rows={rows} rowKey={rowId} rowProps={x=>({...registry.rowProps(rowId(x),()=>onOpen(x)),className:'clickable-row'})} renderRow={x=><><td><strong>{x.id}</strong></td><td>{language==='en'?x.departmentEn:x.department}</td><td>{fmt(x.startedAt)}</td><td>{x.employeeCount??x.records?.length??0}</td><td>{x.positiveCount??0}</td></>}/></div>}
function DemoEmployeeDialog({record,language,fmt,onClose}){const en=language==='en';return <ObserverDialog eyebrow={en?'Employee surveillance':'Επιτήρηση εργαζομένου'} title={en?record.employeeNameEn:record.employeeName} subtitle={`${record.employeeId||'—'} · ${fmt(record.startedAt)}`} width="wide" onClose={onClose}><div className="detail-grid"><div className="detail-item"><span>{en?'Department':'Τμήμα'}</span><strong>{en?record.departmentEn:record.department}</strong></div><div className="detail-item"><span>{en?'Screening':'Έλεγχος'}</span><strong>{(record.screeningTypes||[]).join(', ')||'—'}</strong></div><div className="detail-item"><span>{en?'Result':'Αποτέλεσμα'}</span><strong>{record.resultStatus||'pending'}</strong></div><div className="detail-item"><span>{en?'Recheck':'Επανέλεγχος'}</span><strong>{fmt(record.recheckDue)}</strong></div></div></ObserverDialog>}

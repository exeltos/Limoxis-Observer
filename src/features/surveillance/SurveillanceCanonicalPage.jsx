...
export function SurveillanceCanonicalPage(){
  ...
  const filtered=useMemo(()=>{
    ...
    if(mode==='patients')return groupByPatient(cases.filter(canAccessRecord).filter(x=>`${x.patient||''} ${x.patientEn||''} ${x.patientId||''} ${latestOrganism(x)||''}`.toLowerCase().includes(q)).filter(x=>department==='all'||(language==='el'?x.department:x.departmentEn)===department).filter(x=>status==='all'?!HIDDEN_CASE_STATUSES.has(x.status):x.status===status))
...
function EmployeeRows({rows,registry,language,fmt,t,onOpen}){const rowId=x=>String(x.recordId||x.id);return <div className="scroll-table" ref={registry.scrollRef}><RegistryTable bare columns={[{key:'name',label:language==='en'?'Employee':'Εργαζόμενος'},{key:'department',label:language==='en'?'Department':'Τμήμα'},{key:'date',label:language==='en'?'Started':'Έναρξη'},{key:'screening',label:language==='en'?'Screening':'Έλεγχος'},{key:'result',label:language==='en'?'Result':'Αποτέλεσμα'}]} rows={rows} rowKey={rowId} rowProps={x=>({...registry.rowProps(rowId(x),()=>onOpen(x)),className:'clickable-row'})} renderRow={x=><><td><strong>{language==='en'?x.employeeNameEn:x.employeeName}</strong><small>{x.employeeId||'—'}</small></td><td>{language==='en'?x.departmentEn:x.department}</td><td>{fmt(x.startedAt)}</td><td>{(x.screeningTypes||[]).map(id=>screeningLabel(id,language)).join(', ')||'—'}</td><td><span className={`status-badge ${x.resultStatus==='positive'?'danger':x.resultStatus==='negative'?'active':''}`}>{t(x.resultStatus||'pending')}</span></td></>}/></div>}

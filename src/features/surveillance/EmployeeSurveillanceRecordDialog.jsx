...
// Requested materials are stored as comma-separated codes (e.g. "nasalSwab, handSwab").
const REQUEST_MATERIALS=[{id:'nasalSwab',el:'Ρινικό επίχρισμα',en:'Nasal swab'},{id:'handSwab',el:'Επίχρισμα χεριών',en:'Hand swab'},{id:'throatSwab',el:'Φαρυγγικό επίχρισμα',en:'Throat swab'}]
function splitMaterials(value){return String(value||'').split(',').map(part=>part.trim()).filter(Boolean)}
function materialLabels(value,en){return splitMaterials(value).map(code=>{const item=REQUEST_MATERIALS.find(row=>row.id===code);return item?(en?item.en:item.el):code}).join(', ')}
function toggleMaterial(value,code){const current=splitMaterials(value);return (current.includes(code)?current.filter(item=>item!==code):[...current,code]).join(', ')}
export function EmployeeSurveillanceRecordDialog({organizationId,record,samples=[],canManage,t,language,fmt,onClose,onUpdated,onSaveFollowup}){
  ...
  // Until the laboratory has a validated result, "no follow-up needed" is premature.
  const resultKnown=['negative','positive','cleared'].includes(selected.resultStatus)||resolvedSamples.some(sample=>sample.result&&(sample.finalizedAt||sample.resultStatus==='validated'))
  ...
  return <ObserverDialog eyebrow={en?'Employee screening record':'Καρτέλα ελέγχου εργαζομένου'} title={en?selected.employeeNameEn:selected.employeeName} subtitle={`${compactCode(selected.id)} · ${en?selected.departmentEn:selected.department}`} width="workspace" className="employee-screening-record-card" onClose={onClose} footer={editMode?<DialogActions onCancel={cancelEdit} onSave={saveFollowup} saveLabel={en?'Save changes':'Αποθήκευση αλλαγών'} disabled={saveDisabled} showCancel/>:(canManage&&followupEligible?<Button variant="secondary" onClick={startEdit}>{hasFollowup(selected)?(en?'Edit':'Επεξεργασία'):(en?'Record follow-up':'Καταγραφή παρακολούθησης')}</Button>:null)}>
    ...
    {!followupEligible&&<section className="employee-record-section"><div className="source-truth-note">{resultKnown?(en?'This screening does not require follow-up.':'Αυτός ο έλεγχος δεν απαιτεί παρακολούθηση.'):(en?'Awaiting the laboratory result. Follow-up is decided once the result is validated.':'Αναμένεται το εργαστηριακό αποτέλεσμα. Η ανάγκη παρακολούθησης κρίνεται μετά την επικύρωσή του.')}</div></section>}
    {requestEditor&&<ObserverDialog width="wide" eyebrow={en?'Laboratory request':'Αίτημα εργαστηρίου'} title={compactCode(requestEditor.sample.id)} subtitle={en?'Corrections are allowed only while the request is pending and has no result.':'Διορθώσεις επιτρέπονται μόνο όσο το αίτημα είναι εκκρεμές και δεν έχει αποτέλεσμα.'} onClose={()=>!requestSaving&&setRequestEditor(null)} footer={<DialogActions onCancel={()=>setRequestEditor(null)} onSave={saveRequest} saveLabel={requestSaving?(en?'Saving…':'Αποθήκευση…'):(en?'Save changes':'Αποθήκευση αλλαγών')} disabled={requestSaving}/>}><div className="employee-lab-request-editor"><fieldset className="field entry-span-2 lab-request-materials"><legend>{en?'Requested examination / material':'Ζητούμενος έλεγχος / υλικό'}</legend><div className="lab-request-material-options">{REQUEST_MATERIALS.map(item=>{const selected=splitMaterials(requestEditor.source).includes(item.id);return <label key={item.id} className={selected?'selected':''}><input type="checkbox" checked={selected} onChange={()=>setRequestEditor(v=>({...v,source:toggleMaterial(v.source,item.id)}))}/><span>{en?item.en:item.el}</span></label>})}</div>{splitMaterials(requestEditor.source).filter(code=>!REQUEST_MATERIALS.some(item=>item.id===code)).map(code=><small key={code} className="field-hint">{en?'Other':'Άλλο'}: {code}</small>)}</fieldset><label className="field"><span>{en?'Request type':'Τύπος αιτήματος'}</span><select value={requestEditor.sampleType} onChange={e=>setRequestEditor(v=>({...v,sampleType:e.target.value}))}><option value="surveillance">{en?'Surveillance':'Επιτήρηση'}</option><option value="screening">{en?'Screening':'Προληπτικός έλεγχος'}</option><option value="other">{en?'Other':'Άλλο'}</option></select></label><label className="field"><span>{en?'Priority':'Προτεραιότητα'}</span><select value={requestEditor.priority} onChange={e=>setRequestEditor(v=>({...v,priority:e.target.value}))}><option value="routine">{en?'Routine':'Ρουτίνα'}</option><option value="urgent">{en?'Urgent':'Επείγον'}</option><option value="critical">{en?'Critical':'Κρίσιμο'}</option></select></label></div></ObserverDialog>}
  </ObserverDialog>
}

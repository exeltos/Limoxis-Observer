...
import { SurveillanceJourneyMap } from './SurveillanceJourneyMap'
import {
...
function CloudClinicalJourney({record,t,fmtDate,fmtDateTime,canAssess,canEdit,canClassifyResistance,canIsolation,canTherapy,canOpenPharmacy,canReassess,canOutcome,canDelete,canReopen,onSaved,tenantId}){
  ...
  {isActive&&<><div className="journey-heading"><div><p>{t('clinicalRecords.strictActiveJourneyHelp')}</p></div><span className="journey-case-id">{record.id}</span></div><SurveillanceJourneyMap record={record} t={t} fmtDate={fmtDate} activeStage={activeStage} onSelect={selectStage}/></>}

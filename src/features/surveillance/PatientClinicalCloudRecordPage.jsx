...
function CloudClinicalJourney({record,t,fmtDate,fmtDateTime,overview=false,canAssess,canLab,canEdit,canClassifyResistance,canIsolation,canTherapy,canOpenPharmacy,canReassess,canOutcome,canDelete,canReopen,onSaved,tenantId}){
  const {notify}=useFeedback()
  const navigate=useNavigate()
  ...
  {stageDriven&&<><div className="journey-heading"><div><p>{t('clinicalRecords.strictActiveJourneyHelp')}</p></div><span className="journey-case-id">{record.id}</span></div><SurveillanceJourneyGuidance record={record} t={t} canAssess={canAssess} canLab={canLab} canIsolation={canIsolation} canTherapy={canTherapy} canReassess={canReassess} onSelect={setActiveStage}/><SurveillanceJourneyMap record={record} t={t} fmtDate={fmtDate} activeStage={activeStage} onSelect={setActiveStage}/></>}

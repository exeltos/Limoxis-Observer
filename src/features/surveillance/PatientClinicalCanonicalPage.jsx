...
function EndDialog({title,t,onClose,onSave}){const [d,setD]=useState({endedAt:new Date().toISOString().slice(0,10),reason:''});return <SimpleDialog title={title} t={t} onClose={onClose} onSave={onSave} disabled={!reason.trim()}><label className="entry-span-2"><span>{t('reasonRequired')}</span><textarea rows={4} value={reason} onChange={e=>setReason(e.target.value)} autoFocus/></label></SimpleDialog>}

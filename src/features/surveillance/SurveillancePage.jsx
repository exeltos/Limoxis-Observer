...
export function SurveillancePage(){
  ...
  const saved = registry.loadViewState({query:'',department:'all',resistance:'all',review:'all'})
  ...
  const departments=[...new Set(surveillanceDemoData.map(x=>language==='el'?x.department:x.departmentEn).filter(Boolean))]
  const rows=useMemo(()=>surveillanceDemoData
    ...
    .filter(x=>review==='all'||x.domains.reassessment===review),[query,department,resistance,review,language,version,canAccessRecord])

  function createSurveillance(draft,patient){
    const created=createClinicalSurveillance({patientId:patient.id,patient:patient.name,patientEn:patient.nameEn,dateOfBirth:patient.dateOfBirth,department:patient.department,departmentEn:patient.departmentEn,admissionDate:patient.admissionDate,...draft,createdBy:actor.name})
    createDemoSurveillanceListItem(created);setVersion(v=>v+1);notify(t('surveillanceCreated'),'success');return created
  }
  function openRecord(item){registry.saveViewState({query,department,resistance,review});registry.openRecord(navigate,`/surveillance/${item.id}`,item.id,rows.map(x=>x.id))}
  function clearEmployeeRequest(){
    if(!requestedEmployeeFromUrl)return
    navigate('/surveillance?mode=employees',{replace:true})
  }
  ...
  return <Page fill title={t('clinicalRecords.surveillanceCenter')} subtitle={t('surveillanceSubtitleV051')} actions={<RecordActions actions={[UI_ACTIONS.CREATE,UI_ACTIONS.PRINT,UI_ACTIONS.EXPORT]} actionCapabilities={{[UI_ACTIONS.CREATE]:CAPABILITIES.CREATE_SURVEILLANCE}} onAction={action=>{
    ...
    {registryMode==='patients'&&<div className="workspace-fill surface surveillance-workspace">
      ...
      <div className="surveillance-list surveillance-registry scroll-list" ref={registry.scrollRef}>
        ...
        {rows.map(item=>{const reviewStatus=item.domains.reassessment;return <article key={item.id} {...registry.rowProps(item.id)} className={`surveillance-registry-row ${registry.highlightId===item.id?'registry-row-returned':''}`} onClick={()=>openRecord(item)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openRecord(item)}}}>
          <div className="surv-main"><strong>{item.id}</strong><small>{item.patientId}</small></div><div className="surv-patient"><strong>{language==='el'?item.patient:item.patientEn}</strong></div><div><span>{language==='el'?item.department:item.departmentEn}</span></div><div><span>{fmt(item.startedAt)}</span></div><div className="surv-micro"><strong>{item.organism||'—'}</strong>{item.resistance&&<span className="risk-badge">{item.resistance}</span>}</div><div className="surv-status"><span className="status-badge active">{t('clinicalRecords.activeSurveillanceState')}</span>{item.isolation&&<span className="status-badge active">{t('isolation')}</span>}</div><div className={`surv-review ${reviewStatus==='overdue'?'overdue':''}`}><strong>{fmt(item.reviewDue)}</strong><small>{t(reviewStatus)}</small></div>
        </article>})}
      </div>
    </div>}
...
function EmployeeSurveillanceRegistry({t,language,fmt,version,onChange,requestedRecordId,onRequestedRecordHandled,returnFrom,onReturn,actorName,actorId}){
  ...
  const rows=useMemo(()=>[...employeeSurveillanceRecords],[version])
  ...
  function loadFollowup(row){setIntervention(row.intervention||'');setInterventionType(row.interventionType||'');setInterventionStart(row.interventionStart||'');setInterventionEnd(row.interventionEnd||'');setNoIntervention(Boolean(row.noIntervention));setRecheckDate(row.recheckDate||'');setNoRecheck(Boolean(row.noRecheck));setCorrectionReason('')}
  function openRecord(row){setSelected(row);loadFollowup(row);setEditMode(false)}
  function closeRecord(){if(returnFrom){onReturn?.();return}setSelected(null);setEditMode(false)}
  useEffect(()=>{
  ...
  function startEdit(){if(!selected)return;loadFollowup(selected);setEditMode(true)}
  function cancelEdit(){if(selected)loadFollowup(selected);setEditMode(false)}
  function saveFollowup(){
    if(!selected)return
    const previous={intervention:selected.intervention||'',interventionType:selected.interventionType||'',interventionStart:selected.interventionStart||'',interventionEnd:selected.interventionEnd||'',noIntervention:Boolean(selected.noIntervention),recheckDate:selected.recheckDate||'',noRecheck:Boolean(selected.noRecheck)}
    ...
    const updated=updateEmployeeSurveillanceRecord(selected.id,{intervention:noIntervention?null:(intervention.trim()||null),interventionType:noIntervention?null:(interventionType||null),interventionStart:noIntervention?null:(interventionStart||null),interventionEnd:noIntervention?null:(interventionEnd||null),noIntervention,interventionStatus:noIntervention?'not_required':(intervention.trim()||interventionType?'recorded':'optional'),recheckDate:noRecheck?null:(recheckDate||null),noRecheck,recheckRequired:false,correctionReason:correctionReason.trim()||selected.correctionReason||null,timeline:[{at:now,type:hasFollowup(selected)?'employeeFollowupCorrected':'employeeFollowupRecorded',actor:actorName,detail:correctionReason.trim()||null},...(selected.timeline||[])]})
    if(!noRecheck&&recheckDate&&recheckDate!==previous.recheckDate)createEmployeeRecheck(updated||selected,{date:recheckDate,createdBy:actorName,createdById:actorId})
    const fresh=employeeSurveillanceRecords.find(x=>x.id===selected.id);setSelected(fresh||updated||selected);setEditMode(false);setCorrectionReason('');onChange?.()
  }
  const existingChanged=selected&&hasFollowup(selected)&&((selected.intervention||'')!==intervention.trim()||(selected.interventionType||'')!==interventionType||(selected.interventionStart||'')!==interventionStart||(selected.interventionEnd||'')!==interventionEnd||Boolean(selected.noIntervention)!==noIntervention||(selected.recheckDate||'')!==recheckDate||Boolean(selected.noRecheck)!==noRecheck)
  ...
  return <section className="surface workspace-fill employee-surveillance-registry">
    ...
    <div className="scroll-table"><table className="data-table sticky-table"><thead><tr><th className="col-surveillance">{t('surveillance')}</th><th className="col-employee">{t('employee')}</th><th className="col-screening">{t('screeningType')}</th><th className="col-result">{t('result')}</th><th className="col-intervention">{t('clinicalRecords.intervention')}</th><th className="col-recheck">{t('clinicalRecords.recheck')}</th><th className="col-status">{t('status')}</th></tr></thead><tbody>{rows.map(row=><tr key={row.id} className={`registry-row-clickable ${isPositive(row)?'employee-screening-positive':''}`} tabIndex={0} onClick={()=>openRecord(row)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openRecord(row)}}}>
      <td><strong>{row.id}</strong></td><td><strong>{language==='el'?row.employeeName:row.employeeNameEn}</strong><small>{row.employeeId} · {language==='el'?row.department:row.departmentEn}</small></td><td>{row.screeningTypes.map(x=>t(x)).join(', ')}</td><td><span className={`status-badge ${row.resultStatus==='negative'||row.resultStatus==='cleared'?'active':''}`}>{t(row.resultStatus||'pending')}</span></td><td className="col-intervention">{row.noIntervention?t('clinicalRecords.noInterventionPlanned'):(row.intervention||row.interventionType||(isPositive(row)?t('optional'):'—'))}</td><td className="col-recheck">{row.noRecheck?t('clinicalRecords.noRecheckPlanned'):(row.recheckDate?fmt(row.recheckDate):(isPositive(row)?t('optional'):'—'))}</td><td className="col-status"><span className={`status-badge ${row.status==='active'?'active':''}`}>{t(row.status)}</span></td>
    </tr>)}</tbody></table></div>

    {selected&&<ObserverDialog eyebrow={t('clinicalRecords.employeeScreeningRecord')} title={language==='el'?selected.employeeName:selected.employeeNameEn} subtitle={`${selected.id} · ${language==='el'?selected.department:selected.departmentEn}`} width="workspace" className="employee-screening-record-card" onClose={closeRecord} footer={editMode?<DialogActions onCancel={cancelEdit} onSave={saveFollowup} saveLabel={t('clinicalRecords.saveChanges')} disabled={saveDisabled} showCancel/>:followupEligible(selected)?<Button variant="secondary" onClick={startEdit}>{hasFollowup(selected)?t('edit'):t('clinicalRecords.recordFollowup')}</Button>:null}>
      ...
      {followupEligible(selected)&&<div className="employee-screening-flow"><div className="screening-flow-step done"><span>01</span><strong>{t('clinicalRecords.positiveResult')}</strong><small>{t('completed')}</small></div><div className={`screening-flow-step ${hasFollowup(selected)?'done':'current'}`}><span>02</span><strong>{t('clinicalRecords.intervention')}</strong><small>{t('optional')}</small></div><div className={`screening-flow-step ${selected.recheckDate||selected.noRecheck?'done':''}`}><span>03</span><strong>{t('clinicalRecords.recheck')}</strong><small>{t('optional')}</small></div><div className={`screening-flow-step ${selected.resultStatus==='cleared'?'done':''}`}><span>04</span><strong>{t('outcome')}</strong><small>{selected.resultStatus==='cleared'?t('completed'):t('clinicalRecords.open')}</small></div></div>}
      ...
      {followupEligible(selected)&&<section className="employee-record-section followup-highlight"><div className="followup-section-title"><strong>{t('clinicalRecords.interventionAndRecheck')}</strong><span>{t('clinicalRecords.followupOptionalHelp')}</span></div>
        {!editMode&&<div className="followup-read-grid"><div><small>{t('clinicalRecords.intervention')}</small><strong>{selected.noIntervention?t('clinicalRecords.noInterventionPlanned'):(selected.interventionType||selected.intervention||t('clinicalRecords.notRecorded'))}</strong>{selected.intervention&&selected.interventionType&&<span>{selected.intervention}</span>}{selected.interventionStart&&<span>{t('clinicalRecords.start')}: {fmt(selected.interventionStart)}</span>}{selected.interventionEnd&&<span>{t('clinicalRecords.end')}: {fmt(selected.interventionEnd)}</span>}</div><div><small>{t('clinicalRecords.recheck')}</small><strong>{selected.noRecheck?t('clinicalRecords.noRecheckPlanned'):(selected.recheckDate?fmt(selected.recheckDate):t('notScheduled'))}</strong></div></div>}
        {editMode&&<div className="employee-followup-edit-grid"><div className="entry-span-2 followup-choice-row"><label><input type="checkbox" checked={noIntervention} onChange={e=>{setNoIntervention(e.target.checked);if(e.target.checked){setIntervention('');setInterventionType('');setInterventionStart('');setInterventionEnd('')}}}/><span>{t('clinicalRecords.noInterventionPlanned')}</span></label></div>{!noIntervention&&<><label className="field"><span>{t('clinicalRecords.interventionType')}</span><input list="employee-intervention-types" value={interventionType} onChange={e=>setInterventionType(e.target.value)} placeholder={t('clinicalRecords.chooseOrType')}/><datalist id="employee-intervention-types"><option value={t('clinicalRecords.ointment')}/><option value={t('clinicalRecords.nasalOintment')}/><option value={t('clinicalRecords.topicalTreatment')}/><option value={t('clinicalRecords.decolonizationProtocol')}/><option value={t('other')}/></datalist></label><label className="field"><span>{t('clinicalRecords.interventionDetails')}</span><input value={intervention} onChange={e=>setIntervention(e.target.value)} placeholder={t('clinicalRecords.optionalNotes')}/></label><ManualDateField className="field" label={t('clinicalRecords.start')} value={interventionStart} onChange={setInterventionStart}/><ManualDateField className="field" label={t('clinicalRecords.end')} value={interventionEnd} onChange={setInterventionEnd}/></>}<div className="entry-span-2 followup-choice-row"><label><input type="checkbox" checked={noRecheck} onChange={e=>{setNoRecheck(e.target.checked);if(e.target.checked)setRecheckDate('')}}/><span>{t('clinicalRecords.noRecheckPlanned')}</span></label></div>{!noRecheck&&<div className="entry-span-2"><ManualDateField className="field" label={t('clinicalRecords.recheckDate')} value={recheckDate} onChange={setRecheckDate}/><small className="field-hint">{t('clinicalRecords.recheckOptionalHelp')}</small></div>}{hasFollowup(selected)&&<label className="field entry-span-2"><span>{t('clinicalRecords.correctionReason')}</span><textarea rows="2" value={correctionReason} onChange={e=>setCorrectionReason(e.target.value)} placeholder={t('clinicalRecords.correctionReasonPlaceholder')}/><small>{t('clinicalRecords.correctionReasonHelp')}</small></label>}</div>}
      </section>}
...
function EmployeeBatchRegistry({t,language,fmt,version,onOpenRecord}){
  ...
  const rows=useMemo(()=>[...employeeSurveillanceBatches],[version])
  const batchRecords=batch=>employeeSurveillanceRecords.filter(x=>batch?.surveillanceIds?.includes(x.id))
  return <section className="surface workspace-fill employee-surveillance-registry"><div className="registry-section-heading"><div><span className="eyebrow">{t('clinicalRecords.bulkSurveillance')}</span><h3>{t('clinicalRecords.employeeSurveillanceBatches')}</h3><p>{t('clinicalRecords.clickRowForBatchDetails')}</p></div></div><div className="scroll-table"><table className="data-table sticky-table"><thead><tr><th>{t('batch')}</th><th>{t('department')}</th><th>{t('screeningDate')}</th><th>{t('screeningType')}</th><th>{t('employees')}</th><th>{t('status')}</th></tr></thead><tbody>{rows.map(row=><tr key={row.id} className="registry-row-clickable" tabIndex={0} onClick={()=>setSelectedBatch(row)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSelectedBatch(row)}}}><td><strong>{row.id}</strong></td><td>{language==='el'?row.department:row.departmentEn}</td><td>{fmt(row.startedAt)}</td><td>{row.screeningTypes.map(x=>t(x)).join(', ')}</td><td>{row.employeeIds.length}</td><td><span className={`status-badge ${row.status==='active'?'active':''}`}>{t(row.status)}</span></td></tr>)}</tbody></table></div>
    {selectedBatch&&<ObserverDialog eyebrow={t('clinicalRecords.bulkSurveillance')} title={selectedBatch.id} subtitle={`${fmt(selectedBatch.startedAt)} · ${selectedBatch.screeningTypes.map(x=>t(x)).join(', ')}`} width="wide" className="employee-batch-record-card" onClose={()=>setSelectedBatch(null)}><div className="employee-batch-record-list">{batchRecords(selectedBatch).map(record=><button type="button" key={record.id} className="employee-batch-record-row" onClick={()=>{setSelectedBatch(null);onOpenRecord?.(record.id)}}><div><strong>{language==='el'?record.employeeName:record.employeeNameEn}</strong><span>{record.employeeId} · {language==='el'?record.department:record.departmentEn}</span></div><div><span className={`status-badge ${record.resultStatus==='negative'||record.resultStatus==='cleared'?'active':''}`}>{t(record.resultStatus||'pending')}</span><b>›</b></div></button>)}</div></ObserverDialog>}
  </section>
}

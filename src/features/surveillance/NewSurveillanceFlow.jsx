...
export function NewSurveillanceFlow({patient=null,patients=[],departments=[],initialSample=null,onClose,onCreate,onCancelCreated,onSaveAssessment,onRequestSample,onSaveIsolation,onRecordChange,onPatientsChange}){
  ...
  return <div className="episode-overlay new-surveillance-flow-overlay" role="dialog" aria-modal="true">
    <section className="episode-detail-card new-surveillance-flow-card">
      ...
      {cancelOpen&&<div className="flow-cancel-backdrop" role="presentation"><section className="flow-cancel-dialog" role="dialog" aria-modal="true"><h3>{language==='el'?'Ακύρωση επιτήρησης':'Cancel surveillance'}</h3><p>{language==='el'?'Η επιτήρηση έχει ήδη δημιουργηθεί. Η ακύρωση θα καταγραφεί στο ιστορικό και απαιτεί αιτιολογία.':'The surveillance has already been created. Cancellation is audited and requires a reason.'}</p><label><span>{t('reason')}</span><textarea rows={3} autoFocus value={cancelReason} onChange={e=>setCancelReason(e.target.value)}/></label><div className="flow-cancel-actions"><Button variant="secondary" onClick={()=>setCancelOpen(false)}>{language==='el'?'Επιστροφή':'Back'}</Button><Button disabled={busy||!cancelReason.trim()} onClick={confirmCancel}>{busy?(language==='el'?'Ακύρωση…':'Cancelling…'):(language==='el'?'Επιβεβαίωση ακύρωσης':'Confirm cancellation')}</Button></div></section></div>}
    </section>
  </div>
}

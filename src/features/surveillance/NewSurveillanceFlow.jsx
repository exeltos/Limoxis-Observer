...
export function NewSurveillanceFlow({patient=null,patients=[],departments=[],initialSample=null,onClose,onCreate,onCancelCreated,onSaveAssessment,onRequestSample,onSaveIsolation,onRecordChange,onPatientsChange}){
  ...
  function cancelFlow(){if(record){setCancelReason('');setCancelOpen(true);return}closeFlow()}
  async function confirmCancel(){if(!record||!cancelReason.trim()||busy)return;try{setBusy(true);await onCancelCreated?.(record,cancelReason.trim());setCancelOpen(false);closeFlow();notify(language==='el'?'Η επιτήρηση ακυρώθηκε με καταγραφή αιτιολογίας.':'Surveillance was cancelled with an audit reason.','success')}catch(error){notify(error?.message||t('actionFailed'),'danger')}finally{setBusy(false)}}
  ...
  return <div className="episode-overlay new-surveillance-flow-overlay" role="dialog" aria-modal="true">
    <section className="episode-detail-card new-surveillance-flow-card">
      ...
      <div className="episode-detail-scroll progressive-surveillance-scroll">
        ...
        <div className="progressive-journey-header"><span className="eyebrow">{language==='el'?'Έναρξη επεισοδίου':'Episode start'}</span><h3>{language==='el'?'Νέα ενεργή επιτήρηση':'New active surveillance'}</h3><p>{language==='el'?'Καταγράψτε μόνο τα στοιχεία έναρξης. Μετά τη δημιουργία, η κλινική αξιολόγηση, τα δείγματα, η απομόνωση, η θεραπεία και οι επανεκτιμήσεις ενημερώνονται ανεξάρτητα στο ενεργό workspace.':'Record only the start information. After creation, assessment, samples, isolation, therapy and reassessments are managed independently in the active workspace.'}</p></div>
        ...
        <div className="progressive-guidance"><AlertTriangle size={16}/><div><strong>{language==='el'?'Κλινική αρχή':'Clinical principle'}</strong><span>{language==='el'?'Η δημιουργία επιτήρησης δεν απαιτεί νέο δείγμα ούτε απόφαση απομόνωσης. Αυτά μπορούν να καταγραφούν οποτεδήποτε χρειαστεί.':'Starting surveillance does not require a new sample or an isolation decision. These can be recorded whenever clinically needed.'}</span></div></div>

        {activeStep==='start'&&<section className="flow-step-panel"><div className="flow-step-heading"><div><span>01</span><h3>{t('surveillanceStart')}</h3></div><p>{t('clinicalRecords.surveillanceStartStepHelp')}</p></div><div className="entry-grid"><ManualDateField label={t('surveillanceStartDate')} value={startDraft.startedAt} onChange={v=>setStart('startedAt',v)}/><ManualDateField label={t('nextReview')} optional value={startDraft.reviewDue} onChange={v=>setStart('reviewDue',v)}/><div className="flow-context-field"><span>{t('department')}</span><strong>{contextDepartment||'—'}</strong><small>{language==='el'?'Από την ενεργή νοσηλεία':'From active admission'}</small></div><div className="flow-context-field"><span>{t('room')}</span><strong>{language==='el'?'Δεν έχει καταχωρηθεί στη νοσηλεία':'Not recorded on admission'}</strong><small>{language==='el'?'Η θέση ασθενούς δεν αλλάζει από την επιτήρηση':'Patient location is not edited from surveillance'}</small></div><label><span>{t('clinicalRecords.suspectedSource')}</span><select value={startDraft.suspectedSource} onChange={e=>setStart('suspectedSource',e.target.value)}><option value="">{t('underAssessment')}</option><option value="bloodstream">{t('clinicalRecords.bloodstream')}</option><option value="urinary">{t('clinicalRecords.urinary')}</option><option value="respiratory">{t('clinicalRecords.respiratory')}</option><option value="surgicalSite">{t('clinicalRecords.surgicalSite')}</option><option value="other">{t('other')}</option></select></label><label className="entry-span-2"><span>{t('surveillanceReason')}</span><textarea rows={3} value={language==='el'?startDraft.reason:startDraft.reasonEn} onChange={e=>setStart(language==='el'?'reason':'reasonEn',e.target.value)}/></label></div><div className="flow-step-actions"><Button variant="secondary" onClick={cancelFlow}>{language==='el'?'Ακύρωση':'Cancel'}</Button><Button disabled={busy||!startDraft.startedAt||!(startDraft.reason||startDraft.reasonEn)||(patientMode==='existing'&&!selectedPatient)} onClick={saveStart}>{busy?(language==='el'?'Αποθήκευση…':'Saving…'):(language==='el'?'Έναρξη επιτήρησης':'Start surveillance')}</Button></div></section>}
      ...
      {cancelOpen&&<ObserverDialog title={language==='el'?'Ακύρωση επιτήρησης':'Cancel surveillance'} width="medium" onClose={()=>setCancelOpen(false)}><p>{language==='el'?'Η επιτήρηση έχει ήδη δημιουργηθεί. Η ακύρωση καταγράφεται στο ιστορικό και απαιτεί αιτιολογία.':'The surveillance has already been created. Cancellation is audited and requires a reason.'}</p><label><span>{language==='el'?'Αιτιολογία':'Reason'}</span><textarea rows={3} autoFocus value={cancelReason} onChange={e=>setCancelReason(e.target.value)}/></label><div className="dialog-actions"><Button variant="secondary" onClick={()=>setCancelOpen(false)}>{language==='el'?'Επιστροφή':'Back'}</Button><Button disabled={busy||!cancelReason.trim()} onClick={confirmCancel}>{busy?(language==='el'?'Ακύρωση…':'Cancelling…'):(language==='el'?'Επιβεβαίωση ακύρωσης':'Confirm cancellation')}</Button></div></ObserverDialog>}
    </section>
  </div>
}

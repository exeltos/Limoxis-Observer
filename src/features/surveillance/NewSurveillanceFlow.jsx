...
const clinicalProfiles={
  urinary:{symptoms:['fever','chills','dysuria','urgency','suprapubicPain','hypotension'],risks:['urinaryCatheter','urologicProcedure','recentAntibiotics','recentHospitalization','knownMdro']},
  respiratory:{symptoms:['fever','cough','dyspnea','secretions','chills','hypotension'],risks:['ventilation','aspiration','recentAntibiotics','recentHospitalization','immunosuppression']},
  bloodstream:{symptoms:['fever','chills','hypotension','localSigns'],risks:['centralLine','peripheralLine','device','recentAntibiotics','immunosuppression']},
  surgicalSite:{symptoms:['fever','localSigns','woundDrainage','erythema','pain'],risks:['recentSurgery','recentProcedure','recentAntibiotics','immunosuppression']},
}
...
function orderedClinicalItems(items,preferred=[]){
  const preferredSet=new Set(preferred)
  return [...preferred.filter(item=>items.includes(item)),...items.filter(item=>!preferredSet.has(item))]
}
...
export function NewSurveillanceFlow({patient=null,patients=[],departments=[],onClose,onCreate,onSaveAssessment,onRequestSample,onSaveIsolation,onRecordChange,onPatientsChange}){
  ...
  const profileForSource=clinicalProfiles[startDraft.suspectedSource]||{symptoms:[],risks:[]}
  const symptomItems=orderedClinicalItems(allSymptoms,profileForSource.symptoms)
  const riskItems=orderedClinicalItems(allRisks,profileForSource.risks)
  ...
  useEffect(()=>{
    ...
    try{
      const saved=JSON.parse(sessionStorage.getItem(FLOW_RECOVERY_KEY)||'null')
      if(saved?.record&&Date.now()-(saved.at||0)<120000)restoreCreated({detail:{record:saved.record}})
    }catch{/* ignore stale recovery payload */}
  ...
  async function saveStart(){
    ...
    try{
      ...
      if(!record){
        ...
        try{sessionStorage.setItem(FLOW_RECOVERY_KEY,JSON.stringify({record:created,at:Date.now()}))}catch{/* storage unavailable */}
        ...
        const next={...record,...startDraft};setRecord(next);markComplete('start');onRecordChange?.(next);setSavedDraft(true);setActiveStep('assessment')
      }
  ...
  async function requestSample(){
    ...
    try{
      ...
      setRecord(next);markComplete('microbiology');setSavedDraft(v=>!v);onRecordChange?.(next);notify(t('clinicalRecords.sampleRequestSavedContinueIsolation'),'success');setIsolationNeeded(next.isolation?true:(next.isolationDecision?.required===false?false:null));setActiveStep('isolation')
    }catch(error){notify(error?.message||t('actionFailed'),'danger')}
    finally{setBusy(false)}
  }
  ...
  function addCustom(field,value,setter){const clean=value.trim();if(!clean)return;setAssessmentDraft(d=>({...d,[field]:d[field].includes(clean)?d[field]:[...d[field],clean]}));setter('')}

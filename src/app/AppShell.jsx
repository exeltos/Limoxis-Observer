...
export function AppShell(){
  ...
  const platformItemActive=(to)=>{const [path,hash='']=to.split('#');return location.pathname===path&&((!hash&&!location.hash)||location.hash===`#${hash}`||location.hash.startsWith(`#${hash}?`))}
  ...
  return <div className={`app-shell ${helpPreviewMode?'help-preview-mode':''}`}>
    <aside className="sidebar"><div className="brand"><div className="brand-mark">L+</div><div><strong>Limoxis Observer</strong><span>{platformMode?t('platformAdministration'):t('brandSubtitle')}</span></div></div><nav>
      {platformMode&&<a href="/platform" className="nav-item active"><BarChart3 size={18}/><span>{t('platformAdministration')}</span></a>}
    ...
    <main className="main-column"><header className={`topbar ${platformMode?'platform-topbar':''}`}>{platformMode?<nav className="platform-primary-nav" aria-label={t('platformAdministration')}>{platformItems.map(([to,labelKey,Icon])=>{const active=platformItemActive(to);return <a key={to} href={to} className={active?'active':''} aria-current={active?'page':undefined}><Icon size={16}/><span>{t(labelKey)}</span></a>})}</nav>:<div className="topbar-spacer"/>}<div className="topbar-actions">

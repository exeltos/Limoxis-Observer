...
import { Bell, BookOpen, ChevronDown, Eye, Info, Layers3, LogOut, Search, TestTube2, X } from 'lucide-react'
...
export function AppShell(){
  const {language,setLanguage,t}=useLanguage(); const {tenant,memberships,membership,role,actualRole,isDemo,setTenantByMembership,canRolePreview,isRolePreview,rolePreview,startRolePreview,updateRolePreviewDepartment,stopRolePreview}=useTenant(); const {profile,logout}=useAuth(); const notifications=useNotifications(); const navigate=useNavigate(); const location=useLocation(); const [helpOpen,setHelpOpen]=useState(false); const [notificationOpen,setNotificationOpen]=useState(false); const [birthdayOpen,setBirthdayOpen]=useState(false); const [briefingOpen,setBriefingOpen]=useState(false); const [testOpen,setTestOpen]=useState(false); const [previewOpen,setPreviewOpen]=useState(false); const [moreOpen,setMoreOpen]=useState(false)
  ...
  const canUseTestCenter=isDemo||actualRole===ROLES.PLATFORM_OWNER
  ...
  return <div className="app-shell">
    ...
    </nav><div className="sidebar-footer"><div className="tenant-name">{tenant?.name??t('noOrganization')}</div><div className="sidebar-meta">{role?.replaceAll('_',' ')??'—'}</div>{isDemo&&<span className="demo-pill">{t('demo')}</span>}<button className="sidebar-about-link" onClick={()=>navigate('/about')}><Info size={14}/><span>Πληροφορίες Limoxis</span></button><button className="app-version-stamp version-button" onClick={()=>navigate('/about')}>v{APP_VERSION} · {BUILD_ID}</button></div></aside>
    <main className="main-column"><header className="topbar"><div className="search-box"><Search size={17}/><input aria-label={t('search')} placeholder={`${t('search')}...`}/></div><div className="topbar-actions">
      ...
      <div className="topbar-utility-group">
        <button className="icon-button info-button" aria-label="Πληροφορίες" title="Πληροφορίες" onClick={()=>navigate('/about')}><Info size={18}/></button>
        ...
        <div className="notification-anchor"><button className={`icon-button notification-button ${notificationOpen?'active':''}`} aria-label={t('notifications')} title={t('notifications')} onClick={()=>setNotificationOpen(v=>!v)}><Bell size={18}/>{notifications.unreadCount>0&&<span className="notification-count">{notifications.unreadCount>99?'99+':notifications.unreadCount}</span>}</button><NotificationCenter open={notificationOpen} onClose={()=>setNotificationOpen(false)} onOpenBriefing={()=>{setNotificationOpen(false);setBriefingOpen(true)}} onOpenBirthday={()=>{setNotificationOpen(false);setBirthdayOpen(true)}}/></div>
        {canUseTestCenter&&<div className="notification-anchor"><button className={`icon-button test-center-button ${testOpen?'active':''}`} title="Δοκιμές ιδιοκτήτη" onClick={()=>setTestOpen(v=>!v)}><TestTube2 size={17}/></button>{testOpen&&<div className="owner-test-popover"><strong>Δοκιμές ιδιοκτήτη</strong><small>Ορατό μόνο σε Platform Owner / demo.</small><button onClick={()=>{setTestOpen(false);setBirthdayOpen(true)}}>Προβολή ευχής</button><button onClick={()=>{setTestOpen(false);setBriefingOpen(true)}}>Προβολή ενημέρωσης</button><button onClick={()=>{notifications.addAnnouncement({title:'Δοκιμαστική ειδοποίηση',message:'Έλεγχος κουδουνιού και unread counter.',priority:'high',audienceType:'all',audienceValues:[],createdBy:'Platform Owner',requiresAck:false,startAt:'',endAt:''});notifications.resetDemoReads();setTestOpen(false)}}>Δοκιμή κουδουνιού</button></div>}</div>}

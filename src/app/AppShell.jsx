...
export function AppShell(){
  ...
  return <div className={`app-shell ${helpPreviewMode?'help-preview-mode':''}`}>
    ...
    <main className="main-column"><header className="topbar"><div className="topbar-spacer"/><div className="topbar-actions">
      ...
      {!platformMode&&!isPlatformOwner&&memberships.length>1&&<label className="tenant-switch"><select value={membership?.id??''} onChange={e=>setTenantByMembership(e.target.value)}>{memberships.map(item=><option key={item.id} value={item.id}>{item.organization.name}</option>)}</select><ChevronDown size={14}/></label>}

...
const HelpCenter=lazy(()=>import('../core/help/HelpCenter').then(module=>({default:module.HelpCenter})))
...
export function AppShell(){
  ...
  const {language,setLanguage,t}=useLanguage(); const {tenant,memberships,membership,role,isDemo,platformDemoPreview,togglePlatformDemoPreview,setTenantByMembership,returnToPlatform,actualRole,canRolePreview,isRolePreview,rolePreview,startRolePreview,updateRolePreviewDepartment,stopRolePreview}=useTenant(); const {profile,logout}=useAuth(); const notifications=useNotifications(); const {confirm}=useFeedback(); const navigate=useNavigate(); const location=useLocation(); const [helpOpen,setHelpOpen]=useState(false);const [helpRequested,setHelpRequested]=useState(false);useEffect(()=>{if(helpOpen)setHelpRequested(true)},[helpOpen]); const [notificationOpen,setNotificationOpen]=useState(false); const [birthdayOpen,setBirthdayOpen]=useState(false); const [briefingOpen,setBriefingOpen]=useState(false); const [previewOpen,setPreviewOpen]=useState(false); const [moreOpen,setMoreOpen]=useState(false); const [accountOpen,setAccountOpen]=useState(false)
  ...
  return <div className={`app-shell ${platformMode?'platform-owner-shell':''} ${helpPreviewMode?'help-preview-mode':''}`} style={platformMode?{gridTemplateColumns:'minmax(0,1fr)'}:undefined}>
    ...
    </div></header>{isRolePreview&&<div className="preview-banner"><Eye size={15}/><span>{t('previewingAs')}: <strong>{t(previewRoles.find(([value])=>value===role)?.[1]||'roleLabel')}</strong>{previewNeedsDepartment&&rolePreview?.department?` · ${t(previewDepartments.find(([value])=>value===rolePreview.department)?.[1]||'departmentScope')}`:''}</span><button onClick={stopRolePreview}><X size={14}/>{t('exitPreview')}</button></div>}<div className="content"><Outlet/></div></main>{!helpPreviewMode&&helpRequested&&<Suspense fallback={null}><HelpCenter open={helpOpen} onClose={()=>setHelpOpen(false)}/></Suspense>}{!helpPreviewMode&&<BirthdayGreeting open={birthdayOpen} onClose={closeBirthday}/>}{!helpPreviewMode&&<LoginBriefingDialog open={briefingOpen} onClose={closeBriefing}/>}<AccountDrawer open={accountOpen} onClose={()=>setAccountOpen(false)}/>
  </div>
}

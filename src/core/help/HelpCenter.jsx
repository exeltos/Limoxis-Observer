...
export function HelpCenter({open,onClose}){
 ...
 useEffect(()=>{if(open){setMode('manual');setSelected(currentSection);setChapter(0);setImageOpen(false)}},[open,pathname,currentSection])
 if(!open)return null
 return <aside className="help-panel-shell" role="dialog" aria-modal="false" aria-label="Κέντρο Βοήθειας & Πληροφοριών">
   ...
   <div className="manual-body">
    ...
    {imageOpen&&<div className="manual-image-lightbox manual-image-lightbox-floating" onMouseDown={e=>e.target===e.currentTarget&&setImageOpen(false)}>
      <div className="manual-preview-floating-card" onMouseDown={e=>e.stopPropagation()}>
        <div className="manual-live-floating"><iframe src={netlifyPreviewUrl(selected,role)} title={`Netlify μεγέθυνση ${current.title}`} tabIndex="-1"/></div>
        <button className="manual-lightbox-close" aria-label="Κλείσιμο μεγέθυνσης" title="Κλείσιμο" onClick={()=>setImageOpen(false)}><X size={21}/></button>
      </div>
    </div>}

...
if(!s.includes("const [userEditing,setUserEditing]")){
...
if(!s.includes("setUserDraft({fullName:selectedUser.name")){
  const anchor="  const canSave=Boolean(draft.name.trim()&&draft.code.trim())"
  if(!s.includes(anchor))throw new Error('canSave anchor not found')
...
if(!s.includes('async function saveSelectedUser()')){
  const tabsAnchor='\n\n  const tabs='
  const idx=s.indexOf(tabsAnchor)
  if(idx<0)throw new Error('tabs anchor not found')
  const extra=[
    '',
...
if(!s.includes('className="platform-user-record-shell workspace-fill"')){
  const mainReturn=s.indexOf('  return <EntityRecordShell')
  if(mainReturn<0)throw new Error('main return not found')
  const full=[
  ...
  s=s.slice(0,mainReturn)+full+s.slice(mainReturn)
}
...
if(usersStart>=0){
  ...
  s=s.slice(0,usersStart)+users+s.slice(usersEnd)
}
...
if(edge.includes(oldUpdate))edge=edge.replace(oldUpdate,newUpdate)

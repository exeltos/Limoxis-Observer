...
describe('LIRA governed RAG runtime',()=>{
 ...
 it('grounds the provider prompt in retrieved chunks rather than browser supplied knowledge',()=>{expect(edge).toContain("caller.rpc('get_lira_rag_chunks'");expect(edge).toContain('approvedKnowledge:knowledge');expect(edge).not.toContain('approvedKnowledge:body.knowledge')})

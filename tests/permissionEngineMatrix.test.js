...
describe('system-only capabilities never leak into custom roles or add-ons', () => {
  ...
  it('marks platform administration as system-only and ineligible for custom roles', () => {
    ...
    for (const id of systemOnlyCapabilities) expect(isCustomRoleEligible(id)).toBe(false)
  })
  ...
  it('restricts platform capabilities to the platform owner role only', () => {
    for (const role of allRoleIds) {
  ...
  it('grants hospital_admin every non-platform capability at organization scope', () => {
    for (const capability of allCapabilityIds) {
      ...
      expect(scopeFor(capability,{role:ROLES.HOSPITAL_ADMIN})).toBe(capability===CAPABILITIES.VIEW_MY_PROFILE?DATA_SCOPES.SELF:DATA_SCOPES.ORGANIZATION)
    }
...
describe('sensitive domain isolation', () => {
  ...
  it('lists occupational health and clinical review as sensitive', () => {
    expect(sensitiveCapabilities).toEqual(expect.arrayContaining([
  ...
  it('allows Hospital Admin sensitive hospital capabilities while preserving organization scope', () => {
...
describe('canForRecord: cross-tenant isolation', () => {
  it('denies access to a record owned by a different organization', () => {
    ...
    expect(canForRecord(CAPABILITIES.VIEW_QUALITY,{organizationId:'org-1'},context)).toBe(true)
    expect(canForRecord(CAPABILITIES.VIEW_QUALITY,{organizationId:'org-2'},context)).toBe(false)
  })

  it('does not require an organization match for a platform-scoped capability', () => {
    expect(canForRecord(CAPABILITIES.VIEW_PLATFORM,{organizationId:'org-2'},{role:ROLES.PLATFORM_OWNER,organizationId:'org-1'})).toBe(true)
  })
...
describe('canForRecord: department scoping', () => {
  it('denies a department manager access to another department\'s record', () => {
    ...
    expect(canForRecord(CAPABILITIES.EXECUTE_CONTROL,{organizationId:'org-1',departmentId:'dept-1'},context)).toBe(true)
    expect(canForRecord(CAPABILITIES.EXECUTE_CONTROL,{organizationId:'org-1',departmentId:'dept-2'},context)).toBe(false)
  })
  ...
  it('cannot escalate a department-scoped capability to organization scope via an override', () => {
    ...
    expect(canForRecord(CAPABILITIES.EXECUTE_CONTROL,{organizationId:'org-1',departmentId:'dept-2'},{role:ROLES.DEPARTMENT_MANAGER,organizationId:'org-1',departmentIds:['dept-1'],scopeOverrides:{[CAPABILITIES.EXECUTE_CONTROL]:DATA_SCOPES.ORGANIZATION}})).toBe(false)
  })
})

describe('canForRecord: self scope', () => {
  it('enforces the self relationship whenever a capability resolves to SELF scope', () => {
    const context = { role:'role_outside_the_system_matrix',employeeId:'emp-1',customCapabilities:[CAPABILITIES.VIEW_MY_PROFILE] }
    expect(scopeFor(CAPABILITIES.VIEW_MY_PROFILE,context)).toBe(DATA_SCOPES.SELF)
    expect(canForRecord(CAPABILITIES.VIEW_MY_PROFILE,{employeeId:'emp-1'},context)).toBe(true)
    expect(canForRecord(CAPABILITIES.VIEW_MY_PROFILE,{employeeId:'emp-2'},context)).toBe(false)
  })
...
describe('canForRecord: assignment enforcement', () => {
  it('requires an active, matching assignment for every capability held by an assignment-only role', () => {
    ...
    expect(canForRecord(CAPABILITIES.REVIEW_CLINICAL,patient,{...baseContext,assignments:[{resourceType:'patient',resourceId:'patient-1',active:true}]})).toBe(true)
    expect(canForRecord(CAPABILITIES.REVIEW_CLINICAL,patient,{...baseContext,assignments:[{resourceType:'patient',resourceId:'patient-2',active:true}]})).toBe(false)
  })
  ...
  it('treats a cancelled, completed or expired assignment as inactive', () => {
    ...
    for (const status of ['cancelled','completed','expired']) {
      expect(canForRecord(CAPABILITIES.EDIT_COMMITTEE_MINUTES,committee,{role:ROLES.COMMITTEE_SECRETARIAT,organizationId:'org-1',assignments:[{resourceType:'committee',resourceId:'committee-1',status}]})).toBe(false)
    }
  })
})

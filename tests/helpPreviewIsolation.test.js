...
describe('Platform update center review actions', () => {
  ...
  it('fails loudly when no row was updated and brings postponed changes back after DEFER_DAYS', () => {
    ...
    expect(page).toContain('onReview={reviewUpdate}')
    const center = read('src/features/management/PlatformUpdateCenter.jsx')
    expect(center).toContain("onReview(item,'deferred')")
    expect(center).toContain("onReview(item,'approved')")
  })
})

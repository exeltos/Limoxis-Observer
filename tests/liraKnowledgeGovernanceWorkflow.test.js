...
describe('LIRA knowledge governance workflow',()=>{
 ...
 it('creates review-stage versions and copies chunks',()=>{expect(sql).toContain("if old.status<>'approved' then");expect(sql).toContain("'review'");expect(sql).toContain('insert into public.lira_knowledge_chunks')})

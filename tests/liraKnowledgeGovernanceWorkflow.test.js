...
describe('LIRA knowledge governance workflow',()=>{
 ...
 it('creates review-stage versions and copies chunks',()=>{expect(sql).toMatch(/old\.status\s*<>\s*'approved'/);expect(sql).toContain("'review'");expect(sql).toContain('insert into public.lira_knowledge_chunks')})
 it('stores revision audit snapshots',()=>{expect(sql).toContain('lira_knowledge_source_revisions');expect(sql).toMatch(/to_jsonb\(s\)/)})

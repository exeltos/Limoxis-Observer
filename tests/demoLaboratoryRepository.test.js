...
describe('Demo laboratory repository adapter', () => {
  ...
  it('drives the canonical record workflow without cloud calls', async () => {
    ...
    await repository.addAst('LAB-PARITY-002', 'result-id', { drug: 'Drug', sir: 'S' })
    ...
    expect(sample.microbiologyResults[0].ast[0]).toEqual(expect.objectContaining({ drug: 'Drug', sir: 'S' }))
    expect(sample.microbiologyResults[0].communications).toHaveLength(1)
  })

  it('flips the sample to completed as soon as a result is validated, matching the production repository', async () => {
    const repository = createDemoLaboratoryRepository()
    await repository.create({ draft: { sampleCode: 'LAB-PARITY-003', patient: 'Patient', patientId: 'PT-3', type: 'bloodCulture' } })
    await repository.saveResult('LAB-PARITY-003', { result: 'negative', validationStatus: 'validated' })
    expect(await repository.get('LAB-PARITY-003')).toEqual(expect.objectContaining({ status: 'completed' }))
  })
})

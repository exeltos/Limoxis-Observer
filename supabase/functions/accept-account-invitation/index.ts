...
Deno.serve(async(req)=>{
 ...
 const {error:memberError}=await admin.from('organization_members').update({status:'active'}).eq('user_id',cu.user.id).eq('status','invited');if(memberError){console.error('invitation membership activation failed',memberError);return reply({error:'Ο λογαριασμός ενεργοποιήθηκε, αλλά δεν ολοκληρώθηκε η πρόσβαση στον οργανισμό. Επικοινωνήστε με τον διαχειριστή.'},500)}

-- The management audit trigger is shared by tables with different schemas.
-- Access trigger rows through JSONB so columns such as metadata and organization_id
-- are only read when present on that table.
...
begin
  ...
  insert into public.system_audit_log(
    ...
    jsonb_build_object('source','management_center','operation',tg_op,'organization_id',v_org)
  );

...
begin
  ...
  values(v_assignment.organization_id,v_assignment.employee_user_id,v_email,'training_invitation','training_assignment',v_assignment.id,case when lower(coalesce(p_language,'el'))='en' then 'Training invitation: '||coalesce(v_program.payload->>'title','') else 'Πρόσκληση εκπαίδευσης: '||coalesce(v_program.payload->>'title','') end,jsonb_build_object('path','/training-access/'||v_token,'programTitle',v_program.payload->>'title','employeeName',v_assignment.payload->>'employeeName','dueDate',v_assignment.payload->>'dueDate','requiresAssessment',coalesce((v_program.payload->>'requiresAssessment')::boolean,false),'language',case when lower(coalesce(p_language,'el'))='en' then 'en' else 'el' end),'pending',0,v_now)

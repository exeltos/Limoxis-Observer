-- Wires the pharmacy consumption module (20260919190000) into the
-- indicator metric snapshot: ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014 §2.4's
-- formula is (Σύνολο DDD χορηγηθέντος αντιβιοτικού ÷ 100 ασθενείς-ημέρες).
-- antibiotic_ddd_total sums quantity_grams/ddd_grams across dispensing
-- records in the period, resolving each antibiotic's DDD from
-- who_ddd_reference (preferring an organization-specific override over
-- the shared system value; a dispensing row with no matching reference
-- value at all is simply excluded from the sum rather than guessed at).
...
insert into public.indicator_definitions (
  ...
  (null,'antibiotic-ddd-per-100-patient-days','1.0','Κατανάλωση αντιβιοτικών (DDD ανά 100 ασθενοημέρες)','Antibiotic consumption (DDD per 100 patient-days)','pharmacy','antibiotic_ddd_total','patient_days',100,'DDD/100 ασθενοημέρες','DDD/100 patient-days','Φαρμακείο · ΕΟΔΥ / WHO ATC-DDD · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower')

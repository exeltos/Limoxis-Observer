...
create policy indicator_snapshots_read on public.indicator_snapshots
for select to authenticated
using (
  (
    public.current_user_is_platform_owner()
    or public.current_user_has_capability(organization_id,'view_indicators')
    or public.current_user_has_org_role(organization_id,array['hospital_admin','infection_control_lead','infection_control_member','doctor_reviewer','quality_manager']::public.app_role[])
  )
  and (
    not public.current_user_has_org_role(organization_id,array['link_nurse','department_manager']::public.app_role[])
    or (

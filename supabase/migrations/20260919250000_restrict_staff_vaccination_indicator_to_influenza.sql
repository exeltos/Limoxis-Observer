-- ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014's staff vaccination coverage indicator is
-- specifically the ANNUAL SEASONAL INFLUENZA vaccination rate among active
-- staff, not "any vaccination on file". The ratio (vaccinated staff ÷
-- active staff) × 100 was already correct, but the underlying
-- active_staff_with_vaccination metric counted staff with ANY vaccination
-- record (Hepatitis B, COVID-19, MMR, varicella, ...), silently
-- over-counting flu coverage with unrelated jabs. The staff vaccination
-- editor (src/features/prevention/StaffVaccinationEditor.jsx) writes a
-- fixed label into vaccine_label_snapshot, with the seasonal flu option
-- spelled 'Εποχική γρίπη' — restrict the count to that (and broader
-- γρίπη/influenza/flu variants, for older or English-language records).
...
v_pps_total bigint:=0;v_pps_hai bigint:=0;v_pps_abx bigint:=0;
begin
...
set title_el='Εμβολιαστική κάλυψη προσωπικού (αντιγριπικός εμβολιασμός)',
    title_en='Staff influenza vaccination coverage',
    numerator_definition=jsonb_set(coalesce(numerator_definition,'{}'::jsonb),'{description}','"Active staff with a valid seasonal influenza vaccination record"'::jsonb)
where indicator_key='vaccination-coverage' and organization_id is null;

create or replace function public.training_check_in(p_token text)
returns jsonb
language plpgsql
security invoker
...
begin
  ...
  where record_type='program'
    and payload->>'checkInToken'=p_token
  ...
  from public.training_records
  where organization_id=v_program.organization_id
    and record_type='assignment'
    and employee_user_id=auth.uid()
    and payload->>'programId'=v_program.record_key
  limit 1;
  ...
  v_payload:=v_assignment.payload || jsonb_build_object(
    ...
    'checkInAt',coalesce(v_assignment.payload->>'checkInAt',v_now::text),
    'status',case when coalesce(v_assignment.payload->>'status','assigned')='assigned' then 'in_progress' else coalesce(v_assignment.payload->>'status','assigned') end,
  ...
  return jsonb_build_object('assignmentId',v_assignment.record_key,'programId',v_program.record_key,'status',v_payload->>'status','checkInAt',v_payload->>'checkInAt');
end;
$$;
...
begin
  ...
  from public.training_records
  where record_type='program'
    and payload->>'completionToken'=p_token
    and is_org_member(organization_id)
  limit 1;
  if not found then raise exception 'TRAINING_ACCESS_NOT_AVAILABLE'; end if;
  ...
  if v_requires then
    ...
    loop
      ...
      if (p_answers->>(v_question->>'id'))::integer=coalesce((v_question->>'correctIndex')::integer,-1) then
        v_earned:=v_earned+coalesce((v_question->>'points')::numeric,1);
      end if;
    end loop;
  ...
  v_payload:=v_assignment.payload || jsonb_build_object(
    'attendance',true,'attendanceResponse','confirmed',
    'checkInAt',coalesce(v_assignment.payload->>'checkInAt',v_now::text),
    'completionConfirmedAt',v_now::text,'feedbackSubmittedAt',v_now::text,
    'assessmentSubmittedAt',case when v_requires then v_now::text else null end,
    'status','completed','completedDate',v_now::date::text,'score',v_score,'competent',v_competent,
    'certificateId',v_certificate_key,'feedbackScores',coalesce(p_feedback_scores,'{}'::jsonb),
  ...
  if v_competent then
    ...
    values(v_program.organization_id,v_certificate_key,'certificate',v_assignment.department_id,auth.uid(),jsonb_build_object(
      'id',v_certificate_key,'assignmentId',v_assignment.record_key,'employeeId',v_assignment.payload->>'employeeId',
      'title',v_program.payload->>'title','issuedDate',v_now::date::text,
      'validUntil',case when nullif(v_program.payload->>'validMonths','') is null then null else (v_now::date + make_interval(months=>(v_program.payload->>'validMonths')::integer))::date::text end,
      'issuer','Limoxis Observer','issuedAt',v_now::text,'issuedById',auth.uid()::text
    ),auth.uid(),auth.uid())

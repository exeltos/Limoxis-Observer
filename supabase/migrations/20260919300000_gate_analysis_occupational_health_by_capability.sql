-- Analysis-page audit finding (P0): platform_report_summary's own access
-- gate only checks org role ('hospital_admin' or 'infection_control_lead'),
-- not the view_occupational_health capability. 20260919120000 deliberately
-- stripped hospital_admin's view_occupational_health/manage_occupational_health
-- from the blanket capability grant ("Hospital Admin should have no
-- automatic access to this domain") — but this security definer function,
-- which bypasses RLS entirely, was never updated to match: any
-- hospital_admin still got an unconditional occupational_health_visits
-- count via the Analysis page's Employees tab, reintroducing in aggregate
-- form the exact access this codebase already decided to remove.
--
-- Fix: gate the occupationalHealth field behind
-- current_user_has_capability(...,'view_occupational_health'), the same
-- capability the rest of the app now uses for this domain, instead of the
-- coarser role check. Everyone who currently sees a real count here still
-- does (platform_owner and infection_control_lead both hold the
-- capability); hospital_admin now gets null for this one field, same as
-- the department-scoped case already does.

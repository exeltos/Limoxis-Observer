-- Wires the point prevalence survey module (20260919230000) into the
-- indicator metric snapshot. ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014 §2.2's
-- formula for HAI prevalence is (patients with active HAI ÷ total
-- inpatients that day) × 100; the companion antibiotic-use prevalence
-- follows the same shape with patients on antibiotics as the numerator.
-- Both simply sum the survey's own columns across surveys in the period
-- (normally one snapshot day per campaign) — no cross-table join needed,
-- unlike every other indicator in this series.
...
insert into public.indicator_definitions (
  ...
  (null,'hai-prevalence-pps','1.0','Σημειακός επιπολασμός νοσοκομειακών λοιμώξεων (PPS)','HAI point prevalence (PPS)','surveillance','pps_patients_with_hai','pps_patients_total',100,'%','%','ΕΝΛ · ΕΟΔΥ / ECDC PPS · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'antibiotic-use-prevalence-pps','1.0','Σημειακός επιπολασμός χρήσης αντιβιοτικών (PPS)','Antibiotic-use point prevalence (PPS)','surveillance','pps_patients_on_antibiotics','pps_patients_total',100,'%','%','ΕΝΛ · ΕΟΔΥ / ECDC PPS · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower')

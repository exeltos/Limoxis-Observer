-- Three of the five Employees sub-record tabs (training summary, evaluations,
-- certificates) had no real table at all — unlike occupational_health_visits
-- and employee_vaccinations (both v0.9.0), which already existed. These are
-- administrative/HR records about staff development, not clinical medical
-- data, so RLS mirrors public.employees' own read/write pattern
-- (hr_office/manage_staff_admin) rather than the stricter
-- occupational_physician-only pattern used for clinical workforce data.
-- Note: public.employees has no user_id linking a row to a login account
-- (it's an administrative registry, not tied 1:1 to auth.users), so unlike
-- some other domains this pass, there is no "employee sees their own record"
-- self-read clause to add here — matches the parent employees_read policy's
-- own role/capability-only shape exactly.

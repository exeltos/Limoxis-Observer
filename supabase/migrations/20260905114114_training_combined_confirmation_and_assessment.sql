...
begin
 ...
 if v_assignment.employee_user_id is not null then
  if auth.uid() is null or auth.uid()<>v_assignment.employee_user_id then raise exception 'TRAINING_AUTH_REQUIRED'; end if;
  if not is_org_member(v_assignment.organization_id) then raise exception 'TRAINING_ACCESS_NOT_AVAILABLE'; end if;
 end if;
 ...
 return jsonb_build_object('program',v_program_payload,'assignment',(v_assignment.payload-'accessToken')||jsonb_build_object('id',v_assignment.record_key,'dbId',v_assignment.id,'userId',v_assignment.employee_user_id,'departmentId',v_assignment.department_id,'externalAccess',v_assignment.employee_user_id is null));
end;$function$;
...
begin
 select * into v_assignment from public.training_records where record_type='assignment' and payload->>'accessToken'=p_token limit 1;
 if not found then raise exception 'TRAINING_ACCESS_NOT_AVAILABLE'; end if;
 if v_assignment.employee_user_id is not null and (auth.uid() is null or auth.uid()<>v_assignment.employee_user_id) then raise exception 'TRAINING_AUTH_REQUIRED'; end if;
...
begin
 ...
 v_payload:=v_assignment.payload||jsonb_build_object(
  ...
  'attendanceConfirmedAt',coalesce(v_assignment.payload->>'attendanceConfirmedAt',v_now::text),
  'feedbackSubmittedAt',v_now::text,
  'assessmentSubmittedAt',case when v_requires then v_now::text else null end,
  'assessmentAnswers',coalesce(p_answers,'{}'::jsonb),
  'assessmentReviewStatus',case when v_manual_review then 'pending' else 'not_required' end,
  'completionConfirmedAt',case when v_manual_review then null else v_now::text end,
  'status',case when v_manual_review then 'in_progress' else 'completed' end,
  'completedDate',case when v_manual_review then null else v_now::date::text end,
  'score',v_score,
  'competent',case when v_manual_review then null else v_competent end,
  'certificateId',v_certificate_key,
  'feedbackScores',coalesce(p_feedback_scores,'{}'::jsonb),
  'feedbackComment',nullif(btrim(coalesce(p_feedback_comment,'')),''),
  'updatedAt',v_now::text,
  'updatedById',case when auth.uid() is null then null else auth.uid()::text end
 );
 update public.training_records set payload=v_payload,updated_by=coalesce(auth.uid(),updated_by),updated_at=v_now where id=v_assignment.id;
 ...
 if v_certificate_key is not null then
  ...
  values(v_program.organization_id,v_certificate_key,'certificate',v_assignment.department_id,v_assignment.employee_user_id,jsonb_build_object('id',v_certificate_key,'assignmentId',v_assignment.record_key,'employeeId',v_assignment.payload->>'employeeId','title',v_program.payload->>'title','issuedDate',v_now::date::text,'validUntil',case when nullif(v_program.payload->>'validMonths','') is null then null else (v_now::date+make_interval(months=>(v_program.payload->>'validMonths')::integer))::date::text end,'issuer','Limoxis Observer','issuedAt',v_now::text,'issuedById',case when auth.uid() is null then null else auth.uid()::text end),coalesce(auth.uid(),v_assignment.created_by),coalesce(auth.uid(),v_assignment.updated_by))
  on conflict (organization_id,record_key) do nothing;
 end if;
 return jsonb_build_object('assignmentId',v_assignment.record_key,'programId',v_program.record_key,'score',v_score,'competent',case when v_manual_review then null else v_competent end,'manualReviewRequired',v_manual_review,'certificateId',v_certificate_key,'attendanceConfirmedAt',v_payload->>'attendanceConfirmedAt','completedAt',v_payload->>'completionConfirmedAt');
end;$function$;

create or replace function public.queue_training_invitation(p_assignment_key text,p_language text default 'el')
returns jsonb language plpgsql security definer set search_path to 'public','auth','pg_temp'
as $function$
declare v_assignment public.training_records%rowtype; v_program public.training_records%rowtype; v_email text; v_token text; v_now timestamptz:=now(); v_payload jsonb; v_outbox_id uuid; v_question_count integer:=0;
begin
 ...
 insert into public.notification_outbox(organization_id,recipient_user_id,recipient_email,notification_type,entity_type,entity_id,subject,payload,status,attempts,available_at) values(v_assignment.organization_id,v_assignment.employee_user_id,v_email,'training_invitation','training_assignment',v_assignment.id,case when lower(coalesce(p_language,'el'))='en' then 'Training confirmation & evaluation: '||coalesce(v_program.payload->>'title','') else 'Επιβεβαίωση συμμετοχής & αξιολόγηση: '||coalesce(v_program.payload->>'title','') end,jsonb_build_object('path','/training-access/'||v_token,'programTitle',v_program.payload->>'title','employeeName',v_assignment.payload->>'employeeName','dueDate',v_assignment.payload->>'dueDate','requiresAssessment',coalesce((v_program.payload->>'requiresAssessment')::boolean,false),'questionCount',v_question_count,'externalAccess',v_assignment.employee_user_id is null,'language',case when lower(coalesce(p_language,'el'))='en' then 'en' else 'el' end),'pending',0,v_now) returning id into v_outbox_id;
...
revoke execute on function public.training_submit_evaluation(text,jsonb,jsonb,text) from public;
revoke execute on function public.queue_training_invitation(text,text) from public,anon;
grant execute on function public.training_email_access(text) to anon,authenticated;
grant execute on function public.training_confirm_attendance(text) to anon,authenticated;
grant execute on function public.training_submit_evaluation(text,jsonb,jsonb,text) to anon,authenticated;

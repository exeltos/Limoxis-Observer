-- ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014 §2.6 requires monthly reporting of new
-- MDR/XDR/PDR patient isolation events, per department and per pathogen.
-- isolation_episodes already captures the isolation record itself (start/
-- end, department, precautions) but has no pathogen field; the pathogen
-- is only known via the linked surveillance case's positive, MDR/XDR/PDR-
-- classified microbiology result — the same join already used for the
-- resistant_active_surveillance and mdro_bsi metrics. This adds a
-- mdr_isolation_total plus one metric per reference pathogen, counting
-- distinct isolation episodes that started in the period and are linked
-- to a validated MDR/XDR/PDR isolate of that pathogen.
...
v_ddd_total numeric:=0;
v_iso_total bigint:=0;v_iso_ecoli bigint:=0;v_iso_proteus bigint:=0;v_iso_acineto bigint:=0;v_iso_kleb bigint:=0;v_iso_entb bigint:=0;v_iso_pseudo bigint:=0;v_iso_saureus bigint:=0;v_iso_entc bigint:=0;
begin
  ...
  where a.organization_id=p_organization_id and m.result_status='positive' and m.validation_status in ('validated','amended') and l.collected_at::date between p_from and p_to and (p_department_id is null or l.department_id=p_department_id)
    and not exists(select 1 from public.microbiology_results m2 where m2.organization_id=m.organization_id and m2.amended_from=m.id);
  select coalesce(sum(d.quantity_grams / (
    select r.ddd_grams from public.who_ddd_reference r
  ...
  from public.antibiotic_dispensing_periods d
  join public.master_library_items lib on lib.id=d.antibiotic_item_id
  where d.organization_id=p_organization_id and d.period_start>=p_from and d.period_end<=p_to and (p_department_id is null or d.department_id=p_department_id);
  select
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%escherichia coli%' or lower(coalesce(m.organism,'')) like '%proteus%' or lower(coalesce(m.organism,'')) like '%acinetobacter%' or lower(coalesce(m.organism,'')) like '%klebsiella%' or lower(coalesce(m.organism,'')) like '%enterobacter%' or lower(coalesce(m.organism,'')) like '%pseudomonas%' or lower(coalesce(m.organism,'')) like '%staphylococcus aureus%' or lower(coalesce(m.organism,'')) like '%enterococcus%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%escherichia coli%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%proteus%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%acinetobacter%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%klebsiella%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%enterobacter%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%pseudomonas%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%staphylococcus aureus%'),
    count(distinct ie.id) filter(where lower(coalesce(m.organism,'')) like '%enterococcus%')
  into v_iso_total,v_iso_ecoli,v_iso_proteus,v_iso_acineto,v_iso_kleb,v_iso_entb,v_iso_pseudo,v_iso_saureus,v_iso_entc
  from public.isolation_episodes ie
  ...
  where ie.organization_id=p_organization_id and s.voided_at is null and ie.started_at::date between p_from and p_to and (p_department_id is null or ie.department_id=p_department_id)
...
insert into public.indicator_definitions (
  ...
  (null,'mdr-isolation-new-cases','1.0','Νέες απομονώσεις MDR/XDR/PDR','New MDR/XDR/PDR isolation events','surveillance','mdr_isolation_total',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-ecoli','1.0','Νέες απομονώσεις MDR – E. coli','New MDR isolation events – E. coli','surveillance','mdr_isolation_ecoli',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-proteus','1.0','Νέες απομονώσεις MDR – Proteus spp.','New MDR isolation events – Proteus spp.','surveillance','mdr_isolation_proteus',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-acinetobacter','1.0','Νέες απομονώσεις MDR – Acinetobacter spp.','New MDR isolation events – Acinetobacter spp.','surveillance','mdr_isolation_acinetobacter',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-klebsiella','1.0','Νέες απομονώσεις MDR – Klebsiella spp.','New MDR isolation events – Klebsiella spp.','surveillance','mdr_isolation_klebsiella',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-enterobacter','1.0','Νέες απομονώσεις MDR – Enterobacter spp.','New MDR isolation events – Enterobacter spp.','surveillance','mdr_isolation_enterobacter',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-pseudomonas','1.0','Νέες απομονώσεις MDR – Pseudomonas spp.','New MDR isolation events – Pseudomonas spp.','surveillance','mdr_isolation_pseudomonas',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-saureus','1.0','Νέες απομονώσεις MDR – S. aureus','New MDR isolation events – S. aureus','surveillance','mdr_isolation_saureus',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower'),
  (null,'mdr-isolation-enterococcus','1.0','Νέες απομονώσεις MDR – Enterococcus spp.','New MDR isolation events – Enterococcus spp.','surveillance','mdr_isolation_enterococcus',null,1,'περιστατικά','cases','Τμήματα νοσηλείας · ΝΕΛ · ΕΟΔΥ (Πρόγραμμα Προκρούστης) · ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014','active','auto','lower')

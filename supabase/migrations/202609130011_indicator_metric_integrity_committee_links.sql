...
begin
 ...
 select count(*),coalesce(sum(pd.patient_days),0) into v_daily_count,v_patient_days
 from public.patient_days pd
 where pd.organization_id=p_organization_id and pd.census_date between p_from and p_to and (p_department_id is null or pd.department_id=p_department_id);
 if v_daily_count=0 then
   select coalesce(sum(pp.patient_days),0) into v_patient_days
   from public.patient_day_periods pp
   where pp.organization_id=p_organization_id and pp.period_start>=p_from and pp.period_end<=p_to and (p_department_id is null or pp.department_id=p_department_id);
 end if;

 select count(*) into v_active
 from public.surveillance_cases s
 where s.organization_id=p_organization_id and s.voided_at is null and s.started_at::date<=p_to and (s.closed_at is null or s.closed_at::date>=p_from) and (p_department_id is null or s.department_id=p_department_id);
 ...
 where s.organization_id=p_organization_id and s.voided_at is null and s.started_at::date<=p_to and (s.closed_at is null or s.closed_at::date>=p_from) and (p_department_id is null or s.department_id=p_department_id) and m.result_status='positive'
   and (upper(coalesce(m.resistance_class,'')) in ('MDR','XDR','PDR') or exists(select 1 from public.amr_classifications a where a.organization_id=p_organization_id and a.microbiology_result_id=m.id and a.status<>'rejected' and upper(coalesce(a.classification,'')) in ('MDR','XDR','PDR')));

 select coalesce(sum(h.compliant_observations),0),coalesce(sum(h.observations),0) into v_hh_ok,v_hh_total
 from public.hand_hygiene_sessions h
 where h.organization_id=p_organization_id and h.status='completed' and h.observation_date between p_from and p_to and (p_department_id is null or h.department_id=p_department_id);

 select count(*) filter(where b.score>=100),count(*) into v_bundle_ok,v_bundle_total
 from public.prevention_bundle_assessments b
 where b.organization_id=p_organization_id and b.status='completed' and b.assessment_date between p_from and p_to and (p_department_id is null or b.department_id=p_department_id);

 select coalesce(sum(a.litres),0) into v_abhr
 from public.antiseptic_consumption_periods a
 where a.organization_id=p_organization_id and a.period_start>=p_from and a.period_end<=p_to and (p_department_id is null or a.department_id=p_department_id);

 select count(*) into v_staff
 from public.employees e
 where e.organization_id=p_organization_id and e.employment_status='active' and (p_department_id is null or e.department_id=p_department_id);

 select count(distinct v.employee_id) into v_vaccinated
 from public.employee_vaccinations v join public.employees e on e.id=v.employee_id
 where v.organization_id=p_organization_id and e.employment_status='active' and v.vaccination_date<=p_to and (v.valid_until is null or v.valid_until>=p_to)
   and lower(coalesce(v.status,'active')) not in ('cancelled','canceled','rejected','expired','inactive','voided') and (p_department_id is null or e.department_id=p_department_id);

 select count(*) filter(where coalesce(t.payload->>'status','')='completed'),count(*) into v_training_ok,v_training_total
 from public.training_records t
 where t.organization_id=p_organization_id and t.record_type='assignment'
 ...
 where q.organization_id=p_organization_id and q.occurred_at::date between p_from and p_to and q.severity='high' and q.status<>'closed' and (p_department_id is null or q.department_id=p_department_id);

 select count(distinct m.id) into v_mdro
 from public.microbiology_results m join public.laboratory_samples l on l.id=m.sample_id
 where m.organization_id=p_organization_id and m.result_status='positive' and l.collected_at::date between p_from and p_to
...
-- System seed correction: migration execution has no authenticated actor, so suppress the
-- row-level management audit trigger only for this seed correction. The trigger is restored
-- in the same transaction and runtime user edits remain fully audited.

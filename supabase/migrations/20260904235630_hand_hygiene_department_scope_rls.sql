...
as $$
  select
    ...
    or (
      ...
      and (
        public.current_user_has_capability(target_org,'view_prevention')
        or public.current_user_has_capability(target_org,'record_hand_hygiene')
...
create policy hand_hygiene_read
on public.hand_hygiene_sessions
for select
using (public.current_user_can_read_hand_hygiene(organization_id,department_id));

drop policy if exists hand_hygiene_write on public.hand_hygiene_sessions;
create policy hand_hygiene_write
on public.hand_hygiene_sessions
for all
using (public.current_user_can_write_hand_hygiene(organization_id,department_id))
with check (public.current_user_can_write_hand_hygiene(organization_id,department_id));
...
using (
  exists (
    select 1 from public.hand_hygiene_sessions s
    where s.id=session_id
      and s.organization_id=organization_id
      and public.current_user_can_read_hand_hygiene(s.organization_id,s.department_id)
  )
);
...
using (
  exists (
    ...
    where s.id=session_id
      and s.organization_id=organization_id
      and public.current_user_can_write_hand_hygiene(s.organization_id,s.department_id)
  )
)
with check (
  exists (

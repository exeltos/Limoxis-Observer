-- CRITICAL HOTFIX: private.indicator_metric_snapshot's single
-- jsonb_build_object(...) call had grown, across the recent ΕΟΔΥ
-- indicator series, to 52 key/value pairs = 104 arguments. PostgreSQL
-- hard-caps any function call (including built-in variadic functions
-- like jsonb_build_object) at 100 arguments (FUNC_MAX_ARGS), so every
-- call to this RPC was failing outright with "cannot pass more than 100
-- arguments to a function" — breaking the entire Indicators page (shows
-- zero active indicators / a raw error toast) in production, for every
-- organization.
--
-- Fix: split the one oversized jsonb_build_object call into four
-- smaller ones (well under the limit) and concatenate them with the
-- jsonb `||` operator. Output is byte-for-byte identical; only the
-- construction is split. Applied directly to the live project first
-- (production outage), this migration brings the tracked history back
-- in sync.

...
begin
  ...
  if tg_table_name = 'custom_role_capabilities' then
    if tg_op = 'DELETE' then
      select r.organization_id into v_org from public.custom_roles r where r.id = old.custom_role_id;
    else
      select r.organization_id into v_org from public.custom_roles r where r.id = new.custom_role_id;
    end if;

-- ΥΑ Υ1.Γ.Π.114971/ΦΕΚ Β 388/2014 requires resistance/bacteremia surveillance
-- for 8 reference pathogens: E. coli, Proteus, Acinetobacter, Klebsiella,
-- Enterobacter, Pseudomonas, S. aureus, Enterococcus. The microorganisms
-- baseline library only ever seeded 6 of these 8 (Proteus and Enterobacter
-- were missing), so hospitals could not even record an isolate of either
-- species through the standard "select from library" workflow. This adds
-- the two missing species to the baseline seed function (so new
-- organizations get all 8 going forward) and backfills existing
-- organizations, identical in structure to 20260902154615's seed.

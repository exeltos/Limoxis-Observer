...
## Committee create form: three UX fixes from live testing
User feedback after testing committee creation: the save button felt "stuck" with no explanation, the dialog felt cramped, and it opened pre-filled with a specific template's content instead of blank.

**Save button silently disabled — added visible, specific feedback instead of changing the validation rules.** The rules themselves (name, role, mandate, both term dates in the right order, at least one member with person+title+responsibilities all filled in) look intentional for something as formal as a hospital governance committee, not something to loosen without being asked. Added a `missingReasons` list computed alongside the existing `valid` check, rendered as a plain-language sentence near the save button ("Χρειάζεται ακόμα για αποθήκευση: ..."), naming exactly what's missing — including the easy-to-miss requirement that *every* member row needs its own title and responsibilities filled in, not just the person selected.

**Dialog widened**: `ObserverDialog`'s `width="wide"` → `width="workspace"` (an existing, already-defined size step, not a new one) for the modal version in `CommitteesPage.jsx`. The standalone `CommitteeCreatePage.jsx` was already full-width (`workspace-fill`), so only got the validation-feedback fix, not a width change.

**Stopped defaulting to the first catalog template's content.** Both the modal dialog and the standalone create page previously initialized `draft.name`/`committeeRole`/`mandate`/`legalBasis` straight from `IPC_COMMITTEE_CATALOG[0]` (whichever committee type happens to be listed first) — meaning every new committee opened pre-filled with real institutional text the user then had to notice and clear. Switched the default to the catalog's own `'custom'` entry, which exists specifically for this ("Άλλη επιτροπή / ομάδα", blank role/mandate/legal-basis) — but had to fix a subtlety: that catalog entry's own `name` field ("Άλλη επιτροπή / ομάδα") is really just the *dropdown option label*, not a value meant to populate the actual committee-name field. Naively defaulting to `first.name` would have shown "Άλλη επιτροπή / ομάδα" as the committee's name instead of a blank field — set the initial draft's text fields to genuinely empty strings directly, matching what `chooseTemplate()`'s own existing `t.id==='custom'` special-casing already does when a user picks "custom" from the dropdown mid-form, rather than only fixing the initial mount.

Full pipeline verified clean: `lint` (0/0), `test` (93/93 across 13 files), `build` clean.

## Fixed: "Δεν ήταν δυνατή η αποθήκευση της επιτροπής" — same UUID-vs-employee_code mismatch, now in Committees' member insert
Reported after the user successfully filled in the (now-visible) required fields and tried to save. Root cause was the same class of bug as the earlier live-confirmed `demo-hospital` issue: `committee_members.employee_id` is a real uuid foreign key into `employees.id`, but the member picker's `<select>` (and everything downstream of it — `memberRefs`, the insert payload) had only ever carried the frontend-facing `employee_code` (e.g. "EMP-001") under the name `employeeId`. Inserting that string into a uuid column fails outright.

**Traced the actual chain, not just the failing insert.** The `staff` array both create flows (the modal dialog in `CommitteesPage.jsx` and the standalone `CommitteeCreatePage.jsx`) build from `useEmployeesData()` never carried the real database uuid (`dbId`, added to `employeeService.js` during the Employee sub-records work) at all — only the display-facing `id`/`employee_code`. Added `dbId` to both `staff` mappings, threaded it through as a new `employeeDbId` field on each `memberRefs` entry, and updated `committeeService.js`'s insert to use `employeeDbId` for the real `employee_id` column while keeping `employeeId` (the code) for on-screen display and local-mode compatibility.

**Caught and fixed a second, related inconsistency while doing this, before it caused its own confusing bug later**: loading an *existing* committee's members back out of the database had the opposite problem — `fromMemberRow()` was reading the real uuid into the frontend's `employeeId` field, when every other part of the app expects `employeeId` to be the human-facing code (e.g. any `staff.find(x => x.id === m.employeeId)` lookup would silently never match a loaded member). Added an `employee:employees(employee_code)` embed to the member select (checked first that `committee_members → employees` has only one foreign key, unlike the earlier `committees` embed ambiguity, so no disambiguation hint was needed) and fixed `fromMemberRow()` to read the code from the embed and the uuid separately into `employeeDbId`, matching the create path's shape exactly. Also fixed the in-memory "just created" object `createCommitteeAsync` builds for its own return value (used to update UI state immediately after a successful save, before any reload) to match this same corrected shape.

Full pipeline verified clean: `lint` (0/0), `test` (93/93 across 13 files), `build` clean.

## Fixed: "Αυτός ο κωδικός επιτροπής χρησιμοποιείται ήδη" — orphaned row from the earlier bug, plus its actual root cause
Live database check confirmed the direct cause: one committee row (`COM-001`) existed with **zero members** — the exact leftover from testing the earlier `employee_id`-uuid bug, where the committee insert had already succeeded before the (now-fixed) member insert failed. Deleted it after confirming it genuinely had no members (not real data).

**But the deeper, actual root cause was separate and needed its own fix**: committee code generation (`nextCommitteeId()`) has always been purely local — `COM-${localRows.length + 1}`, blind to what's actually in the cloud. Once *any* cloud-created committee exists, the local seed data's count and the real database's codes drift apart, and a fresh code generated locally can collide with a real one. Added `getNextCommitteeCodeAsync()` to `committeeService.js`, which asks the live `committees` table which codes are already taken and returns the first genuinely free `COM-NNN`, used first in both create flows with the local generator kept only as the demo/no-cloud fallback.

**Also fixed the failure mode that created the orphan in the first place**, not just its one-time symptom: `createCommitteeAsync` now deletes the just-inserted committee row if the subsequent member insert fails, instead of leaving a permanent, member-less placeholder sitting on that code forever. A genuine code collision between two different concurrent creates can still happen in principle (`getNextCommitteeCodeAsync` and the insert are two separate round-trips) — left the existing `DUPLICATE_COMMITTEE_CODE` unique-constraint catch in place as the safety net for that rare case, rather than trying to make code allocation fully atomic for what's a low-concurrency, human-paced action.

Full pipeline verified clean: `lint` (0/0), `test` (93/93 across 13 files), `build` clean.

## Fixed: certificate attachment silently disappearing after save
Two things reported together: (1) committee meetings/decisions/plan-items/attendance/minutes-approvals/history don't save to the cloud, and (2) a file attached to a new employee certificate vanished, requiring a re-upload.

**(1) is expected, already-documented behavior, not a new bug** — flagged explicitly in the Committees domain README entry above: only `committees` + `committee_members` are cloud-wired; the meeting/decision/plan sub-workflow is a deliberately separate, not-yet-started follow-up because of how tightly it's coupled to a single mutable local array in the current architecture.

**(2) was a real, confirmed bug**, and specifically a consequence of the file-storage design documented above: `AttachmentField` only gets real cloud storage once a record has a genuine id (`entityId`); a *new* certificate has no id yet while its dialog is still open, so any file added at that point stays in the original local/base64 path — held only in React component state, never written anywhere durable. Once the certificate itself saves (now correctly going to the cloud, per the Employees sub-records work above), the certificate list re-fetches fresh from the database, which returns `attachments: []` for every row (attachments are looked up separately, never embedded in the certificate row) — silently discarding the staged file, since nothing had migrated it anywhere and nothing was holding onto it after the component re-rendered.

**Fix**: after a new certificate is successfully created and has a real id, the save handler now looks for any attachments still only in local/staged form (`objectUrl` present, no `storagePath` yet) and uploads each one for real, associated with the certificate's new id — recovering the file from its still-valid in-session `objectUrl` (fetched back into a `Blob`, wrapped in a `File`) rather than needing the original file input a second time. Failures are reported per-file rather than silently swallowed or failing the whole save (the certificate record itself is already safely persisted by that point; only the attachment upload might fail, e.g. on a flaky connection).
